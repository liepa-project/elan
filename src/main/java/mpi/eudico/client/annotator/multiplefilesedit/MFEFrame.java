...
//		addCloseActions();
	...
	/**
	 * Update localized interface elements.
	 */
        ...
        public void run() {
        	if ((domainPanel.getSearchFiles() != null) && (domainPanel.getSearchFiles().length != 0)) {
        		...
        		if (!parseErrorList.isEmpty()) {
        			StringBuffer buffer = new StringBuffer(ElanLocale.getString("MFE.Loader.ParseErrorMessage")).append("\n");
        			for (String file : parseErrorList) {
        				buffer.append(file).append("\n");
        			}
        			String title = ElanLocale.getString("MFE.Loader.ParseErrorTitle");
        			showMessageDialog(buffer.toString(), title);
        		}

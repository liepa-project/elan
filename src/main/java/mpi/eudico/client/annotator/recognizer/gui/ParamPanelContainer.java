...
public class ParamPanelContainer extends JPanel implements ParamPreferences {
	...
	/**
	 * Updates localized user interface elements.
	 */
	...
	private Color getBackgroundColorForPanel(int index){
		if(index % 2 != 0){
			return Constants.LIGHTBACKGROUNDCOLOR;
	 ...
	 * 
	 * @return the advanced parameter panel
	 */
	...
	private void showAdvanceParametersDialog(){
		if(advancedDialog == null){
			...
			closeButton.addActionListener(new ActionListener(){
				@Override
				public void actionPerformed(ActionEvent e) {
					WindowLocationAndSizeManager.storeLocationAndSizePreferences(
							advancedDialog, "RecognizerAdvancedParameterDialog");
		...
		WindowLocationAndSizeManager.postInit(advancedDialog, "RecognizerAdvancedParameterDialog",
	...
	public void setParamPreferences(Map<String, Object> storedPrefs) {
		for (Map.Entry<String, Object> entry : storedPrefs.entrySet()) {
			for (AbstractParamPanel p : panels) {
				if (p.getParamName() != null && p.getParamName().equals(entry.getKey())) {

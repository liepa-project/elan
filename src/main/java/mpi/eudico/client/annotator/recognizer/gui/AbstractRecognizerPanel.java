...
RecognizerHost, ElanLocaleListener, ItemListener, ACMEditListener {
	...
	/** reduce file prompt boolean */
	protected boolean reduceFilePrompt = true;
	/** last start time identifier */
	...
	/** boolean identifier to poll progress report*/
	...
	protected void initComponents() {
		...
		recognizerList = new JComboBox<String>();
	...
	/**
	 * Returns a map of available recognizers.
	 * 
	...
	private void createOutputFile(String path) {
		...
		try {
			...
			if (!out.exists()) {
				...
				if (created && currentRecognizer instanceof SharedRecognizer) {
					try {
...
//			}
	...
	public void detachParamPanel() {
		...
		WindowLocationAndSizeManager.postInit(detachedDialog, "RecognizerParameterDialog",
				Math.min(screen.width / 2 - 20, curRect.width), 
				Math.min(screen.height - 30, curRect.height));		
	...
	public void showHelpDialog(){
		try {
    		...
    		dialog.addWindowListener(new DialogCloseAdapter("RecognizerHelpDialog"));
    		dialog.pack();
    		WindowLocationAndSizeManager.postInit(dialog, "RecognizerHelpDialog", 600, 500);
	...
	protected void writeFile(String filepath, Object value, int contentType) throws IOException {		
		...
		if(value instanceof List){			
			...
			if (tier != null) {					
				...
				throw new IOException("The tier does not exist: " + value);
			}
		}
	}
	...
	protected void showReport() {
		...
		if (report != null) {
			...
			rD.pack();
			rD.addWindowListener(new DialogCloseAdapter("RecognizerReportDialog"));
			WindowLocationAndSizeManager.postInit(rD, "RecognizerReportDialog", 800, 400);
	...
	protected void updateProgressReport(String toAppend) {
		...
		if (progressReportTextArea != null) {
			EventQueue.invokeLater(() -> {
				...
				// XXX thread safety
				if (wasAtEnd) {
					progressReportTextArea.setCaretPosition(doc.getLength());
				}
			});
		}
	}
	...
	private class DialogCloseAdapter extends WindowAdapter {
		...
		/**
		 * The preferences key for storing size and location
		 * @param prefKey the preferences key
		 */
		public DialogCloseAdapter(String prefKey) {
			super();
		...
		public void windowClosing(WindowEvent e) {
			WindowLocationAndSizeManager.storeLocationAndSizePreferences(e.getWindow(), prefKey);
		}

...
public class SharedRecognizer extends LocalRecognizer {
	...
	/**
	 * A recognizer mediator that connects to a {@code REST} or {@code telnet}
	 * service. 
	 */
	...
	private void startTelnet() {
		try {
			...
			PrintWriter writer = new PrintWriter(new BufferedWriter(new OutputStreamWriter(socket.getOutputStream(), StandardCharsets.UTF_8)), true);// BufferedOutputStream, the input of the process
	...
	/**
	 * Runs a recognizer through a {@code HTTPUrlConnection} to a {@code REST}
	 * interface.
	 */
	...
	private String toUNC(String path) {
		...
		for (Map.Entry<String, String> entry: localToShareMap.entrySet()) {
			String local = entry.getKey();
			if (path.startsWith(local)) {
				return path.replaceFirst(local, entry.getValue()).replace('/', '\\');

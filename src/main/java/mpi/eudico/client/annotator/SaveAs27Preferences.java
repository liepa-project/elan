...
public class SaveAs27Preferences {
	...
	/**
	 * Saving the transcription passed as eaf format
	 * @param t the transcription
	 * @return value indicating the file type
	 */
	...
	private static Object convert2_7CVEntryPreferences(TranscriptionImpl t,
		...
		if (fromPrefs != null) {
			...
			for (Map.Entry<String, Map<String, Object>> fromEntry : frommap.entrySet()) {
				String cvName = fromEntry.getKey();
				ControlledVocabulary cv = t.getControlledVocabulary(cvName);
				if (cv == null) {
					continue;
				}
				Map<String, Object> fromCvEntries = fromEntry.getValue();
				Map<String, Object> toCvEntries = new HashMap<String, Object>();
				// Loop over the CV entry keys (ids or values)
				for (Map.Entry<String, Object> cvEntryEntry : fromCvEntries.entrySet()) {
					String fromKey = cvEntryEntry.getKey();
					Object entryPrefs = cvEntryEntry.getValue();
					String toKey = null;

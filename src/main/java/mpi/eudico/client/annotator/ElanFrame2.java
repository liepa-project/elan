...
import mpi.eudico.client.annotator.commands.global.AutoBackupSettingsMA;
import mpi.eudico.client.annotator.commands.global.AutoBackupToggleMA;
...
import mpi.eudico.client.annotator.commands.global.ExportAnnotatedPartMultiMA;
...
import mpi.eudico.client.annotator.commands.global.ImportCollectionFromServerMA;
...
import mpi.eudico.client.annotator.commands.global.MultiFileModifyBoundariesOfAnnotationsMA;
...
import mpi.eudico.client.annotator.commands.global.RestoreBackupMA;
...
public class ElanFrame2 extends JFrame implements ActionListener,
    ...
    /** menu file identifier */
    protected JMenu menuFile;
    /** menu item  file new identifier */
    protected JMenuItem menuItemFileNew;
    /** file open menu item identifier */
    protected JMenuItem menuItemFileOpen;
    /** file restore backup file menu item identifier */
	protected JMenuItem menuItemRestoreBackup;
    /** recent files menu identifier */
    protected JMenu menuRecentFiles;
    /** file exit menu item identifier */
    protected JMenuItem menuItemFileExit;
    /** backup menu identifier */
    protected JMenu menuBackup;
    /** import menu identifier */
    protected JMenu menuImport;
    /** export menu identifier */
    protected JMenu menuExport;
    /** annotation server menu identifier */
    protected JMenu menuAnnotationServer;
    /** export smil identifier */
    protected JMenu menuExportSMIL;
    /** shoebox import identifier */
    protected JMenuItem menuItemShoeboxImport;
    /** chati import identifier */
    protected JMenuItem menuItemCHATImport;
    /** transcriber import identifier */
    ...
    /** layout manager identifier */
    protected ElanLayoutManager layoutManager;
    /** viewer manager identifier */
    ...
    /** close menu item identifier */
    protected ElanMenuItem closeMI;
    /** save menu item identifier */
	protected ElanMenuItem saveMI;
	/** save as menu item identifier */
	...
	/** undo menu item identifier */
	protected ElanMenuItem undoMI;
	/** redo menu item identifier */
	...
	private ElanMenuItem exportJSONToServerMI;
	private ElanMenuItem listCollectionIDsMI;
	private ElanMenuItem importCollectionMI;
	private ElanMenuItem autoBackupSettingsMI;
	...
	private ButtonGroup automaticBackupGroup;
	private JCheckBoxMenuItem menuItemAutomaticBackupOn;
	private JCheckBoxMenuItem menuItemAutomaticBackupOff;
    ...
    void openEAF(String fullPath, List<String> mediaFiles) {
        ...
        if (!lowerPath.endsWith(".eaf") && !lowerPath.endsWith(".etf") && !FileUtility.isBackupFile(lowerPath)) {
            String strMessage = ElanLocale.getString("Menu.Dialog.Message1");
            strMessage += fullPath;
	...
	protected void setFrameIcon() {
		try {
			ImageIcon icon = new ImageIcon(ElanFrame2.class.getResource("/mpi/eudico/client/annotator/resources/ELAN16.png"));
			setIconImage(icon.getImage());
		} catch (Throwable t) {
			if (LOG.isLoggable(Level.INFO)) {
				LOG.log(Level.INFO, "Could not set the ELAN frame icon: " + t.getMessage());
			}
		}
	}
...
//        	}
        	...
        	setLocation((int) ((wRect.getWidth() / 2) - (getWidth() / 2d)), 
        			(int) ((wRect.getHeight() / 2) - (getHeight() / 2d)));
        }
    ...
    protected void initMenuBar() {
        ...
        ma = new RestoreBackupMA(ELANCommandFactory.RESTORE_BACKUP_DOC, this);
		menuActions.put(ELANCommandFactory.RESTORE_BACKUP_DOC, ma);
		menuItemRestoreBackup = new JMenuItem(ma);
		menuFile.add(menuItemRestoreBackup);
        ...
        menuItemAutomaticBackupOn = new JCheckBoxMenuItem(
				new AutoBackupToggleMA(ELANCommandFactory.AUTOMATIC_BACKUP_TOGGLE_ON, this));

		menuItemAutomaticBackupOff = new JCheckBoxMenuItem(
				new AutoBackupToggleMA(ELANCommandFactory.AUTOMATIC_BACKUP_TOGGLE_OFF, this));

		automaticBackupGroup = new ButtonGroup();
		automaticBackupGroup.add(menuItemAutomaticBackupOn);
		automaticBackupGroup.add(menuItemAutomaticBackupOff);

		Boolean autoBackup = Preferences.getBool("AutomaticBackupOn", null);
		Integer backupDelay = Preferences.getInt("BackUpDelay", null);
		
		// facilitating the previous version pre-existing settings as well
		if (autoBackup == null) {
			if(backupDelay == null) {
				menuItemAutomaticBackupOff.setSelected(true);
				Preferences.set("AutomaticBackupOn", false, null, false);
			} else if(backupDelay.intValue() == 0) {
				menuItemAutomaticBackupOff.setSelected(true);
				Preferences.set("AutomaticBackupOn", false, null, false);
			} else if (backupDelay.intValue() != 0) {
				menuItemAutomaticBackupOn.setSelected(true);
				Preferences.set("AutomaticBackupOn", true, null, false);
			}
		} else {
			if (autoBackup.booleanValue()) {
		...
		menuBackup.add(menuItemAutomaticBackupOn);
		menuBackup.add(menuItemAutomaticBackupOff);
        
        ma = new AutoBackupSettingsMA(ELANCommandFactory.AUTOMATIC_BACKUP_SETTINGS_MA, this);
		autoBackupSettingsMI = new ElanMenuItem(ma);
		menuActions.put(ELANCommandFactory.AUTOMATIC_BACKUP_SETTINGS_MA, ma);
		menuBackup.add(autoBackupSettingsMI);
       ...
       // JMenuItem menuItemMultipleFileAnnotationsFromOverlaps = new JMenuItem(ma);
        ...
        ma = new MultiFileModifyBoundariesOfAnnotationsMA(ELANCommandFactory.MODIFY_BOUNDARIES_ALL_ANNOTATIONS_MULTI, this);
        menuActions.put(ELANCommandFactory.MODIFY_BOUNDARIES_ALL_ANNOTATIONS_MULTI, ma);
        mfProcessMenu.add(new JMenuItem(ma));
		...
		ma = new MenuAction(ELANCommandFactory.EXPORT_JSON_TO_SERVER);
		exportJSONToServerMI = new ElanMenuItem(ma, false);
        menuActions.put(ELANCommandFactory.EXPORT_JSON_TO_SERVER, ma);
        menuAnnotationServer.add(exportJSONToServerMI);
		
		ma = new MenuAction(ELANCommandFactory.LIST_IDS_FROM_SERVER);
		listCollectionIDsMI = new ElanMenuItem(ma, false);
        menuActions.put(ELANCommandFactory.LIST_IDS_FROM_SERVER, ma);
        menuAnnotationServer.add(listCollectionIDsMI);
		
		ma = new ImportCollectionFromServerMA(ELANCommandFactory.IMPORT_COLLECTION_FROM_SERVER, this);
		importCollectionMI = new ElanMenuItem(ma);
		menuActions.put(ELANCommandFactory.IMPORT_COLLECTION_FROM_SERVER, ma);
		menuAnnotationServer.add(importCollectionMI);
        ...
        ma = new ExportAnnotatedPartMultiMA(ELANCommandFactory.EXPORT_ANN_PART_MULTI, this);
        menuActions.put(ELANCommandFactory.EXPORT_ANN_PART_MULTI, ma);
        exportMenuMulti.add(new JMenuItem(ma));
        ...
        menuItemSpectrogramViewer.setSelected(false);// Aug. 2023 changed default to false        
...
//        menuOptions.add(menuItemRateVol);
        ...
        ma = new SetLocaleMA(ELANCommandFactory.INDONESIAN, this, ElanLocale.INDONESIAN);
        langRBMI = new JRadioButtonMenuItem(ma);
        languageBG.add(langRBMI);
        menuAppLanguage.add(langRBMI);
        ...
        ma = new WebMA("Menu.Help.Website.PrivacyPolicy", this, Constants.ELAN_PRIVACY_POLICY_URL);
        menuActions.put("Menu.Help.Website.PrivacyPolicy", ma);
        websiteMenu.add(new JMenuItem(ma));
    ...
    /**
     * Clears shortcut map
     * @param modeConstant the mode constant
     */
...
//    	InputMap inputMap = getRootPane().getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
    ...
    /**
     * Initializes the menus and commands
     */
...
//        }     
        ...
        menuAnnotation.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame,
                ELANCommandFactory.MODIFY_ANNOTATION_RESOURCE_URL)));
        menuAnnotation.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame,
        ...
        ma = new MenuAction("Menu.Tier.Participant");
        menuActions.put("Menu.Tier.Participant", ma);
        JMenu participantMenu = new JMenu(ma);
        menuTier.add(participantMenu);
        
        participantMenu.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame,
        ...
        participantMenu.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
        		transcriptionForThisFrame, 
        		ELANCommandFactory.DELETE_PARTICIPANT)));
        ...
        menuTier.add(new ElanMenuItem(ELANCommandFactory.getCommandAction(
        		transcriptionForThisFrame, 
        		ELANCommandFactory.MODIFY_ALL_ANNOTATION_BOUNDARIES)));
        ...
        menuActions.remove(ELANCommandFactory.EXPORT_JSON_TO_SERVER);
        exportJSONToServerMI.setAction(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame, ELANCommandFactory.EXPORT_JSON_TO_SERVER),true);
        
        menuActions.remove(ELANCommandFactory.LIST_IDS_FROM_SERVER);
        listCollectionIDsMI.setAction(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame, ELANCommandFactory.LIST_IDS_FROM_SERVER),true);
        
        menuActions.remove(ELANCommandFactory.IMPORT_COLLECTION_FROM_SERVER);
        importCollectionMI.setAction(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame, ELANCommandFactory.IMPORT_COLLECTION_FROM_SERVER),true);
...
//        menuItemRateVol.setEnabled(true);
        ...
        JRadioButtonMenuItem customMI = new JRadioButtonMenuItem(ELANCommandFactory.getCommandAction(
                transcriptionForThisFrame, ELANCommandFactory.SET_CUSTOM_MS_PER_FRAME));
        videoGroup.add(palMI);
        palMI.setSelected(true);
        videoGroup.add(pal50MI);
        videoGroup.add(ntscMI);
        videoGroup.add(customMI);
        menuFrameLength.add(palMI);
        menuFrameLength.add(pal50MI);
        menuFrameLength.add(ntscMI);
        menuFrameLength.add(customMI);
...
//        }
    ...
    /**
     * Gets wave form viewer manager menu
     * @return the wave form viewer manager menu
     */
        ...
        SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					*/
        		    ...
        		    Boolean autoBackupOn = Preferences.getBool("AutomaticBackupOn", null);

        			if (autoBackupOn != null && autoBackupOn.booleanValue() == true) {
        				...
        				if ((backupDelay != null) && (backupDelay.intValue() > 0)) {
        					...
        					c.execute(ELANCommandFactory.getCommandAction(transcriptionForThisFrame,
        							ELANCommandFactory.BACKUP), new Object[] { backupDelay });
        				}
        			}
...
//        	}
		...
		if (intPref != null && transcriptionForThisFrame != null) {
			...
			((BackupCA) ELANCommandFactory.getCommandAction(transcriptionForThisFrame, ELANCommandFactory.BACKUP))
					.setNumBuFiles(nf);
		}
		...
		if (autoBackupOn != null) {
			if (autoBackupOn.booleanValue() == true) {

				if ((backupDelay != null) && (backupDelay.intValue() > 0)) {
					Command c = ELANCommandFactory.createCommand(transcriptionForThisFrame, ELANCommandFactory.BACKUP);
					c.execute(ELANCommandFactory.getCommandAction(transcriptionForThisFrame, ELANCommandFactory.BACKUP),
    ...
    */
    ...
    /**
     * boolean to determine if initialized or not

...
/**
 * A class that merges elements from one transcription to another transcription such that the merging can be undone. There
 * are several options for the merging; if a selection of tiers is provided those tiers and everything connected to them is
 * copied else everything is copied (types, controlled vocabularies, external refs, doc properties) Copying of linked files
 * etc is not (yet) provided for.
 *
 * @author Han Sloetjes
 */
public class UndoableTranscriptionMerger {
    ...
    private final Map<AbstractAnnotation, DefaultMutableTreeNode> createdAnnotations;
    private final Map<AbstractAnnotation, DefaultMutableTreeNode> deletedAnnotations;
    private final Map<AbstractAnnotation, DefaultMutableTreeNode> modifiedAnnotations;
    private final List<ArrayList<DefaultMutableTreeNode>> createdAnnotationSequences;

    private final Map<String, TierImpl> createdTierMap;
    private final Map<String, LinguisticType> createdTypeMap;
     ...
     * @param second the second transcription, the supplier
     * @param overwrite a flag to indicate whether annotations maybe overwritten
     * @param tiersToAdd a list of names of tiers that should be added to the first transcription, if null all tiers will
     *     be added
     */
    public void mergeWith(TranscriptionImpl first, TranscriptionImpl second, boolean overwrite, List<String> tiersToAdd) {
        ...
        for (int i = 0; i < sortedTierObj.size(); i++) {
            TierImpl tier = sortedTierObj.get(i);
        ...
        for (int i = 0; i < tiersWithoutParentInGroup.size(); i++) {
            TierImpl tier = transB.getTierWithId(tiersWithoutParentInGroup.get(i));
    ...
    public void undo() {
        ...
        for (int i = tierKeys.length - 1; i >= 0; i--) {
            ...
            TierImpl t = transA.getTierWithId(tierName);
            if (t != null) {
        ...
        while (caIter.hasNext()) {
            ...
            AbstractAnnotation curAnn = (AbstractAnnotation) t.getAnnotationAtTime((createdAnn.getBeginTimeBoundary()
                                                                                    + createdAnn.getEndTimeBoundary()) / 2);
            if (curAnn != null) {
        ...
        while (modIter.hasNext()) {
            ...
            if (recreatedModAnn == null) {
                LOG.warning("Could not recreate a modified annotation: "
                            + modAnn.getTier().getName()
                            + " BT: "
                            + modAnn.getBeginTimeBoundary());
            }
        }
        ...
        while (delIter.hasNext()) {
            ...
            if (recreatedAnn == null) {
                LOG.warning("Could not recreate a deleted annotation: "
                            + deletedAnn.getTier().getName()
                            + " BT: "
                            + deletedAnn.getBeginTimeBoundary());
            }
        }
    ...
    private void undoModifiedAnnotations() {
        ...
        while (modIter.hasNext()) {
            ...
            TierImpl tier = transA.getTierWithId(tierName);
    ...
    public void redo() {
        ...
        for (int i = 0; i < tierKeys.length; i++) {
            ...
            if (t == null) {
                ...
                if (oldTier.getParentTier() != null) {
                    parentTier = transA.getTierWithId(oldTier.getParentTier().getName());
        ...
        while (annIter.hasNext()) {
            ...
            // check and log
            if (nextAa == null) {
                LOG.warning("Could not create the annotation again in redo: "
                            + aa.getTier().getName()
                            + " BT: "
    ...
    /**
     * Creates a new top level tier and possibly sub-tiers. Note: the private methods assume that both transcription objects
     * are not null.
     *
    ...
    /**
     * Creates a new depending tier and possibly sub-tiers. Note: the private methods assume that both transcription objects
     * are not null.
     *
    ...
    private void addNewDependingTier(TierImpl tier, List<? extends Tier> allTiersToAdd) {
        ...
        TierImpl parentTier = transA.getTierWithId(parentName);
        if (parentTier != null) {
                    ...
                    if (typeComp) { // use it
                        ...
                        if (nextLTName != null) {
                            ...
                            if (nextConstraint != null) {
                                ...
                                nextTier = new TierImpl(parentTier, tier.getName(), tier.getParticipant(), transA, nextType);
                                nextTier.setAnnotator(tier.getAnnotator());
                                nextTier.setDefaultLocale(tier.getDefaultLocale());
                                nextTier.setLangRef(tier.getLangRef());
                                createdTierMap.put(nextTier.getName(), nextTier);
    ...
    /**
     * Adds the annotations of the source tier to the corresponding tier in destination transcription. This assumes that, if
     * necessary, the destination tier(s) etc have been set up beforehand.
     *
     * <p>Note: the private methods assume that both transcription objects are not null.
     * HS April 2015 at the moment this method is only called for top level tiers, but there are still some remaining checks
     * on root/parent tier
     *
    ...
    private void addAnnotations(TierImpl sourceTier, List<String> tierToInclude) {
        ...
        TierImpl destTier = transA.getTierWithId(sourceTier.getName());
        if (destTier == null) {
    ...
    /**
     * Adds the annotations of the source dependent tier to the corresponding dependent tier in the destination
     * transcription. This assumes that, if necessary, the destination tier(s) etc have been set up beforehand.
     *
     * <p>Note: the private methods assume that both transcription objects are not null.
     *
     * @param sourceTier a tier from the second transcription
    ...
    private void addAnnotationsToDependentTier(TierImpl sourceTier, List<String> tierToInclude) {
        ...
        if (destTier.getLinguisticType().getConstraints().getStereoType() != Constraint.SYMBOLIC_ASSOCIATION) {
            ...
            for (AbstractAnnotation parAnn : parAnnotations) {
                ...
                //                modifiedAnnotations.put(rootAnnotation, AnnotationRecreator.createTreeForAnnotation(
                //                rootAnnotation));
                //            }
            ...
            for (AbstractAnnotation srcAnn : sourceAnnotations) {
                long refTime = (srcAnn.getBeginTimeBoundary() + srcAnn.getEndTimeBoundary()) / 2;
                ...
                if (curDestAnn != null) {
                    ...
                    deletedAnnotations.put(curDestAnn,
                                           new DefaultMutableTreeNode(AnnotationRecreator.createTreeForAnnotation(curDestAnn)));

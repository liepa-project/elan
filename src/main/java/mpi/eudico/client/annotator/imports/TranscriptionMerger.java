...
/**
 * A class that merges two source transcriptions to a new destination transcription, in a separate thread. ProgressListeners
 * can register to be informed about the merging progress.
 *
 * @author Han Sloetjes
 */
public class TranscriptionMerger {
    ...
    private final File source2File;
     ...
     * @throws IllegalArgumentException thrown if any of the source files does not exist
     */
    public TranscriptionMerger(String source1, String source2, String destination) throws
                                                                                   IOException {
        ...
        if (!destinationFile.exists() && !destinationFile.createNewFile()) {
            LOG.warning("Attempt to create file failed : %s".formatted(destination));
        }

        if (!destinationFile.canWrite() || destinationFile.isDirectory()) {
            LOG.warning("Cannot write to file: " + destinationFile.getAbsolutePath());
            throw new IOException("Cannot write to file: " + destinationFile.getAbsolutePath());
        }
    }

    /**
     * Creates a new TranscriptionMerger instance.<br> Note: Merging with an existing transcription is not implemented yet.
     *
     ...
     * @throws IllegalArgumentException thrown if any of the source files does not exist
     */
    public TranscriptionMerger(Transcription transcription, String source2, String destination) throws
                                                                                                IOException {
        ...
        if (!destinationFile.exists() && !destinationFile.createNewFile()) {
            LOG.warning("Attempt to create new file failed : %s".formatted(destination));
        }
    ...
    /**
     * Starts the merging process. This creates a separate thread that directs the  merging steps.
     */
    ...
    /**
     * Creates the first source transcrition.  Note: only the construction of a transcription from a source file is
     * implemented. Duplication or cloning of an existing, in-memory, transcription is not yet supported.
     */
    ...
    /**
     * The actual merging process. Checks what tiers, types and CV's should be added and then copies the annotations to the
     * destination transcription.
     */
    private void mergeTranscriptions() {
        progressUpdate(2 * LOAD_PERCENTAGE, "Adding Tiers, LinguisticTypes and ControlledVocabularies...");
        ...
        try {
            ...
            while (topTierEnum.hasMoreElements()) {
                ...
                if (o instanceof TierImpl tier) {
                    progressUpdate(tierStartProgress, "Merging tier: " + tier.getName());
                    ...
                    if (numAnn > 0) {
                        ...
                        for (int i = 0; i < numAnn; i++) {
                            ...
                            AnnotationRecreator.createAnnotationFromTree(destTranscription, recordNode);
                    ...
                    progressUpdate(tierStartProgress + (tierNum * progressPerIndepTier),
            ...
            transcriptionStore.storeTranscription(destTranscription,
                                                  null,
                                                  new ArrayList<TierImpl>(0),
                                                  destinationFile.getAbsolutePath(),
                                                  saveAsType);

            LOG.info("Transcription saved to: " + destinationFile.getAbsolutePath());
    ...
    /**
     * Adds the tiers in the specified List to the destination transcription, after performing some checks.  If Linguistic
     * types and/or CV's should be copied these are copied first.
     *
     * @param tiersToAdd a list of tiers to add to the destination
     */
    private void addTiersTypesAndCVs(List<TierImpl> tiersToAdd) {
        if ((tiersToAdd == null) || (destTranscription == null) || (transcription2 == null)) {
        ...
        for (TierImpl t : tiersToAdd) {
            ...
            if (parentTierName == null) {
                newTier = new TierImpl(t.getName(), t.getParticipant(), destTranscription, null);
            } else {
                t2 = destTranscription.getTierWithId(parentTierName);

                if (t2 != null) {
                    newTier = new TierImpl(t2, t.getName(), t.getParticipant(), destTranscription, null);
                } else {
    ...
    /**
     * A thread that directs the main steps in the merging process. Merging is done in a separate thread in order to be able
     * to update ui elements that visualize the progress of the process.
     *
     * @author HS

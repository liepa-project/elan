...
/**
 * A class to extract annotations from a {@code Praat .TextGrid} file. Only {@code IntervalTier}s and {@code TextTier}s are
 * supported. The expected format is roughly like below, but the format is only loosely checked.
 * {@code 1 File type = "ooTextFile" 2 Object class = "TextGrid" 3 4 xmin = 0 5 xmax = 36.59755102040816 6 tiers?
 * &lt;exists&gt; 7 size = 2 8 item []: 9     item [1]: 10         class = "IntervalTier" 11         name = "One" 12
 * xmin = 0 13         xmax = 36.59755102040816 14         intervals: size = 5 15         intervals [1]: 16             xmin
 * = 0 17             xmax = 1 18             text = "" }
 *
 * @version Feb 2013 the short notation format (roughly the same lines without the keys and the indentation) is now also
 *     supported
 */
public class PraatTextGrid {
     ...
     */
    public PraatTextGrid(String fileName) throws
                                          IOException {
     ...
     * @throws IOException if the file can not be read, for whatever reason
     */
    public PraatTextGrid(String fileName, boolean includeTextTiers, int pointDuration) throws
                                                                                       IOException {
        if (fileName != null) {
     ...
     * @throws IOException if the file can not be read, for whatever reason
     */
    public PraatTextGrid(File gridFile) throws
                                        IOException {
     ...
     * @throws IOException if the file can not be read, for whatever reason
     */
    public PraatTextGrid(File gridFile, boolean includeTextTiers, int pointDuration) throws
                                                                                     IOException {
     ...
     * @throws IOException if the file can not be read, for whatever reason
     */
    public PraatTextGrid(File gridFile, boolean includeTextTiers, int pointDuration, String encoding) throws
                                                                                                      IOException {
     ...
     */
    private boolean isShortNotation(BufferedReader reader) throws
                                                           IOException {
        if (reader == null) {
            return false;
        }
                ...
                if (inTextTier) {
                    ...
                    if (eqPos > 0) {
                        ...
                        if (!inPoints && (line.indexOf(nm) >= 0) && (line.indexOf(nm) < eqPos)) {
                            ...
                            // finish and add the annotation annotationDataRecord
                            inPoints = false;
                            //System.out.println("T: " + annValue);
                            annotationDataRecord =
                                new AnnotationDataRecord(tierName, annValue, begin, begin + pointDuration);
                            records.add(annotationDataRecord);
                            // reset
                            annValue = "";
                            begin = -1;
                        }
                    ...
                    if (eqPos > 0) {
                        ...
                        if (!inInterval && (line.indexOf(nm) >= 0) && (line.indexOf(nm) < eqPos)) {
                            ...
                            inInterval = false;
                            //System.out.println("T: " + annValue);
                            annotationDataRecord = new AnnotationDataRecord(tierName, annValue, begin, end);
                            records.add(annotationDataRecord);
                            // reset
                            annValue = "";
                            begin = -1;
    ...
    /**
     * Parses the short notation of a Praat TextGrid file. Handling completely separated from the long notation.
     *
     * @param reader the (configured) reader
     */
    private void parseShortNotation(BufferedReader reader) throws
                                                           IOException {
        if (reader == null) {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (linePos == SN_POSITION.NEXT_IS_MARK) {
                if (includeTextTiers) {
                    ...
                    // finish and add the annotation annotationDataRecord
                    annotationDataRecord = new AnnotationDataRecord(tierName, annValue, begin, begin + pointDuration);
                    records.add(annotationDataRecord);
                    // reset
                    annValue = "";
                    begin = -1;
    ...
    /**
     * Extracts the text value and, if needed, converts Praat's special character sequences into Unicode chars.
     *
     * @param value the text value
     * @param eqPos the index of the equals sign
     *
     * @return the annotation value. If necessary Praat's special symbols have been converted to Unicode.
     */
    ...
    /**
     * Extracts a double time value, multiplies by 1000 (sec to ms) and converts to long.
     *
    ...
    /**
     * Removes a beginning and end quote mark from the specified string. Does no null check nor are spaces trimmed.
     *
     * @param value the value of which leading and trailing quote chars should be removed
     *
    ...
    private String removeQuotes(String value) {
        boolean removeOuterQuotes = !value.startsWith(escapedOuterQuote) || !value.endsWith(escapedOuterQuote);

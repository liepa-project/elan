...
/**
 * Utility class for tiers and type functionalities
 */
public class MergeUtil {
    ...
    /**
     * Checks whether the tiers can be added: this depends on tier dependencies and compatibility of linguistic types.
     *
    ...
    /**
     * Check whether the LinguisticTypes of the tiers have the same stereotype. This is a loose check, other attributes could
     * also be checked; name, cv etc.
     *
    ...
    public boolean lingTypeCompatible(LinguisticType lType1, LinguisticType lType2) {
        ...
        return constraintsCompatible(lType1.getConstraints(), lType2.getConstraints());
    }

    /**
     * Check if two Constraint objects are compatible, which in this case means either both null, or both having the same
     * stereotype.
     *
     * @param c1 the first constraint
     * @param c2 the second constraint
     *
     * @return {@code true} if both constraints are {@code null} or if both constraints have the same stereotype
     */
    ...
    /**
     * Checks whether the second type can be used for a tier that is a direct child of a tier based on the first type.
     *
     * @param parentType the type for the parent tier
     * @param childType the type for the child tier
     *
     * @return true if the the second type can be used for a tier that is a child of a tier of the first type, false
     *     otherwise
     */
        ...
        switch (parentCon.getStereoType()) {
            ...
            case Constraint.INCLUDED_IN:
                switch (childCon.getStereoType()) {
                    // all currently used types are compatible, the first two cases mean that if parentType
                    // .isTimeAlignable() could return true
                    case Constraint.INCLUDED_IN:
                    case Constraint.TIME_SUBDIVISION:
                    case Constraint.SYMBOLIC_SUBDIVISION:
                    case Constraint.SYMBOLIC_ASSOCIATION:
    ...
    public <T extends Tier> List<T> sortTiers(List<T> tiersToSort) {
        ...
        for (int i = 0; i < tiersToSort.size(); i++) {
            ...
            if ((t.getParentTier() == null) || !tiersToSort.contains(t.getParentTier())) {
                ...
                nodes.get(t.getParentTier()).add(nodes.get(t));
            }
        }
        ...
        while (en.hasMoreElements()) {
            ...
            if (node.getUserObject() instanceof Tier) {
                sorted.add((T) node.getUserObject());
            }
        }
        return sorted;
    }
    ...
    /**
     * Sorts the list of tier names based on the hierarchy the tiers have in the transcription. First the actual tiers are
     * retrieved from the transcription, that lists is sorted and then a list of tier names is returned.
     *
    ...
    /**
     * Returns all tiers from the specified list of tiers that don't have their parent tier (or ancestor) in the same list.
     *
    ...
    /**
     * Adds the tiers that are not yet in the destination transcription, after performing some checks.  If Linguistic types
     * and/or CV's should be copied/added these are copied/added first. It is assumed that it is save to add LT's and CV's to
     * the destination Transcription without cloning.
     *
     * <p>NOTE: this method assumes that the source transcription is loaded for the
     * purpose of this merging. Some objects that are added to the destination transcription are not cloned, e.g. tier type
     * objects of the source transcription are added to the destination transcription (and should therefore not be edited or
     * saved anymore in source.
     *
     * @param srcTrans the first transcription
     * @param destTrans the second transcription
        ...
        for (int i = 0; i < tiersToAdd.size(); i++) {
            ...
            if (newTier != null) {
                ...
                if (lt2 != null) {
                    ...
                    LOG.warning("Could not add tier: "
                                + newTier.getName()
    ...
    /**
     * Checks which of the selected tiers is already present in the destination transcription and renames those tiers (and
     * dependent tiers) in the source transcription. The renaming is implemented as adding a number/renumbering.
     *
     * <p>Important note: the assumption is that the source transcription can be safely changed,
     * without undo etc.
     *
     ...
     *
     * @return a map containing mappings from original name to modified name or to the same name if no renaming is required
     */
                        ...
                        if (destTrans.getTierWithId(nextTName) == null && !nameMap.containsValue(nextTName)) {
                            ...
                            for (TierImpl ti : depTiers) {
                                if (selTiers.contains(ti.getName())) {
                                    ...
                                    if (destTrans.getTierWithId(nextDepTierName) == null && !nameMap.containsValue(
                                        nextDepTierName)) {
    ...
    /**
     * Renames the tiers in the specified transcription based on the provided old name - new name mapping.
     *
    ...
    /**
     * Update a target controlled vocabulary in a target transcription based on the source controlled vocabulary which is
     * assumed to have the same ID (= name). New languages are added, descriptions are updated, new entries are added (this
     * concerns internal CV's), internal CV's are converted to external CV's and vice versa, url's of ECV's are updated.
     * Entries that are in the target CV but not in the source are left untouched (unless the forceReplace flag is true).
     *
    ...
    public void updateControlledVocabulary(TranscriptionImpl srcTrans, TranscriptionImpl targetTrans, String cvName,
        ...
        if (srcCV instanceof ExternalCV srcECV) {

            if (targetCV instanceof ExternalCV targetECV) {
                if (srcECV.getExternalRef() != null) {
                    if (targetECV.getExternalRef() != null) {
                        if (srcECV.getExternalRef().getReferenceType() == targetECV.getExternalRef().getReferenceType()
                            && srcECV.getExternalRef().getValue() != null
                            ...
                            targetTrans.setChanged(); // the ExternalReference object does not notify listeners of the
                            // change
                            if (report != null) {
                                report.append(String.format("Updated external URL of target CV \"%s\" to: \"%s\"",
                                                            targetCV.getName(),
                ...
                ExternalCV convertCV =
                    new ExternalCV(srcECV); // copies external reference as well //or new ExternalCV(targetCV);
            ...
            if (numSrcLanguages == numTargetLanguages) {
                ...
                for (int i = 0; i < numSrcLanguages; i++) {
                    ...
                    if (index2 < 0) {
                        ...
                        if (srcCV.getIndexOfLanguage(curLid) < 0) {
                            ...
                            if (report != null) {
                                report.append(String.format(
                                    "Changed Language of target CV: \"%s\", at index %d from \"%s\" to \"%s\"",
                                    targetCV.getName(),
                                    i,
                                    curLid,
                                    lid));
                            ...
                            // check description
                            if (srcCV.getDescription(i) != null && !srcCV.getDescription(i)
                                                                         .equals(targetCV.getDescription(i))) {
                                targetCV.setDescription(i, srcCV.getDescription(i));
                                if (report != null) {
                                    report.append(String.format(
                                        "Updated Description of target CV: \"%s\" at index %d to \"%s\"",
                                        targetCV.getName(),
                                        i,
                                        targetCV.getDescription(i)));
                                }
                            }
                        } else {
                            // the language currently at index i of the target is also there in the source but at
                            // different index
                        }
                    } else if (i == index2) {
                        // check description
                        if (srcCV.getDescription(i) != null && !srcCV.getDescription(i).equals(targetCV.getDescription(i))) {
                            ...
                            if (report != null) {
                                report.append(String.format("Updated Description of target CV: \"%s\" at index %d to \"%s\"",
                                                            targetCV.getName(),
                                                            i,
                                                            targetCV.getDescription(index2)));
                            }
                        }
                    } else {
                        ...
                        // check description
                        if (srcCV.getDescription(i) != null && !srcCV.getDescription(i)
                                                                     .equals(targetCV.getDescription(index2))) {
                ...
                for (int i = 0; i < numSrcLanguages; i++) {
                    ...
                    if (index2 < 0) {
                        ...
                        if (ni < 0) {
                            if (report != null) {
                                report.append(String.format("Failed to add Language: \"%s\" to target CV: \"%s\"",
                                                            lid,
                                                            targetCV.getName()));
                            }
                            continue;
                            ...
                            index2 = ni;
                            if (report != null) {
                                report.append(String.format("Added Language: \"%s\" to target CV: \"%s\"",
                                                            lid,
                                                            targetCV.getName()));
                            }
                        }
                    ...
                    if (index2 >= 0) {
                        if (srcCV.getDescription(i) != null && !srcCV.getDescription(i)
                            ...
                            targetCV.setDescription(index2, srcCV.getDescription(i));
                            if (report != null) {
                                report.append(String.format("Updated Description of target CV: \"%s\"  at index %d to "
                                                            + "\"%s\"",
                                                            targetCV.getName(),
                                                            index2,
                                                            targetCV.getDescription(index2)));
                        ...
                        for (int i = 0; i < numSrcLanguages; i++) {
                            ...
                            if (index2 >= 0) {
                                ...
                                if (!sv.equals(tv)) {
                                    ...
                                    // entry external ref
                                    if (entry.getExternalRef() != null && !entry.getExternalRef()
                                                                                .equals(tarEntry.getExternalRef())) {
                                        try {
    ...
    /**
     * Updates or adds a tier type in the target transcription based on the attributes and properties of the same type (with
     * the same name or id) in the source transcription. The Stereotype of a tier type will not be updated because that could
     * lead to inconsistencies and data loss. The association with a CV, a lexicon and lexical entry field and with a data
     * category are updated (including setting to null if tha is the case in the source).
     *
    ...
    public void updateTierType(TranscriptionImpl srcTrans, TranscriptionImpl targetTrans, String typeName,
        ...
        if (targetType != null) {
            ...
            if (srcStereo != tarStereo) {
                LOG.info(String.format("Incompatible stereotypes of Tier Type: \"%s\", cannot update the stereotype",
                                       typeName));
                if (report != null) {
                    report.append(String.format("Incompatible stereotypes of Tier Type: \"%s\", cannot update the "
                                                + "stereotype",
                                                typeName));
                }
            }
            ...
            if (srcType.isUsingControlledVocabulary()) {
                if (!srcType.getControlledVocabularyName().equals(targetType.getControlledVocabularyName())) {
                    if (targetTrans.getControlledVocabulary(srcType.getControlledVocabularyName()) != null) {
                        ...
                        changed = true;
                        if (report != null) {
                            report.append(String.format("Changed the CV of Type \"%s\" to: \"%s\"",
                                                        typeName,
                                                        srcType.getControlledVocabularyName()));
                        }
                    } else {
                        if (report != null) {
                            report.append(String.format(
                                "Cannot change the CV of Type \"%s\" to: \"%s\"; the CV does not exist in target",
                                typeName,
                                srcType.getControlledVocabularyName()));
                        }
                    }
            ...
            if (srcType.getDataCategory() != null) {
                if (!srcType.getDataCategory().equals(targetType.getDataCategory())) {
                    ...
                    changed = true;
                    if (report != null) {
                        report.append(String.format("Changed the Data Category of Type \"%s\" to: \"%s\"",
                                                    typeName,
                                                    srcType.getDataCategory()));
                    }
                }
            } else {
                if (targetType.getDataCategory() != null) {
                    ...
                    changed = true;
                    if (report != null) {
                        report.append(String.format("Removed the Data Category of Type \"%s\" (was: \"%s\")",
                                                    typeName,
                                                    oldDC));
                    }
                }
            ...
            if (srcType.getLexiconQueryBundle() != null) {
                ...
                if (targetType.getLexiconQueryBundle() == null) {
                    ...
                    changed = true;
                    if (report != null) {
                        report.append(String.format("Added a Lexicon Link to Type \"%s\": \"%s\"",
                                                    typeName,
                                                    srcBundle.getLinkName()));
                    }
                } else {
                    ...
                    if (srcLefi != null) {
                        if (tarLefi != null) {
                            ...
                            if ((srcLefi.getDescription() == null && tarLefi.getDescription() != null)
                                || (srcLefi.getDescription() != null && !srcLefi.getDescription()
                                                                                .equals(tarLefi.getDescription()))) {
                        ...
                        if (tarLL != null) {
                            ...
                            if (srcLI != null && tarLI != null) {
                                ...
                                if ((srcLI.getDescription() == null && tarLI.getDescription() != null)
                                    || (srcLI.getDescription() != null && !srcLI.getDescription()
                                                                                .equals(tarLI.getDescription()))) {
                    ...
                    if (copySrcLEFI || copySrcLL) {
                        ...
                        if (copySrcLL) {
                            if (srcLL != null) {
                                nextLL = new LexiconLink(srcLL.getName(),
                                                         srcLL.getLexSrvcClntType(),
                                                         srcLL.getUrl(),
                                                         null,
                                                         srcLL.getLexId() == null
                                                         ? null
                                                         : new LexiconIdentification(srcLL.getLexId()));
                            }
                        }
                ...
                if (targetType.getLexiconQueryBundle() != null) {
                    ...
                    changed = true;
                    if (report != null) {
                        report.append(String.format("Removed the Lexicon Link of Type \"%s\" (was: \"%s\")",
                                                    typeName,
                                                    oldName));
                    }
                }
    ...
    /**
     * Updates the target tier in the target transcription based on properties of the source tier. A few properties will not
     * be updated: the parent tier of the target tier (this could potentially result in deletion of annotations) and the tier
     * type in case the Stereotype of the source tier and target tier are different.
     *
    ...
    public void updateTier(TranscriptionImpl srcTrans, TranscriptionImpl targetTrans, String tierName,
        ...
        if ((srcParTier == null && tarParTier != null) || (srcParTier != null && (tarParTier == null || !srcParTier.getName()
                                                                                                                   .equals(
                                                                                                                       tarParTier.getName())))) {
            if (report != null) {
                report.append(String.format(
                    "Incompatible Tier hierarchy of source (parent = \"%s\") and target (parent = \"%s\"), cannot "
                    + "update",
        ...
        if (srcType.getLinguisticTypeName().equals(tarType.getLinguisticTypeName())) {
            ...
            if (nextType != null) {
                ...
                if (report != null) {
                    report.append(String.format("Changed the Type of Tier \"%s\" to \"%s\"",
                                                tierName,
                ...
                if (report != null) {
                    report.append(String.format(
                        "Cannot change the Type of Tier \"%s\" to \"%s\", the Tier Type is not in the target",
                        tierName,
                        srcType.getLinguisticTypeName()));
                }
            }
        } else {
            if (report != null) {
                report.append(String.format(
                    "Incompatible Type of source (\"%s\") and target (\"%s\") Tier, cannot change the Type",
                    ConstraintImpl.getStereoTypeName(srcType.getConstraints() == null
                                                     ? -1
                                                     : srcType.getConstraints().getStereoType()),
                    ConstraintImpl.getStereoTypeName(tarType.getConstraints() == null
                                                     ? -1
                                                     : tarType.getConstraints().getStereoType())));
            }
        }
        // check participant
        if (srcTier.getParticipant() != null && !srcTier.getParticipant().equals(targetTier.getParticipant())) {
            ...
            if (report != null) {
                report.append(String.format("Changed the Participant of Tier \"%s\" to \"%s\"",
                                            tierName,
                                            srcTier.getParticipant()));
            }
        }
        // annotator
        if (srcTier.getAnnotator() != null && !srcTier.getAnnotator().equals(targetTier.getAnnotator())) {
            ...
            if (report != null) {
                report.append(String.format("Changed the Annotator of Tier \"%s\" to \"%s\"",
                                            tierName,
                                            srcTier.getAnnotator()));
            }
        }
        ...
        if (srcTier.getLangRef() != null && !srcTier.getLangRef().equals(targetTier.getLangRef())) {
            ...
            if (report != null) {
                report.append(String.format("Changed the Content Language of Tier \"%s\" to \"%s\"",
                                            tierName,
                                            srcTier.getLangRef()));
            }
        }
        ...
        if (srcTier.getDefaultLocale() != null && !srcTier.getDefaultLocale().equals(targetTier.getDefaultLocale())) {
            ...
            if (report != null) {
                report.append(String.format("Changed the Locale (Input Method) of Tier \"%s\" to \"%s\"",
                                            tierName,
                                            srcTier.getDefaultLocale().toString()));
            }
    ...
    /**
     * Adds new tiers to the target transcription. It is assumed that checks have been performed before to ensure that the
     * tiers in the list are not yet present in the target (but it is double checked here). It is also assumed that necessary
     * tier types have been added beforehand, but it is double checked here, including some checks on the compatibility of
     * tier types.
     *
     ...
     * @param report if not null, messages will be added to this report
     */
    public void updateWithNewTiers(TranscriptionImpl srcTrans, TranscriptionImpl targetTrans, List<String> newTiersNames,
                                   ProcessReport report) {
        if (srcTrans == null) {
            LOG.warning("The source Transcription is null");
            return;
        ...
        for (TierImpl t : tiersToAdd) {
            ...
            if (t.hasParentTier()) {
                ...
                if (tarParent == null) {
                    if (report != null) {
                        report.append(String.format(
                            "Cannot add Tier \"%s\" to target, the target Parent Tier \"%s\" is not there",
                            t.getName(),
                            t.getParentTier().getName()));
                    }
                    continue;
                }
    ...
    /**
     * Method to update the preferences of the target transcription with part of the preferences of the source
     * transcription.
     *
     * @param srcTrans the source transcription
     * @param targetTrans the target transcription
     * @param prefKeys the keys of preferences to copy (e.g. specific fonts, font sizes and colors)
     */
    ...
    /**
     * Method to update the preferences of the target transcription with part of the preferences present in the specified
     * map.
     *
     * @param targetTrans the target transcription

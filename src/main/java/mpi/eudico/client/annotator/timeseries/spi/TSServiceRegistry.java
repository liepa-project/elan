...
public final class TSServiceRegistry {
    ...
    private void registerPropertySpi() {
        ...
        try {
            ...
            prefProvider = (TSServiceProvider) c.getDeclaredConstructor().newInstance();
        } catch (ReflectiveOperationException reo) {
        	LOG.warning(reo.getMessage());
        ...
        if (prefProvider == null) {
        	...
        	try {
        		...
        		if (propImpl != null) {
        			try {
        			    prefProvider = (TSServiceProvider) propImpl.getDeclaredConstructor().newInstance();
        			} catch (ReflectiveOperationException reo) {
        	        	LOG.warning(reo.getMessage());
        	        } catch (SecurityException se) {
        	            LOG.warning(se.getMessage());
     ...
     * @deprecated replaced by {@link #registerClasspathSpis2()}
     */
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if ((line.length() != 0) && (line.charAt(0) != '#')) {
                        ...
                        if (TSServiceProvider.class.isAssignableFrom(cl)) {
                            providers.add((TSServiceProvider) cl.getDeclaredConstructor().newInstance());
                        }

...
public class ManageTierSetDlg extends ClosableDialog implements ActionListener, ListSelectionListener{	
	...
	private void initTierSetButtonPanel(){
		...
		try {
			ImageIcon upIcon = new ImageIcon(this.getClass().getResource(
					Constants.ICON_LOCATION + "Up.gif"));
			ImageIcon downIcon = new ImageIcon(this.getClass().getResource(
					Constants.ICON_LOCATION + "Down.gif"));
		...
		try {
			...
			ImageIcon removeIcon = new ImageIcon(this.getClass().getResource(
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(source == addButton){
			...
			for(int i=0; i < tierSetTable.getRowCount(); i++){
				...
				if(tierMap != null){					
					for(Map.Entry<String, Boolean> tierEntry : tierMap.entrySet()){
						tierSet.setTierVisiblity(tierEntry.getKey(), tierEntry.getValue());
					}
				}
			}

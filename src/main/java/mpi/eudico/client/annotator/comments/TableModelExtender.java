...
/**
 * A table model that adds an extra column to an existing table model. It is assumed that the number of columns in the
 * delegate is fixed. The elements in the new column are not editable.
 *
 * <p>For reasons of practicality, it includes the extra methods from
 * CommentTableModelInterface, but that can be split off to make it more general if needed.
 *
 * @param <T> the type of values in the model
 *
 * @author olasei
 */
public class TableModelExtender<T> implements TableModel,
                                              CommentTableModel {

    private final CommentTableModel delegate;
    private final String columnName;
    private final Class<T> clazz;
    private List<T> column;
    private final int delegateColumns;
     ...
     *
     * <p>Setting this does not fire any events, so that replacing the table
     * data can be done with this column first, and then the main table, which will call fireTableDataChanged().
     *
    ...
    public void setValueAt(Object value, int rowIndex, int columnIndex) {
        if (columnIndex < delegateColumns) {
            delegate.setValueAt(value, rowIndex, columnIndex);
        }
    }

 ...
 *
 * <p>Extracted from GridViewer on Jun 29, 2004
 *
...
public class AnnotationTable extends JTable {
    private static final int MIN_COLUMN_WIDTH = 15; // swing default
     ...
     */
    private final Map<Object, Font> prefTierFonts;

    /**
     * stores default minimal widths of columns
     */
    private static final Map<String, Integer> preferredWidths = new HashMap<String, Integer>();

    /**
     * stores default maximal widths of columns
     */
    private static final Map<String, Integer> maxWidths = new HashMap<String, Integer>();
    ...
    */
    ...
    /**
     * Restores the default row height after an inline edit operation. If this method is called by the GridEditor, the editor
     * may be called to commit instead of cancel, depending on a preference flag.
     *
    ...
    /**
     * Distributes remaining table space among annotation columns. To be called after other columns have been set/removed or
     * component resized
     */
        ...
        if (getParent() != null) {
            ...
            if (remainingSpace > 0) {
                for (Iterator<TableColumn> iter = visibleAnnotationColumns.iterator(); iter.hasNext(); ) {
                    iter.next().setPreferredWidth(remainingSpace / visibleAnnotationColumns.size());
                }
            }
        }
    }
    ...
    public boolean isColumnVisible(String columnName) {
        ...
        return column != null && column.getWidth() > 0;
    }

    /**
     * Returns default preferred width for columns. If there is no preferred width for that column, the minimum width is
     * returned.
     *
    ...
    private int getDefaultPreferredWidth(String columnName) {
        return preferredWidths.containsKey(columnName) ? preferredWidths.get(columnName).intValue() : MIN_COLUMN_WIDTH;
        //swing default
    }

    /**
     * Returns the default max width for columns. If there no maximum width has been recorded for a column,
    ...
    private int getDefaultMaxWidth(String columnName) {
        return maxWidths.containsKey(columnName) ? maxWidths.get(columnName).intValue() : Integer.MAX_VALUE;
        //swing default
    }
    ...
    public void updateLocale() {
        for (int i = 0; i < dataModel.getColumnCount(); i++) {
            ...
            getColumnModel().getColumn(index)
                            .setHeaderValue(ElanLocale.getString("Frame.GridFrame." + dataModel.getColumnName(i)));
        }
    }
    ...
    /**
     * Sets the names of the fonts for tiers. Based on this map and the current font size Font objects are created.
     *
     * @param fontNames a map of tier name to font name<br> NB the table model identifier for the annotations of the main
     *     tier in the table is used for that tier instead of its name
     */
    ...
    /**
     * Returns the font for the specified column. Only applicable for annotation columns.
     *
    ...
    public Font getFontForColumn(int column) {
        if (dataModel instanceof GridViewerTableModel && prefTierFonts.size() > 0) {
            String name = dataModel.getColumnName(column);
     ...
     *
     * <p>Strictly speaking, this map isn't really needed. The color can be found
     * from the {@code Annotation->Tier->Linguistic Type->CV->Entry(by Id)->Color}. This can be cached as it is looked up, in
     * particular the {@code TierName->CV}.
     *
     * @param colorsForAnnotations the mapping, per tier an CVEId to color mapping
     */
    public void setColorsForAnnotations(Map<String, Map<String, Color>> colorsForAnnotations) {
        annColorsMap = colorsForAnnotations;
    }
    ...
    /**
     * This table was constructed with a GridViewerTableModel, so we can safely return it.
     */
    @Override

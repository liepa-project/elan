...
import mpi.eudico.server.corpora.clomimpl.subtitletext.exporters.SubtitleFileExporter;
...
/**
 * A dialog for subtitle text export, in formats like .srt etc. A minimal duration for each subtitle can be specified.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
...
public class ExportSubtitleDialog extends AbstractExtTierExportDialog implements ChangeListener {
    ...
    private JCheckBox exportIndividualFilesCB;
    private JLabel exportIndividualFileNames;
     ...
     */
    public ExportSubtitleDialog(Frame parent, boolean modal, TranscriptionImpl transcription, Selection selection) {
    ...
    /**
     * Configures the tier table and adds option elements: minimal duration textfield.
     */
    @Override
    protected void makeLayout() {
        ...
        exportIndividualFilesCB = new JCheckBox();
        exportIndividualFilesCB.setSelected(false);
        gridBagConstraints = new GridBagConstraints();
        gridBagConstraints.gridx = 0;
        ...
        gridBagConstraints.anchor = GridBagConstraints.WEST;
        gridBagConstraints.insets = insets;
        optionsPanel.add(exportIndividualFilesCB, gridBagConstraints);

        exportIndividualFileNames = new JLabel();
        exportIndividualFileNames.setVisible(false);
        Font labelFont = exportIndividualFileNames.getFont().deriveFont(Font.PLAIN);
        exportIndividualFileNames.setFont(labelFont);
        gridBagConstraints = new GridBagConstraints();
        gridBagConstraints.gridx = 0;
        ...
        gridBagConstraints.fill = GridBagConstraints.NONE;
        gridBagConstraints.anchor = GridBagConstraints.WEST;
        gridBagConstraints.insets = new Insets(4, 32, 4, 6);
        optionsPanel.add(exportIndividualFileNames, gridBagConstraints);

        minimalDurCB.addChangeListener(this);
        restrictCheckBox.addChangeListener(this);
        overrideFrameRateCB.addChangeListener(this);
        exportIndividualFilesCB.addChangeListener(this);
    ...
    protected boolean startExport() throws
        ...
        File exportFile =
            promptForFile(ElanLocale.getString("ExportDialog.Subtitles.Title"), null, FileExtension.SUBTITLE_EXT, true);
        ...
        SubtitleEncoderInfo subtitleEncoderInfo = new SubtitleEncoderInfo(new SubtitleFile(exportFile, encoding),
                                                                          transcription,
                                                                          new InputFilter(selectedTiers,
                                                                                          b,
                                                                                          e,
                                                                                          minimalDur,
                                                                                          offset),
                                                                          new SubtitleTransformationOptions(frameRate,
                                                                                                            recalculateTimesCB.isSelected(),
                                                                                                            exportIndividualFilesCB.isSelected()));

        return SubtitleFileExporter.getExporter(exportFile).export(subtitleEncoderInfo);
    }
    ...
    protected void updateLocale() {
        ...
        minimalDurCB.setText(ElanLocale.getString("ExportDialog.Label.MinimalDur"));

        overrideFrameRateCB.setText(ElanLocale.getString("ExportDialog.OverrideTC"));
        palTimecodeRB.setText(ElanLocale.getString("ExportDialog.OverrideTC.PAL"));
        ntscDFTimecodeRB.setText(ElanLocale.getString("ExportDialog.OverrideTC.NTSCDF"));
        ntscNDFTimecodeRB.setText(ElanLocale.getString("ExportDialog.OverrideTC.NTSCNDF"));
        exportIndividualFilesCB.setText(ElanLocale.getString("ExportDialog.exportIndividualFiles"));
        exportIndividualFileNames.setText(ElanLocale.getString("ExportDialog.exportIndividualFileNamesExample"));
    }
    ...
    public void stateChanged(ChangeEvent e) {
        ...
        } else if (e.getSource() == exportIndividualFilesCB) {
            exportIndividualFileNames.setVisible(exportIndividualFilesCB.isSelected());
        }
    }
    ...
    private void setPreferredSetting() {
        ...
        boolPref = Preferences.getBool("ExportSubtitleDialog.exportIndividualFiles", null);
        if (boolPref != null) {
            exportIndividualFilesCB.setSelected(boolPref);
            exportIndividualFileNames.setVisible(boolPref);
        }
    }
    ...
    private void savePreferences() {
        ...
        Preferences.set("ExportSubtitleDialog.exportIndividualFiles", exportIndividualFilesCB.isSelected(), null);
        /*

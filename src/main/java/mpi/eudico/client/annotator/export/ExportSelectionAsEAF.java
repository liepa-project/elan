...
/**
 * Creates a new transcription document with the annotations that are found within  a selected time interval in the source
 * document. First all MediaDescriptors, Controlled Vocabularies, Linguistic Types and Tiers  are copied to a new
 * Transcription. Next all annotations that overlap the selected time  interval are copied. The copied annotations are forced
 * into the interval (i.e. if  the begintime is smaller than the interval begin time, the annotation's begin time  is updated
 * to the interval begin time. Same if the annotation's end time is greater than the interval end time. <br> Finally all
 * annotations are shifted in time by a value of -(selection begin time),  making the original selection begin time the time
 * origin (point 0) of the new  transcription.
 *
 * @author Han Sloetjes
...
public class ExportSelectionAsEAF {
    ...
    /**
     * Prompts for a file name, creates a progress monitor and spawns a new thread for the actual work.
     */
    private void startExport() {
        ...
        monitor = new ProgressMonitor(null, ElanLocale.getString("SaveDialog.Message.Title"), "", 0, 100);
    ...
    /**
     * Copies MediaDescriptors, Controlled Vocabularies, Linguistic Types, Tiers and  the relevant Annotations to the new
     * Transcription. The annotations are forced  into the selection interval. Finally the annotations are shifted with a
     * value of -begin and the transcription is written to file.  This method could be split into separate parts for copying
     * each group of  objects, when there is a need for this. Maybe in its own helper class.
     *
    ...
    private void copy(TranscriptionImpl sourceTrans, TranscriptionImpl copyTrans, long begin, long end, boolean clipMedia) {
            ...
            if (t1 != null) {
                ...
                if (lt2 != null) {
                    if (t1.hasParentTier()) {
                        ...
                        t2 = copyTrans.getTierWithId(parentName);
        ...
        for (int i = 0; i < root.getChildCount(); i++) {
            ...
            for (int j = 0; j < srcAnnos.size(); j++) {
                ...
                defaultMutableTreeNode = AnnotationRecreator.createTreeForAnnotation(ann);
                copyAnn =
                    (AlignableAnnotation) AnnotationRecreator.createAnnotationFromTree(copyTrans, defaultMutableTreeNode);
        ...
        try {
            store.storeTranscription(copyTrans, null, new ArrayList<TierImpl>(), path, saveAsType);
    ...
    private void copyPreferences(TranscriptionImpl sourceTrans, TranscriptionImpl copyTrans) {
        ...
        if (sourcePath != null && !sourcePath.equals(TranscriptionImpl.UNDEFINED_FILE_NAME)) {
            if (sourcePath.endsWith(FileExtension.EAF_EXT[0])) {
                String prefFile = sourcePath.substring(0, sourcePath.length() - FileExtension.EAF_EXT[0].length())
                                  + FileExtension.ELAN_XML_PREFS_EXT[0];
    ...
    private String promptForFileName() {
        ...
        chooser.createAndShowFileAndClipMediaDialog(ElanLocale.getString("SaveDialog.Title"),

...
/**
 * A (kind of) table model for a group of annotations, part of a hierarchy. Contains methods to add annotations with the same
 * begin and end time to the same row, as well as to repeat values of annotations that overlap multiple other annotation in
 * order to reduce the number of empty cells.<br>
 * <br>
 * Note: in each row the first cell contains an Annotation (or null), the second cell the begin time (as Long), the third the
 * end time (as Long).
 *
 * @author Han Sloetjes
 */
class MinimalTabExportTableModel {
    ...
    private final Map<String, String> cvEntryMap;

    /**
     * Constructor accepting default values for repeating spanning annotations and whether or not to completely remove
     * spanning rows.
     *
    ...
    /**
     * Constructor that allows to specify whether to repeat values of annotations spanning other annotations and whether to
     * "hide" spanning  lines rather than deleting them.
     *
     * @param columnNames the tier names
     * @param annotations the annotations
     * @param cvEntryMap a map containing controlled vocabulary entries
     * @param repeatValues if true the value of a spanning annotation is repeated
     * @param hideSpanningRowsInsteadOfRemove if hidden (true) the rows can be used for repeating values across multiple
     *     blocks
     * @param includeAnnotationIds if true, the id's of annotations are appended to the annotation values (mainly useful
     *     in the 'repeat' scenario's)
     */
    MinimalTabExportTableModel(List<String> columnNames, List<Annotation> annotations, Map<String, String> cvEntryMap,
        ...
        for (int i = 0; i < numCols; i++) {
            template.add(null); // this is a kind of filler object.
        }
    ...
    /**
     * Creates the data for the model, fills the rows, replicates values, sorts and hides or deletes spanning rows.
     */
    ...
    /**
     * A comparator for sorting rows, based on the time values in cell 1 and 2.
     *
     * @author Han Sloetjes
     * @version 1.0
     */
    private static class RowComparator implements Comparator<List<Object>>,
                                                  Serializable {
        @Serial
        private static final long serialVersionUID = 202310251031L;
         ...
         *
         * @return -1 if the first row is smaller (earlier), 0 if they have the same begin and end time, 1 if the second row
         *     is earlier. <br> *  Note: if the begin times are equal, the row with the greatest end time will be "smaller"
         */
        @Override
        public int compare(List<Object> row1, List<Object> row2) {
            long l1 = (Long) row1.get(1);
            long l2 = (Long) row1.get(2);
            long l3 = (Long) row2.get(1);
            long l4 = (Long) row2.get(2);

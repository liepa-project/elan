 ...
 *
 * <p>Set toolbox options for the files
...
public class MultipleFileToolBoxExportStep2 extends StepPane implements ItemListener,
                                                                        ActionListener {
    ...
    private void checkInputFields() {
        ...
        if (wrapBlocksCB.isSelected()) {
            if (numCharTF.getText() == null || numCharTF.getText().trim().length() <= 0) {
        ...
        if (typeRB.isSelected()) {
            if (typField.getText() == null || typField.getText().trim().length() <= 0) {
                databaseErrorLabel.setText("- " + ElanLocale.getString("ImportDialog.Message.SpecifyType"));
                moveForward = false;
            }
        } else if (specRB.isSelected()) {
            if (dbTypField.getText() == null || dbTypField.getText().trim().length() <= 0) {
        ...
        if (customRMRB.isSelected()) {
            if (markerTF.getText() == null || markerTF.getText().trim().length() <= 0) {
        ...
        if (mediaMarkerCB.isSelected()) {
            if (mediaMarkerNameTF.getText() == null || mediaMarkerNameTF.getText().trim().length() <= 0) {
                if (recordMarkerErrorLabel.getText().trim().length() > 0) {
                    recordMarkerErrorLabel.setText(recordMarkerErrorLabel.getText() + " " + ElanLocale.getString(
                        "ExportShoebox.Message.NoMediaMarker"));
                } else {
    ...
    public boolean leaveStepForward() {
        ...
        if (wrapBlocksCB.isSelected()) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this,
                                              ElanLocale.getString("ExportShoebox.Message.InvalidNumber"),
        ...
        if (typeRB.isSelected()) {
            ...
            if (!tf.exists()) {
                JOptionPane.showMessageDialog(this,
                                              ElanLocale.getString("ImportDialog.Message.NoType"),
    ...
    private void updateDetectedRecordMarker() {
        ...
        if (recordMarkerList.size() == 1) {
            detectedRMRB.setText(detectedRMRB.getText() + " (\\" + recordMarkerList.get(0) + ")");
    ...
    private void initOutputOptionsPanel() {
        ...
        outputOptionsPanel.setBorder(new TitledBorder(ElanLocale.getString("ExportDialog.Label.Options")));

        //create all components
        wrapBlocksCB = new JCheckBox(ElanLocale.getString("ExportShoebox.Label.WrapBlocks"));
        wrapBlocksCB.setSelected(true);
        wrapBlocksCB.addItemListener(this);

        charPerLineLabel = new JLabel(ElanLocale.getString("ExportShoebox.Label.NumberChars"));
        ...
        ssMSFormatRB = new JRadioButton(ElanLocale.getString("InterlinearizerOptionsDlg.TimeCodeFormat.Seconds"));
        ssMSFormatRB.setSelected(true);

        hhMMSSMSFormatRB = new JRadioButton(ElanLocale.getString("InterlinearizerOptionsDlg.TimeCodeFormat.TimeCode"));

        correctTimesCB = new JCheckBox(ElanLocale.getString("ExportDialog.CorrectTimes"));

        wrapLinesCB = new JCheckBox(ElanLocale.getString("ExportShoebox.Label.WrapLines"));
        wrapLinesCB.setSelected(true);
        wrapLinesCB.addItemListener(this);

        wrapNextLineRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.WrapNextLine"));
        wrapNextLineRB.setSelected(true);

        wrapAfterBlockRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.WrapEndOfBlock"));

        includeEmptyLinesCB = new JCheckBox(ElanLocale.getString("ExportShoebox.Label.IncludeEmpty"));
    ...
    private void initToolboxOptionsPanel() {
        ...
        toolboxOptionsPanel.setBorder(new TitledBorder(ElanLocale.getString("ExportShoebox.Label.ToolboxOptions")));
        ...
        specRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.SpecifyType"));
        ...
        detectedRMRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.Detected"));
        detectedRMRB.setSelected(true);


        defaultRMRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.DefaultMarker") + " (\\block)");


        customRMRB = new JRadioButton(ElanLocale.getString("ExportShoebox.Label.CustomMarker"));
    ...
    /**
     * sets the value in the numCharTF
     */
    ...
    private void loadPreferences() {
        ...
        customRMRB.setSelected(!defaultRMRB.isSelected() && !detectedRMRB.isSelected());

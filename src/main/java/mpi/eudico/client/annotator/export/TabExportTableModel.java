...
/**
 * A table model for tab delimited text export, where there is a separate column  per tier and where annotations with the
 * same begin and end time share the same  row, regardless of whether or not there is any relation between the tiers.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
public class TabExportTableModel implements TableModel {
    ...
    /**
     * Creates and fills the table model, annotations with the same begin and end time  are added to the same row.  NB no
     * null checking is performed
     *
     * @param annotations the annotations
     * @param cvMap a controlled vocabulary map
     * @param tierNames the tier names, for the columns
     */
    public TabExportTableModel(List<Annotation> annotations, Map<String, Map<String, String>> cvMap,
    ...
    /**
     * Adds the annotations to the proper table cell, based on tier name and begin and end time. It is assumed that the
     * annotations are sorted in ascending order, so that an annotations  is either added to a new row, or to the row of the
     * previous annotation.
     *
    ...
    private void fillRows(List<Annotation> annotations, Map<String, Map<String, String>> cvMap) {
        ...
        for (int i = 0; i < annotations.size(); i++) {
            ...
            if (cvMap != null) {
                cvName = ann.getTier().getLinguisticType().getControlledVocabularyName();
                if (cvName != null) {
    ...
    public String getColumnName(int columnIndex) {
        if ((columnIndex < 0) || (columnIndex >= columnIds.size())) {
            throw new ArrayIndexOutOfBoundsException("Index " + columnIndex + " < 0 or > " + (columnIds.size() - 1));
        }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        if ((rowIndex < 0) || (rowIndex >= rows.size())) {
            throw new ArrayIndexOutOfBoundsException("Index " + rowIndex + " < 0 or > " + (rows.size() - 1));
        }

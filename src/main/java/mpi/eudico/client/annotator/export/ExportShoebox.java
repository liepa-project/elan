...
public class ExportShoebox extends AbstractExtTierExportDialog implements ItemListener,
    ...
    public List<TierImpl> getTierTree(TierImpl tier) {
        List<TierImpl> tierTree = new ArrayList<>();
        List<List<TierImpl>> tierTrees = new ArrayList<>();

        List<TierImpl> children = tier.getChildTiers();

        tierTree.add(tier);

        for (TierImpl child : children) {
            tierTrees.add(getTierTree(child));
        }

        tierTrees.sort(new ListComparator());

        for (List<TierImpl> tree : tierTrees) {
            tierTree.addAll(tree);
        }
    ...
    protected void makeLayout() {
        ...
        ((GridBagLayout) getContentPane().getLayout()).setConstraints(buttonPanel, gridBagConstraints);
    ...
    protected boolean startExport() {
        ...
        File exportFile =
            promptForFile(ElanLocale.getString("ExportShoeboxDialog.Title"), null, FileExtension.SHOEBOX_TEXT_EXT, false);
    ...
    protected void updateLocale() {
        ...
        markerPanel.setBorder(new TitledBorder(ElanLocale.getString("ExportShoebox.Label.Markers")));
        wrapLinesCB.setText(ElanLocale.getString("ExportShoebox.Label.WrapBlocks"));
        ...
        allUnicodeCB.setText(ElanLocale.getString("ExportShoebox.CheckBox.AllUnicode"));
        fieldSpecButton.setText(ElanLocale.getString("ExportShoebox.Button.FieldSpec"));
        dbTypeLabel.setText(ElanLocale.getString("ExportShoebox.Label.SpecifyType"));
        tierNamesLabel.setText(ElanLocale.getString("ExportShoebox.Label.UseTierNames"));
        generateMarkersCB.setText(ElanLocale.getString("ExportShoebox.CheckBox.AutoGenerateMarkers"));
    }
        ...
        if (typeRB.isSelected()) {
            ...
            if (!tf.exists()) {
                ...
                try {
                    ...
                    LOG.log(Level.WARNING, "Some thing wrong with file reading", e);
                }
            }
        } else {
    ...
    static class ListComparator implements Comparator<List<?>>,
                                           Serializable {
        @Serial
        private static final long serialVersionUID = 202310251026L;

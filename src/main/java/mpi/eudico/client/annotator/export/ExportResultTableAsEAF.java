...
import mpi.eudico.server.corpora.clom.*;
...
/**
 * Export the annotations in the search result table with their context (the annotation block the annotation is part of) to a
 * new eaf file.
 */
public class ExportResultTableAsEAF {
    ...
    /**
     * Prompts for a file name, extracts the tiers (and thus types and cv's) that need to be copied and copies the toplevel
     * parent annotations of all annotations in the annotation table.
     *
    ...
    private void copyAnnotations(Transcription transcription, Transcription nextTrans, ArrayList<Annotation> topAnnos) {
        ...
        for (int i = 0; i < topAnnos.size(); i++) {
            ...
            defaultMutableTreeNode = AnnotationRecreator.createTreeForAnnotation(aa);
            copyAnn = (AlignableAnnotation) AnnotationRecreator.createAnnotationFromTree(nextTrans, defaultMutableTreeNode);
            if (copyAnn == null) {
                LOG.warning("Could not copy annotation: "
                            + aa.getValue()
                            + " ("
    ...
    private AlignableAnnotation rootAnnotationOf(Annotation ann) {
        ...
        if (tier.hasParentTier()) {
            ...
            return (AlignableAnnotation) tier.getAnnotationAtTime((ann.getBeginTimeBoundary() + ann.getEndTimeBoundary())
                                                                  / 2);
        } else {

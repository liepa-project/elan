 ...
 * This dialog uses the TierExportTableModel in a somewhat nonstandard manner: in the tier column, which normally contains
 * Strings, it puts {@code SelectableObject<String>}s. The selection indicates the "insert blank line after" flag.
 *
 * @author Han Sloetjes
 */
@SuppressWarnings("serial")
public class ExportToolboxDialog extends AbstractTierExportDialog implements ItemListener,
    ...
    /**
     * Returns a list of tiers that are in the tree.
     *
        ...
        if (model != null) {
            ...
            if (transcription != null) {
                ...
                List<String> tierOrderList =
                    ELANCommandFactory.getViewerManager(transcription).getTierOrder().getTierOrder();
                List<TierImpl> v = new ArrayList<TierImpl>();
                for (int i = 0; i < tierOrderList.size(); i++) {
                    t = transcription.getTierWithId(tierOrderList.get(i));
                    if (t != null) {
                        v.add(t);
                    }
                }
                ...
                //                if(tierOrder == null){
                //                    tierOrder = mpi.eudico.client.annotator.commands.ELANCommandFactory.getViewerManager
                //                    (transcription).getTierOrder().getTierOrder();
                //                }
                //
                ...
                if (numRootTiers == 1) {
                    ...
                    if (detectedRMRB != null) {
                        ...
                        detectedRMRB.setText(detectedRMRB.getText() + " (\\" + recordMarker + ")");
                    }
                } else {
                ...
                for (int i = 0; i < mergedTiers.size(); i++) {
                    model.addRow(new Object[] {Boolean.TRUE, new SelectableObject<String>(mergedTiers.get(i), false)});
                }
    ...
    private void savePreferences() {
        ...
        //List tierOrder = mpi.eudico.client.annotator.commands.ELANCommandFactory.getViewerManager(transcription)
    ...
    protected boolean startExport() {
        ...
        File exportFile =
            promptForFile(ElanLocale.getString("ExportShoebox.Title.Toolbox"), null, FileExtension.TOOLBOX_TEXT_EXT, false);
        ...
        // export....
        boolean success = doExport(exportFileName, selectedTiers, charsPerLine, timeFormat, correctTimesCB.isSelected());
    ...
    protected void updateLocale() {
        ...
        markerPanel.setBorder(new TitledBorder(ElanLocale.getString("ExportShoebox.Label.ToolboxOptions")));
        wrapBlocksCB.setText(ElanLocale.getString("ExportShoebox.Label.WrapBlocks"));
        charPerLineLabel.setText(ElanLocale.getString("ExportShoebox.Label.NumberChars"));
        wrapLinesCB.setText(ElanLocale.getString("ExportShoebox.Label.WrapLines"));
        wrapAfterBlockRB.setText(ElanLocale.getString("ExportShoebox.Label.WrapEndOfBlock"));
        wrapNextLineRB.setText(ElanLocale.getString("ExportShoebox.Label.WrapNextLine"));
        includeEmptyLinesCB.setText(ElanLocale.getString("ExportShoebox.Label.IncludeEmpty"));
        appendFileNameCB.setText(ElanLocale.getString("MultiFileExportToolbox.AppendFileName"));
        timeFormatLabel.setText(ElanLocale.getString("ExportShoebox.Label.Format"));
        hhMMSSMSFormatRB.setText(ElanLocale.getString("InterlinearizerOptionsDlg.TimeCodeFormat.TimeCode"));
        ssMSFormatRB.setText(ElanLocale.getString("InterlinearizerOptionsDlg.TimeCodeFormat.Seconds"));
        correctTimesCB.setText(ElanLocale.getString("ExportDialog.CorrectTimes"));
        toolboxDBTypeLabel.setText(ElanLocale.getString("ExportShoebox.Label.ToolboxBDName"));
        ...
        recordMarkerLabel.setText(ElanLocale.getString("ExportShoebox.Label.RecordMarker"));
        detectedRMRB.setText(ElanLocale.getString("ExportShoebox.Label.Detected"));
        defaultRMRB.setText(ElanLocale.getString("ExportShoebox.Label.DefaultMarker") + " (\\block)");
        customRMRB.setText(ElanLocale.getString("ExportShoebox.Label.CustomMarker"));
    ...
    /**
     * Extracts the media file names from the media descriptors and adds them to the combobox.
     */
    ...
    */
    ...
    private void autoGenerateMarkerFile() {
        ...
        try {
            ...
            } else if (fileName.toLowerCase(Locale.getDefault()).endsWith(".eaf")) {
            ...
            if (newSaveFile != null) {
                ...
                try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(newSaveFile),
                                                                                       UTF_8))) {
                    for (MarkerRecord markerRecord : markerRecords) {
    ...
    /**
     * Checks the contents of marker input fields and next the existence of the designated files.
     *
     * @return true if the files exist, false otherwise
     */
    private boolean checkFields() {
        // database type check
        if (typeRB.isSelected() && ((typField.getText() == null) || (typField.getText().isEmpty()))) {
        ...
        if (typeRB.isSelected()) {
            ...
            if (!tf.exists()) {
                ...
                try {
                    ...
                    LOG.log(Level.WARNING, "Something wrong with file reading", e);
                }
            }
        } else {
        ...
        if (mediaMarkerCB.isSelected()) {
            ...
            if (mediaMarker == null || mediaMarker.trim().length() <= 0) {
    ...
    private void chooseTyp() {
        ...
        chooser.createAndShowFileDialog(ElanLocale.getString("ImportDialog.Title.Select"),
                                        FileChooser.OPEN_DIALOG,
                                        ElanLocale.getString("ImportDialog.Approve"),
                                        null,
                                        FileExtension.SHOEBOX_TYP_EXT,
                                        false,
     ...
     * @param correctTimes if true the master media time offset will be added to all time values
     *
     * @return {@code true} if all went well, {@code false} otherwise
     */
    private boolean doExport(final String fileName, final List<String> orderedTiers, final int charsPerLine,
                             final int timeFormat, final boolean correctTimes) {
        ...
        if (fileName != null) {
            try {
                ...
                encoder.encodeAndSave(transcription, tbEncoderInfo, transcription.getTiersWithIds(orderedTiers), fileName);
    ...
    protected List<String> getSelectedTierNames() {
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
            ...
            if (include.booleanValue()) {
                selectedTiers.add(model.getValueAt(i, nameCol).toString());
            }
        }
    ...
    private List<String> getCurrentTierOrder() {
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
            tierOrder.add(model.getValueAt(i, nameCol).toString());
        }
        return tierOrder;
    }
...
*/
    ...
    static class ListComparator implements Comparator<List<?>>,
        ...
        @Serial
        private static final long serialVersionUID = 202310251030L;

        /**
         * Compares Lists, on basis of their size. The largest one comes first
         *

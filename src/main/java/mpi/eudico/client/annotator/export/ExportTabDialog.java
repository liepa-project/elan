...
/**
 * A dialog for exporting a set of tiers to a tab delimited text file. Provides ui elements to customize the output.
 *
 * @author Han Sloetjes
 */
...
public class ExportTabDialog extends AbstractExtTierExportDialog implements ChangeListener {
    ...
    private final Insets insets = new Insets(2, 4, 2, 4);
    ...
    /**
     * Enables / disables PAL and NTSC radio buttons. Enables / disables check boxes concerning column-per-tier output or
     * not.
     *
    ...
    public void stateChanged(ChangeEvent ce) {
        if (ce.getSource() == timecodeCB) {
            ...
            repeatValuesCB.setEnabled(colPerTierCB.isSelected() && !slicedOutputCB.isSelected());
            ...
            includeAnnotationIdCB.setEnabled(repeatValuesCB.isSelected() || includeAnnotationIdCB.isSelected());
        } else if (ce.getSource() == slicedOutputCB) {
            repeatValuesCB.setEnabled(!slicedOutputCB.isSelected());
            repeatOnlyWithinCB.setEnabled(!slicedOutputCB.isSelected() && repeatValuesCB.isSelected());
            includeAnnotationIdCB.setEnabled(repeatValuesCB.isSelected() || includeAnnotationIdCB.isSelected());
        }
    }
    ...
    protected boolean startExport() throws
        ...
        File exportFile =
            promptForFile(ElanLocale.getString("ExportTabDialog.Title"), formats, FileExtension.TEXT_EXT, true);

        if (exportFile == null) {
            return false;
        }

        boolean csvExport = exportFile.getName().toLowerCase(Locale.getDefault()).endsWith(FileExtension.CSV_EXT[0]);
        // export....
        long selectionBT = 0L;
        ...
        if (transcription != null) {
            ...
            encInfoTrans.setIncludeCVDescription(includeCVEntryDesCB.isEnabled() && includeCVEntryDesCB.isSelected());
            ...
            if (suppressNamesCB.isEnabled()) {
                encInfoTrans.setIncludeNames(!suppressNamesCB.isSelected());
            }
            if (suppressParticipantsCB.isEnabled()) {
                encInfoTrans.setIncludeParticipants(!suppressParticipantsCB.isSelected());
            }
            encInfoTrans.setIncludeAnnotationId(includeAnnotationIdCB.isEnabled() && includeAnnotationIdCB.isSelected());
            ...
            encInfoFiles.setIncludeCVDescription(includeCVEntryDesCB.isEnabled() && includeCVEntryDesCB.isSelected());
            ...
            encInfoFiles.setIncludeAnnotationId(includeAnnotationIdCB.isEnabled() && includeAnnotationIdCB.isSelected());
    ...
    protected void updateLocale() {
        ...
        suppressNamesCB.setText(ElanLocale.getString("ExportTabDialog.Label.SuppressNames"));
        ...
        timeCodesLabel.setText(ElanLocale.getString("ExportTabDialog.Label.Columns"));
        timeFormatLabel.setText(ElanLocale.getString("ExportTabDialog.Label.Formats"));
        btCheckBox.setText(ElanLocale.getString("Frame.GridFrame.ColumnBeginTime"));
        etCheckBox.setText(ElanLocale.getString("Frame.GridFrame.ColumnEndTime"));
        durCheckBox.setText(ElanLocale.getString("Frame.GridFrame.ColumnDuration"));
        ...
        ntscTimecodeRB.setText(ElanLocale.getString("TimeCodeFormat.TimeCode.SMPTE.NTSC"));
        palTimecodeRB.setText(ElanLocale.getString("TimeCodeFormat.TimeCode.SMPTE.PAL"));
        pal50TimecodeRB.setText(ElanLocale.getString("TimeCodeFormat.TimeCode.SMPTE.PAL50"));

 ...
 *
 * <p>Except that the dialog is never shown, and inherits from a type of dialog
 * which facilitates selection of tiers, which isn't used...
 *
 * <p>It only uses it for its promptForFile() method.
 *
...
public class ExportStatistics extends AbstractTierExportDialog {
     ...
     */
    public ExportStatistics(Frame parent, boolean modal, TranscriptionImpl transcription, JTable statTable) {
    ...
    protected boolean startExport() throws
        ...
        try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file), encoding))) {
            ...
            LOG.log(Level.WARNING, "Something went wrong processing a file.", e);
        }
    ...
    /**
     * Converts table to String, columns are separated by tabs rows are separated by new lines.
     *

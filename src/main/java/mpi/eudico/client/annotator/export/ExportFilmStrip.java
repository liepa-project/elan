...
/**
 * A class for exporting a number of video frames together with a waveform to an image file.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
public class ExportFilmStrip {
    ...
    private final ElanMediaPlayer[] players;
     ...
     * @throws NullPointerException if players is null
     */
    public ExportFilmStrip(ElanMediaPlayer[] players, String waveFile, int frameWidth, int frameStep,
                           boolean includeWaveform, int waveHeight) {
        if (players == null) {
            throw new NullPointerException("No players to grab image from.");
        }
    ...
    /**
     * Sets the frame step. A value of e.g. 3 means that one frame is painted and then 2 are skipped. In other words; every
     * n-th frame.
     *
    ...
    /**
     * Creates the complete image on a separate thread, allowing listeners to monitor the progress. Call getImage() after
     * completion.
     *
    ...
    /**
     * Creates the image. First creates a row or multiple rows of video images, then adds the waveform below the images. If
     * stepframe is &gt; 1, it is assured that the last frame of the interval is painted, so there might be a smaller time
     * gap between the last 2 images.
     *
    ...
    public Image createImage(long beginTime, long endTime) {
        ...
        int numSamples = totalNumSamples / frameStep;
        ...
        // runs until end time
        if (frameStep > 1) {
            long timeSpan = (long) ((numSamples - 1) * (msPerSample * frameStep)) + (long) msPerSample;
        ...
        float perImg = includeWaveform ? (80 / (float) totalNumImages) : (98 / (float) totalNumImages);

        //System.out.println("Num samples: " + totalNumImages
        //        + " Per img: " + perImg);
        BufferedImage buf = new BufferedImage(totalImageWidth, totalImageHeight, BufferedImage.TYPE_INT_RGB);
        ...
        for (int i = 0; i < players.length; i++) {
            if (players[i] instanceof VideoFrameGrabber) {
                if (players[i].getAspectRatio() > 0) {
                    ...
                    paintFramesOfPlayer(g2d, players[i], i * (numSamples * perImg), perImg);
                }
            }
        }
        ...
        if (includeWaveform) {
            ...
            float mspp = (et - bt) / (float) ((numSamples * frameWidth) + ((numSamples - 1) * margin));
    ...
    /**
     * Grabs the images from the player, at times based on the segment interval and the frame step size.
     *
    ...
    private void paintFramesOfPlayer(Graphics2D g2d, ElanMediaPlayer player, float startProg, float perImg) {
        ...
        for (; curTime < et; curTime += (frameStep * msPerSample)) {
            ...
            if (curImg != null) {
                if (at == null) {
                    ...
                    at.scale(frameWidth / (float) player.getSourceWidth(), imgH / (float) player.getSourceHeight());
                ...
                if (curImg instanceof RenderedImage) {
                    if (SystemReporting.isMacOS()) {
                        ...
                        try {
                            scaleImg = new BufferedImage(frameWidth, imgH, BufferedImage.TYPE_INT_RGB);
        ...
        if (lastFrameTime < (et - msPerSample)) {
            curImg = ((VideoFrameGrabber) player).getFrameImageForTime(et - (long) msPerSample + 1);
        ...
        g2d.translate((-g2d.getTransform().getTranslateX() + margin), imgH + margin);
    }

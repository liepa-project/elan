...
public class InterlinearPreviewDlg extends ClosableDialog implements ActionListener,
    ...
    private JComboBox<String> timeCodeFormatComboBox;
    ...
    private JComboBox<Integer> silenceDecimalComboBox;
    ...
    private JComboBox<String> blockWrapComboBox;
    ...
    protected void initComponents() {
        ...
        timeCodeFormatComboBox = new JComboBox<String>();
        ...
        silenceDecimalComboBox = new JComboBox<Integer>();
        ...
        blockWrapComboBox = new JComboBox<String>();
...
//        getContentPane().add(optionsPanel, gbc);
    ...
    private void doSetFontSizes() {
        ...
        if (model != null) {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                String name = (String) model.getValueAt(i, tierCol);
                Integer size = (Integer) model.getValueAt(i, fontCol);
                fontNames.add(name);
            ...
            // update the table
            for (Map.Entry<String, Integer> entry : fontMap.entrySet()) {
    ...
    private void doApplyChanges() {
     ...
     // check the minimal silence duration
        ...
        //int imageHeight = 0;
        if (interlinearizer.getOutputMode() == Interlinear.PRINT) {
            PageFormat pageFormat = PrintCommand.getInstance().getPageFormat();
            int pageWidth = Double.valueOf(pageFormat.getImageableWidth()).intValue(); //width of printer page
            widthTextField.setText(Integer.toString(pageWidth));
            interlinearizer.setWidth(pageWidth);

            int pageHeight = Double.valueOf(pageFormat.getImageableHeight()).intValue();
    ...
    private boolean isSettingsChanged() {         	
        ...
        if(tierNames.size() != tiers.size()){
        	...
        	for (int i=0; i< tiers.size(); i++ ){
        		if(!tierNames.contains(tiers.get(i).getName())){
        			return true;
        		}
        	}
        }        

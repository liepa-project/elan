...
public class BlockMetrics {
    ...
    public static final String TC_TIER_NAME = "TC";

    /** constant for a timecode separator */
    public static final String TIME_SEP = " - ";
    
    /** constant for a silence duration tier label */
    public static final String SD_TIER_NAME = "SD";
    
    /** silence duration separator */
    public static final String SD_SEP1 = "(";
    
    /** silence duration separator */
    public static final String SD_SEP2 = ")";    
            	   ...
            	   if(sdLabel != null){            		 
            		   ...
            		   if (dur >= interlinearizer.getMinSilenceDuration()){
            			   ...
            			   if(interlinearizer.getNumOfDecimalDigits() == Constants.ONE_DIGIT){     			              			  
            				   ...
            				   silDur = SD_SEP1 + String.valueOf(Math.round(dur/10f) / 100f) + SD_SEP2;              				   
            			   } else if (interlinearizer.getNumOfDecimalDigits() == Constants.THREE_DIGIT){
            				   silDur = SD_SEP1 + String.valueOf(dur / 1000f) + SD_SEP2;              				   
            			   } 
    ...
    private void propagateSizeDown(DefaultMutableTreeNode rootNode) {
        ...
        if (rootNode.getChildCount() == 1) {
            ...
            for (int i = 0; i < rootNode.getChildCount(); i++) {
                ...
                if (tierName != null && tierName.equals(lastTierName)) {
    ...
    private void splitAnnotation(InterlinearAnnotation prAnn, int available,
        FontMetrics metrics) {
        int estimatedNrLines = (int) Math.ceil(prAnn.realWidth / (double)available);
    ...
    private boolean isTierAncestor(String tier, String otherTier) {
        ...
        Enumeration<TreeNode> en = tierTree.breadthFirstEnumeration();

        while (en.hasMoreElements()) {
            temp = (DefaultMutableTreeNode) en.nextElement();

            if (temp.getUserObject() != null && temp.getUserObject().equals(tier)) {
                node = temp;
            }

            if (temp.getUserObject() != null && temp.getUserObject().equals(otherTier)) {
                otherNode = temp;
            }
        }

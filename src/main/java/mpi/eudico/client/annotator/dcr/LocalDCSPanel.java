...
public class LocalDCSPanel extends LocalDCSelectPanel {
    ...
    protected void selectAndAddCategories() {
        ...
        if (this.getTopLevelAncestor() instanceof JDialog) {
            dialog = new ELANDCRDialog((JDialog) this.getTopLevelAncestor(), true, ELANDCRDialog.REMOTE_MODE);
        } else {
            dialog = new ELANDCRDialog((JFrame) this.getTopLevelAncestor(), true, ELANDCRDialog.REMOTE_MODE);
        }

...
/**
 * Panel to add or remove a data category reference on an annotation
 *
 * @author aarsom
 */
public class AnnotationDCPanel2 extends LocalDCSPanel {
    ...
    /**
     * Updates the text fields with information for the data category with the specified id.
     *
    ...
    public String getAnnotationDCId() {
        if ((idValueArea.getText() != null) && (idValueArea.getText().length() > 0)) {
    ...
    /**
     * Override the initialization of the description panel Adds new components to show data category reference on the
     * annotation
     */
    @Override
    ...
    protected void updateLocale() {
        ...
        if (resBundle != null) {
            isocat = getStringData(resBundle, "DCR.Label.ISOCategory", isocat);
            ident = getStringData(resBundle, "DCR.Label.Identifier", ident);
            id = getStringData(resBundle, "DCR.Label.Id", id);
            profs = getStringData(resBundle, "DCR.Label.Profiles", profs);
            tierName = getStringData(resBundle, "EditTierDialog.Label.TierName", tierName);
            ann = getStringData(resBundle, "Frame.GridFrame.ColumnAnnotation", ann);
            remove = getStringData(resBundle, "DCR.Label.RemoveReference", remove);
            remCat = getStringData(resBundle, "DCR.Label.RemoveCategory", remCat);
        }
    ...
    private String getStringData(ResourceBundle resourceBundle, String key, String defaultStr) {
        String str = defaultStr;
        try {
            str = resourceBundle.getString(key);
        } catch (Exception ex) {
            LOG.log(Level.WARNING, "Getting string from RES Bundle failed.", ex);
        }

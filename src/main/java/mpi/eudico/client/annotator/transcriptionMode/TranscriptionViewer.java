...
public class TranscriptionViewer  extends AbstractViewer 
    ...
    public void editOrNoneditableTier(String tierName){    	
    	...
    	if(tierMap.size() >=1 && columnTypeList.size() >1){
    		for (Map.Entry<TierImpl, List<TierImpl>> tierEntry : tierMap.entrySet()) {
    			if(tierEntry.getValue() != null && tierEntry.getValue().size() > 1){
     ...
     * 
     * @param tierList the tiers to change to non-editable
     */
    ...
    public void hideTiersLinkedWith(String tierName){    
		...
		if(linkedTier != null){	
    		for (Map.Entry<TierImpl, List<TierImpl>> tierEntry : tierMap.entrySet()) {
    			if (tierEntry.getValue() != null && tierEntry.getValue().contains(linkedTier)) {
    				if (!hiddenTiersList.contains(tierEntry.getKey().getName())) {
    					hiddenTiersList.add(tierEntry.getKey().getName());
    					loadTable();
    					break;
    				}
    			}
...
//		});
	...
	/**
	 * Sets whether colors should only be shown in the index column and not in
	 * the other columns.
	 * 
	 * @param selected if {@code true} colors are only shown in the index 
	 * column, otherwise colors are shown in the tier/type columns too 
	 */
		...
		if(columnTypeList != null && columnTypeList.size() > 0){
			...
			if(tierList != null && tierList.size() > 0){
				...
				if(tier.hasParentTier() && tier.getLinguisticType().getConstraints().getStereoType() == Constraint.SYMBOLIC_ASSOCIATION){						
					...
					if(parentTier != null && parentTier.getLinguisticType().getConstraints() == null){
						merge = true;							
					}	
...
//	}
	...
	public void loadTable(){		
		...
		for(int x = 0; x < parentTierListType.size(); x++){
	 	...
	 	if(annotationsList.size() == 0 && hiddenTiersList.size() == 0){
...
//				}
	...
	/**
	 * A request to revalidate and repaint the table.
	 */
	...
	public void loadPreferences(){		
		...
		if(newColumnWidth != null){			
			...
			for (Map.Entry<String, Integer> columnEntry : newColumnWidth.entrySet()) {
...
//	}
    ...
    private Map<String, List<String>> changeToStorableMap(Map<TierImpl, List<TierImpl>> map){
    	if(map != null){
    		...
    		for (Map.Entry<TierImpl, List<TierImpl>> tierEntry : map.entrySet()) {
    			TierImpl keyTier = tierEntry.getKey();
    			List<TierImpl> tierList = tierEntry.getValue();
    			...
    			newMap.put(keyTier.getName(), tierNamesList);
    		}
    		return newMap;
    	}    	
    	return null;
	...
	private void validateColumns(){		
		...
		if(nullValueIndex.size() > 0){
			...
			while(it.hasNext()){	
				tierList = it.next();
				for(int i = (nullValueIndex.size()-1) ; i >= 0; i--){
					tierList.remove(nullValueIndex.get(i).intValue());
				}
			}
			...
			if (curValue instanceof AnnotationCellPlaceholder) {
				for (int column = cell[1] + 1; column < tableModel.getColumnCount(); column++) {
					...
					if (cellValue instanceof AnnotationCellPlaceholder) {
						...
						if (acph.tierName != null) {
							Tier t = viewerManager.getTranscription().getTierWithId(acph.tierName);
							if (t != null && t.getParentTier() == ((Annotation) replacement).getTier()) {
								acph.canCreate = true;
							}
						}
					}
			...
			if (curValue instanceof Annotation) {
				for (int column = cell[1] + 1; column < tableModel.getColumnCount(); column++) {
					...
					if (cellValue instanceof Annotation) {
						...
						AnnotationCellPlaceholder acph = (AnnotationCellPlaceholder) cellValue;
						Tier t = viewerManager.getTranscription().getTierWithId(acph.tierName);
						if (t != null && t.getParentTier().getName().equals(((AnnotationCellPlaceholder) replacement).tierName)) {
							acph.canCreate = false;
						} else { //don't change
						}
					}
...
//		});
	...
	public void ACMEdited(ACMEditEvent e) {	
		...
		switch (e.getOperation()){	
			case ACMEditEvent.ADD_ANNOTATION_HERE:
				...
				if (newAnnotationPH != null && Objects.equals(addedAnn.getTier().getName(), newAnnotationPH.tierName) 
			...
			case ACMEditEvent.CHANGE_ANNOTATION_VALUE:
				// repaint the table 
				table.repaint();
				// if the annotations just has been edited the row height is already recalculated
				// but after e.g. undo/redo this is not the case, ensure row update explicitely
				Annotation modAnn = (Annotation) e.getSource();
				if (modAnn != null) {
					int[] cellRC = getCellForAnnotation(modAnn);
					if (cellRC != null) {
						table.reCalculateRowHeight(cellRC[0]);
					}
				}
	...
	public void updateActiveAnnotation() {	
		if (tableModel == null || !table.isEnabled()) {
			return;
		}

...
public class TranscriptionTableEditBox extends JPanel implements ActionListener,
	...
	// keep in sync with the same variables in TranscriptionTable
	private int extraRowHeight = 8;
	private int extraRowMargin = 2;
	// diacritical marks can cause a TextHitInfo out of range in 
	// getPreferredSize, count them and report now and then 
	private int hitInfoErrorCount = 0;
    /**
	...
	public void setFont(Font font) {
     //   super.setFont(font);
        ...
        // setFont() is called when initializing the superclass - the textarea is
        // not yet instantiated yet at that time
        if (textArea != null) {
...
//    		}
    ...
    /**
     * A request to show the popup menu at the specified location.
     * 
     * @param obj the event source component
    ...
    /**
     * Notification of a change in the specified controlled vocabulary.
     * 
     * @param cv the changed vocabulary
     */
	...
	public void keyPressed(KeyEvent e) {
    	...
    	// (e.getModifiersEx() | Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()) == Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()
    	...
    	// paste
        else if (e.getKeyCode() == KeyEvent.VK_V && (e.getModifiersEx() & 
        		Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()) != 0){
        	e.consume();
        	doPaste();
        }
    	//cut
        else if (e.getKeyCode() == KeyEvent.VK_X && (e.getModifiersEx() & 
        		Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()) != 0){
        	e.consume();
        	doCut();
        }     
...
//        }	    	
    ...
    private void initializeHeightParameterSettings(){
		...
		fontHeight = fm.getAscent()+ fm.getDescent() + extraRowMargin;
		currentRow = table.getEditingRow();
    	minRowHeight = table.getDefaultRowHeight() + extraRowHeight;	
    	maxHtForOtherColumn = table.getRowHeight(currentRow);//0
    ...
    private void recalculateRowHeight(){   
    	...
    	if(table.isEditing()){     	
    		...
    		recalculateRowHeightLater(currentHeight);
    	}
	}
    
    private  void recalculateRowHeightLater(final int currentHeight) {
			...
			public void run() {
				try {
		    		...
		    		if(maxHtForOtherColumn > height){
		    			...
		    			table.setRowHeight( currentRow, height + extraRowHeight); 
	    			...
	    			if (LOG.isLoggable(Level.INFO)) {
	    				if (hitInfoErrorCount < 4) {
	    					LOG.log(Level.INFO, "Cannot update row height: " + iae.getMessage());
	    				} else if (hitInfoErrorCount % 10 == 0) {
	    					LOG.log(Level.INFO, "Number of row height update errors: " + hitInfoErrorCount);
	    				}
	    			}
	    		} catch (Throwable t) {
	    			if (LOG.isLoggable(Level.INFO)) {
	    				LOG.log(Level.INFO, "Cannot update row height: " + t.getMessage());
	    			}
	    		}
    ...
    class CVEntryComponent {
        ...
        /** mouselistener for bringing up the popup menu */
        ...
        public void ensureSelectionIsVisible() {
            if (delegate != null && entryList != null) {

...
public class ViewerPanel extends AbstractEditPrefsPanel implements PreferenceEditor, MouseListener, ActionListener {
    ...
    private boolean origZoomSliderVisible = true;
  
    private boolean videoInCentre = false; 
    
    private JComboBox<Integer> numSubCB;
    private JCheckBox aaBoldCB;
    private JCheckBox redTierHeightCB;
    private JCheckBox zoomSliderVisibleCB;
    ...
    private void readPrefs() {
    	...
    	boolPref = Preferences.getBool("TimeLineViewer.ZoomSliderVisible", null);
    	if (boolPref != null) {
    		origZoomSliderVisible = boolPref.booleanValue();
    	}
    ...
    private void readViewerPref(){
    	for(int x=0; x< viewerTable.getRowCount(); x++){	
    		if( viewerTable.getModel().getValueAt(x, 0) instanceof SelectEnableObject){
    			...
    			boolean bool = getPrefValue(seo.getValue().toString());
    ...
    */
    ...
    private void initComponents() {
	    ...
	    numSubCB = new JComboBox<Integer>(new Integer[] { 1, 2, 3, 4, 5, 6, 7, 8 });
        ...
        zoomSliderVisibleCB = new JCheckBox(ElanLocale.getString("TimeLineViewer.ZoomSliderVisible"));
        zoomSliderVisibleCB.setFont(aaBoldCB.getFont().deriveFont(Font.PLAIN));
        zoomSliderVisibleCB.setSelected(origZoomSliderVisible);
        ...
        gbc.gridy = gy++;
        outerPanel.add(zoomSliderVisibleCB, gbc);
	...
	public Map<String, Object> getChangedPreferences() {
        if (isChanged()) {
            ...
            if (origZoomSliderVisible != zoomSliderVisibleCB.isSelected()) {
            	chMap.put("TimeLineViewer.ZoomSliderVisible", Boolean.valueOf(zoomSliderVisibleCB.isSelected()));
            }
            ...
            if(videoInCentre){
            	for(int x=0; x< viewerTable.getRowCount(); x++){	
            		if( viewerTable.getModel().getValueAt(x, 0) instanceof SelectEnableObject){
            			SelectEnableObject<?> seo = (SelectEnableObject<?>) viewerTable.getModel().getValueAt(x, 1);
            			String refValue = getRefValue(seo.getValue().toString());
    ...
    private List<String> getNewViewerSortOrder(){
    	...
    	while(row < viewerTable.getRowCount()){     		
    		newSortOrder.add(((SelectEnableObject<?>)viewerTable.getModel().getValueAt(row, 0)).getValue().toString());
    		row++;
    	}

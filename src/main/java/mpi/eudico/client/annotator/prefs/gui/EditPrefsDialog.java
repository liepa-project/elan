...
*/
        ...
        //treeScrollPane.setBackground(Constants.DEFAULTBACKGROUNDCOLOR);
    ...
    private void updateLocale() {
		...
		if (appName == null || appName.equals(ELAN.class.getName())) {
			// tree rows
			val = ElanLocale.getString("PreferencesDialog.Category.Edit");
			catKeyMap.put(val, "PreferencesDialog.Category.Edit");
			((DefaultMutableTreeNode) catTree.getModel().getRoot()).add(new DefaultMutableTreeNode(val));
		...
		val = ElanLocale.getString("PreferencesDialog.Category.AutomaticBackup");
		catKeyMap.put(val, "PreferencesDialog.Category.AutomaticBackup");
		((DefaultMutableTreeNode) catTree.getModel().getRoot()).add(new DefaultMutableTreeNode(val));
    ...
    private JPanel getPanelForKey(String key) {
        ...
        } else if (key.equals("PreferencesDialog.Category.Viewer")) {
        	return new ViewerPanel();
        } else if (key.equals("PreferencesDialog.Category.AutomaticBackup")) {
        	return new AutomaticBackupPanel();
        }
    ...
    /**
     * Pre-Loads the panel in the Edit preference dialog
     * @param val the panel value which needs to be displayed beforehand
     */
	public void preLoadPanel(String val) {
		
		JPanel panel = getPanelForKey(val);
		if (panel != null) {
			prefCatPanel.add(panel, val);
			cardLayout.show(prefCatPanel, val);
			currentEditPanel = panel;
			activatedPanels.put(val, panel);
		}
	}
}

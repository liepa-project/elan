...
*/	
	 ...
	 * This method was primarily intended for video players (that don't always
	 * provide access to that property), but in order to allow the user to 
	 * change the number of milliseconds the existing frame forward/backward
	 * buttons move the cursor, this method now returns {@code false}.
	 * 
	 * @return {@code false}, to allow setting the frame forward/backward 
	 * step size 
	 */
	@Override
	...
	public double getMilliSecondsPerSample() {
		...
		if (playerClip != null) {
			return (playerClip.getMicrosecondLength() / (double)playerClip.getFrameLength()) / MIC_TO_MIL;
		}
	...
	public void setMilliSecondsPerSample(double milliSeconds) {
		...
		if (layoutManager != null) {
			layoutManager.setPreference("MediaPlayer.CustomFrameDuration", 
					Double.valueOf(milliSecondsPerSample), 
					layoutManager.getViewerManager().getTranscription());
		}
	}

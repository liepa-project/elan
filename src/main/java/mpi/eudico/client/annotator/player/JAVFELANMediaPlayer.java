...
VideoScaleAndMove, VideoFrameGrabber {
    ...
    private boolean sampleDurChecked = false;
	...
	private volatile boolean endWatcherStarted = false;
	// for play selection
	private final ReentrantLock syncLock = new ReentrantLock();
	private boolean useNativeStopTime = true;
	
	// ensure the debug mode is set before the first native player is created
	static {
		Boolean boolPref = Preferences.getBool("NativePlayer.DebugMode", null);
		
		if (boolPref != null) {
			boolean mode = boolPref.booleanValue();
			AVFBaseMediaPlayer.setDebugMode(mode);
		}
	}
...
//        }
		...
		//endWatcher.start(); // start the thread later (at first call to start()?)
		playSelectionFlag = new AtomicBoolean();
		EventQueue.invokeLater(() -> startEndWatcherThread());
	}
	...
	private void startEndWatcherThread() {
		if (!endWatcherStarted) {
			endWatcherStarted = true;
			endWatcher.start();
		}
	}
	...
	public void preferencesChanged() {
		...
		if (boolPref != null) {
			...
			AVFBaseMediaPlayer.setDebugMode(debugMode);
...
//        setStopTime(stopTime);//??
	...
	public void nextFrame() {
		...
		if (javfPlayer.getFrameDuration() > 0.0) {
			...
			//setMediaTimeEx(javfPlayer.getMediaTimeSeconds() * 1000 - offset + getMilliSecondsPerSample());
		}
	}
	...
	/*
	 * The parameter is a double but still represents a milliseconds value.
	 * This way of setting the media time still doesn't make a difference in
	 * frame forward/backward with a msPerSample with a fractional part
	 */
	/*
	private void setMediaTimeEx(double time) {
		if (javfPlayer.isPlaying()) {
			stop();
		}
		...
		javfPlayer.setMediaTimeSeconds((time + offset) / 1000d);
		long nextTime = javfPlayer.getMediaTime();
		setControllersMediaTime(nextTime - offset);
	}
	 */
	...
	public long getMediaTime() {
		//return Math.round(javfPlayer.getMediaTimeSeconds() * 1000 - offset);

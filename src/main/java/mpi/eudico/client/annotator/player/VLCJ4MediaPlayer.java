...
public class VLCJ4MediaPlayer extends ControllerManager implements
    ...
    static {
    	...
    	if (SystemReporting.isLinux()) {
	    	LinuxWellKnownDirectoryProvider lp = new LinuxWellKnownDirectoryProvider();
	    	System.out.println(lp.priority() + " - " + lp.toString());
	    	for (String d : lp.directories())
	    		System.out.println(d);
    	}
    	
    	SystemPathDirectoryProvider spp = new SystemPathDirectoryProvider();
    	System.out.println(spp.priority() + " - " + spp.toString());
    	for (String d : spp.directories())
    		System.out.println(d);
    	
    	UserDirDirectoryProvider udp = new UserDirDirectoryProvider();
    	System.out.println(udp.priority() + " - " + udp.toString());
    	for (String d : udp.directories())
    		System.out.println(d);
    }
	...
	private void internalCreateAudioPlayer(String[] prepareOptions) {
        ...
        // try to prepare the player so that the media position can be set
        // before playing part of the audio
    	...
    	System.out.println(factory.nativeLibraryPath());
	}
    ...
    private void tryHiddenPlayer(String media) {
        ...
        try {
        	...
        	if (semaphore.tryAcquire(timeOut, TimeUnit.MILLISECONDS)) {
        		if (logger.isLoggable(Level.FINE)) {
        			logger.log(Level.FINE, "tryHiddenPlayer: acquired semaphore permit");
        		}
        	} else {
        		if (logger.isLoggable(Level.FINE)) {
        			logger.log(Level.FINE, "tryHiddenPlayer: did not acquire semaphore permit within waiting time");
        		}
        	}
...
//    			return playerComponent;	
    ...
    public void nextFrame() {
		...
		if (frameStepsToFrameBegin) {
	        ...
	        long t2 = getMediaTime();
	        if (t2 < 0 || t2 == curTime) { // nextFrame failed
	        	long curFrame = (long)(curTime / msecPerSample);	
	        	t2 = (long) Math.ceil((curFrame + 1) * msecPerSample);
	        	t2 = t2 > duration ? duration : t2;
	        	internalSetMediaTime(t2);
	        } else {
	        	setControllersMediaTime(t2);
	        }
		} else {
			long nextTime = (long) Math.ceil(curTime + msecPerSample);
			nextTime = nextTime > duration ? duration : nextTime;
			internalSetMediaTime(nextTime);
		}
		...
		if (!frameRateAutoDetected) {
			if (layoutManager != null) {
				...
				if (prefFrameDur != null) {
					msPerFrame = prefFrameDur.doubleValue();
				}
			}
		}
    }
...
// ###  Video Scale and Move  #########################################################################################
	...
	public void setVideoScaleFactor(float scaleFactor) {
		...
		if (isSpherical) {
			if (Math.abs(videoScaleFactor - 1f) < 0.01f) { //videoScaleFactor == 1
				// reset
				...
				if (Math.abs(videoScaleFactor - 1.5f) < 0.01f) {// videoScaleFactor == 1.5f
					vp.setFov(5.0f);
				} else if (Math.abs(videoScaleFactor - 2.0f) < 0.01f) {// videoScaleFactor == 2.0f
					vp.setFov(10.0f);
				} else if (Math.abs(videoScaleFactor - 3.0f) < 0.01f) {//  videoScaleFactor == 3.0f
					vp.setFov(-5.0f);
				} else if (Math.abs(videoScaleFactor - 4.0f) < 0.01f) {// videoScaleFactor == 4.0f
					vp.setFov(-10.0f);
				}

...
public class VideoMouseAdapter implements MouseListener, MouseMotionListener, ActionListener {
    ...
    private JRadioButtonMenuItem ratio_4_5_Item;
	private JRadioButtonMenuItem ratio_3_4_Item;
	private JRadioButtonMenuItem ratio_2_3_Item;
	private JRadioButtonMenuItem ratio_10_16_Item;
	private JRadioButtonMenuItem ratio_9_16_Item;
	private JRadioButtonMenuItem ratio_1_185_Item;
	private JRadioButtonMenuItem ratio_1_221_Item;
	private JRadioButtonMenuItem ratio_1_235_Item;
	private JRadioButtonMenuItem ratio_1_239_Item;
	...
	private List<JMenuItem> customItems;
	private boolean adjustCoordinates = false;
	...
	/**
	 * Removes the adapter as listener from the visual component and sets 
	 * references to passed objects to {@code null}.
	 */
	public void release() {
	...
	/**
	 * Sets whether the coordinates of a mouse event need to be recalculated
	 * for this video component.
	 * 
	 * @param adjust the new value for the adjust coordinates flag
	 */
	public void setAdjustCoordinates(boolean adjust) {
		adjustCoordinates = adjust;
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (scaledPlayer != null) {
	        try {
        		...
        		if (defTrans != null && ! defTrans.isIdentity() && adjustCoordinates) {
        			videoBounds[0] = (int) Math.round(videoBounds[0] / defTrans.getScaleX());
        			videoBounds[1] = (int) Math.round(videoBounds[1] / defTrans.getScaleY());
        			videoBounds[2] = (int) Math.round(videoBounds[2] / defTrans.getScaleX());
        			videoBounds[3] = (int) Math.round(videoBounds[3] / defTrans.getScaleY());
        		}
...
//        }
	...
	private void createPopup() {
		...
		arMenu.addSeparator();
		// portrait ratios
		ratio_4_5_Item = new JRadioButtonMenuItem("4:5");
		ratio_4_5_Item.setActionCommand("ratio_4_5");
		ratio_4_5_Item.addActionListener(this);
		arbg.add(ratio_4_5_Item);
		ratio_3_4_Item = new JRadioButtonMenuItem("3:4");
		ratio_3_4_Item.setActionCommand("ratio_3_4");
		ratio_3_4_Item.addActionListener(this);
		arbg.add(ratio_3_4_Item);
		ratio_2_3_Item = new JRadioButtonMenuItem("2:3");
		ratio_2_3_Item.setActionCommand("ratio_2_3");
		ratio_2_3_Item.addActionListener(this);
		arbg.add(ratio_2_3_Item);
		ratio_10_16_Item = new JRadioButtonMenuItem("10:16");
		ratio_10_16_Item.setActionCommand("ratio_10_16");
		ratio_10_16_Item.addActionListener(this);
		arbg.add(ratio_10_16_Item);
		ratio_9_16_Item = new JRadioButtonMenuItem("9:16");
		ratio_9_16_Item.setActionCommand("ratio_9_16");
		ratio_9_16_Item.addActionListener(this);
		arbg.add(ratio_9_16_Item);
		ratio_1_185_Item = new JRadioButtonMenuItem("1:1.85");
		ratio_1_185_Item.setActionCommand("ratio_1_185");
		ratio_1_185_Item.addActionListener(this);
		arbg.add(ratio_1_185_Item);
		ratio_1_221_Item = new JRadioButtonMenuItem("1:2.21");
		ratio_1_221_Item.setActionCommand("ratio_1_221");
		ratio_1_221_Item.addActionListener(this);
		arbg.add(ratio_1_221_Item);
		ratio_1_235_Item = new JRadioButtonMenuItem("1:2.35");
		ratio_1_235_Item.setActionCommand("ratio_1_235");
		ratio_1_235_Item.addActionListener(this);
		arbg.add(ratio_1_235_Item);
		ratio_1_239_Item = new JRadioButtonMenuItem("1:2.39");
		ratio_1_239_Item.setActionCommand("ratio_1_239");
		ratio_1_239_Item.addActionListener(this);
		arbg.add(ratio_1_239_Item);
		
		arMenu.add(ratio_4_5_Item);
		arMenu.add(ratio_3_4_Item);
		arMenu.add(ratio_2_3_Item);
		arMenu.add(ratio_10_16_Item);
		arMenu.add(ratio_9_16_Item);
		arMenu.add(ratio_1_185_Item);
		arMenu.add(ratio_1_221_Item);
		arMenu.add(ratio_1_235_Item);	
		arMenu.add(ratio_1_239_Item);
	...
	/**
	 * Allows to add custom menu items, configured and managed by the calling
	 * player framework.
	 * 
	 * @param customMI a custom menu item
	 */
	public void addCustomMenuItem(JMenuItem customMI) {
		if (customMI != null) {
			if (customItems == null) {
				customItems = new ArrayList<JMenuItem>();
			}
			if (!customItems.contains(customMI)) {
				customItems.add(customMI);
				if (popup != null) {
					popup.add(customMI);
				}
			}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(detachItem) && (layoutManager != null)) {
	        ...
	        if (e.getSource() == origRatioItem) {
				...
				aspectRatio = 1.60f; // or 8:5, or 5:3 => 1.67
				...
				aspectRatio = 2.39f;
			} // portrait versions of the above
			else if (e.getSource() == ratio_4_5_Item) {
				aspectRatio = 0.8f;
			} else if (e.getSource() == ratio_3_4_Item) {
				aspectRatio = 0.75f;
			} else if (e.getSource() == ratio_2_3_Item) {
				aspectRatio = 0.67f;
			} else if (e.getSource() == ratio_10_16_Item) {
				aspectRatio = 0.625f; // or 5:8, or 3:5 => 0.6
			} else if (e.getSource() == ratio_9_16_Item) {
				aspectRatio = 0.56f;
			} else if (e.getSource() == ratio_1_185_Item) {
				aspectRatio = 0.54f;	
			} else if (e.getSource() == ratio_1_221_Item) {
				aspectRatio = 0.45f;		
			} else if (e.getSource() == ratio_1_235_Item) {
				aspectRatio = 0.425f;
			} else if (e.getSource() == ratio_1_239_Item) {
				aspectRatio = 0.418f;
			}
    ...
    /**
     * For some video components the coordinates of a mouse click or mouse 
     * press need to be recalculated in order to get the correct location.
     *  
     * @param comp the source component
     * @param org the original, reported coordinates
     * @return the recalculated coordinates
     */

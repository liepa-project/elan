...
public class TranscriptionManager implements ModeLayoutManager, ActionListener {
	...
	/** default {@code select a type} string for combo boxes */
	...
	public void doLayout() {				
        ...
        if(viewerManager != null && viewerManager.getMasterMediaPlayer() instanceof EmptyMediaPlayer
        		&& signalViewer == null){
	...
	/**
	 * Returns the main layout manager.
	 * 
	 * @return the main layout manager
	 */
	...
	/**
	 * Returns the player controller of the Transcription Mode.
	 *   
	 * @return the special player controller
	 */
    ...
    private Map<TierImpl, List<TierImpl>> getValidatedTierMap(Map<String, List<String>> map, List<Integer> deletedTypeIndexList){
    	...
    	for (Map.Entry<String, List<String>> entry : map.entrySet()) {
    		...
    		List<String> tierNamesList = entry.getValue();
    		
    		if (tierNamesList != null) {
	    		TierImpl referenceTier = (TierImpl)viewerManager.getTranscription().getTierWithId(keyObj);
	...
	private void applyChanges(Map<TierImpl, List<TierImpl>> hashMap, List<String> hiddenTiers, 
		...
		if(size != null){
			if(!size.equals(transViewer.getFontSize())){				
	...
	public void actionPerformed(ActionEvent e) {			
		if (e.getSource() == butAlignVideoToRight){
			...
			viewerManager.getMasterMediaPlayer().stop();
			TranscriptionModeSettingsDlg settingsDialog = new TranscriptionModeSettingsDlg(layoutManager, transViewer.getColumnTypes(), 
					transViewer.getTierMap(), transViewer.getHiddenTiers(), transViewer.getFontSize());
	        WindowLocationAndSizeManager.postInit(settingsDialog, "TranscriptionModeSettingsDialog");
			settingsDialog.setVisible(true);
			WindowLocationAndSizeManager.storeLocationAndSizePreferences(settingsDialog, "TranscriptionModeSettingsDialog");
			applyChanges(settingsDialog.getTierMap(), settingsDialog.getHiddenTiersList(), settingsDialog.getColumnTypeList(), 
					settingsDialog.getFontSize(), settingsDialog.isValueChanged());
		} 
		else {			
...
//			}					
	...
	public void isClosing() {	
		...
		if(signalViewer != null){	
			...
			Float zoom = Preferences.getFloat("SignalViewer.ZoomLevel", transcription);
			if (zoom != null) {
				setPreference("TM.SignalViewer.ZoomLevel", zoom, transcription);
			}

...
public class SyncManager implements ActionListener, ItemListener, ChangeListener, ModeLayoutManager {
   ...
   private void resetOffsets() {
     ...
     */
    private void updateMultiPlayer(JComboBox<? extends Object> sourceComponent) {
    	Object playerObj = null;
    	JPanel panel = null;
    	
    	labelloop:
    	for (Map.Entry<ElanMediaPlayer, JPanel> entry : labelForPlayer.entrySet()) {
    		playerObj = entry.getKey();
		...
		if (e.getStateChange() == ItemEvent.SELECTED) {
			if (e.getSource() instanceof JComboBox<?>) {
				...
				if (playerButtons.containsValue(e.getSource())) {
					...
					for (Map.Entry<ElanMediaPlayer, JRadioButton> entry : playerButtons.entrySet()) {
						ElanMediaPlayer player = entry.getKey();
	...
	public void updateLocale() {
		...
		//update player radio buttons and player labels
		for (Map.Entry<ElanMediaPlayer, String> entry : nameForPlayer.entrySet()) {
			ElanMediaPlayer player = entry.getKey();
			String playerName = entry.getValue();
			JRadioButton b = playerButtons.get(player);
			b.setText(ElanLocale.getString("SyncMode.Label.Player") + " " + 
					playerName);
			JPanel p = labelForPlayer.get(player);
			try {

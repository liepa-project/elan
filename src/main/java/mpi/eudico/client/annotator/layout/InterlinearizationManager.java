...
public class InterlinearizationManager
	...
	private void readPreferences() {	
		...
		Map<String, List<String>> excludedTiersMap = null;
	...
	private void storeGUIPreferences() {
		...
		double size = (double) leftRightSplit.getWidth() - leftRightSplit.getDividerSize();
		double loc = size > 0 ? (value / size) : -1d;
		if (loc >= 0 && loc <= 1) {
			Preferences.set(INTER_LR_SPLIT_KEY,
					Double.valueOf(loc), transcription, false, false);
		}
		
		value = leftTopBottomSplit.getDividerLocation();
		size = (double) leftTopBottomSplit.getHeight() - leftTopBottomSplit.getDividerSize();
		loc = size > 0 ? (value / size) : -1d;
		if (loc >= 0 && loc <= 1) {
	...
	private void readGUIPreferences() {
		...
		if (intObj != null) {
			double hpropLoc = intObj * hScale;
			if (hpropLoc >= 0 && hpropLoc <= 1) {
				leftRightSplit.setDividerLocation(hpropLoc);
			}
		}
		
		intObj = Preferences.getDouble(INTER_LTB_SPLIT_KEY, transcription);
		if (intObj != null) {
			double vpropLoc = intObj * vScale;
			if (vpropLoc >= 0 && vpropLoc <= 1) {
				leftTopBottomSplit.setDividerLocation(vpropLoc);
			}
		}
		/*

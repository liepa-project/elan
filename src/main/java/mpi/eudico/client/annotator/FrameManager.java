...
public class FrameManager implements PreferencesListener{
    ...
    /** the max length of the file path, maybe this should be configurable (not final) */
    ...
    private static final String FR_NAME = "Frame-";

    /** the preferences key for the recent file list */
    private static final String RECENTS_PREF = "FrameManager.RecentFiles";

    /** exit message preference */
    private static final String EXIT_PREF = "FrameManager.ExitMes";
...
//                	frame.updateShortcutMap(modeName);
    ...
    public void exit() {    	
    	...
    	try {
	        ...
	        if (exitAllowed) {
	        	...
	        	// to suppress static analysis warnings
	        	exitLock.unlock();
	            System.exit(0);
	        }
    	} finally {
    ...
    /**
     * adds window closing event to the shutdown listener

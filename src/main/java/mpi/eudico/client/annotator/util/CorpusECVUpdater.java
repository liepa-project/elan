...
public class CorpusECVUpdater {
	...
	public int updateFiles(String[] folders, String forceLang) {
		...
		logMessage("\n");
		for (Map.Entry<String, Integer> unknownEntry : unknownAnnotations.entrySet()) { 
			logMessage("UNKNOWN ANNOTATION: " + unknownEntry.getKey() + " " + unknownEntry.getValue());
		}
				...
				if (file.getName().toLowerCase().endsWith(".eaf")) {
					...
					if (outputFolder == null) {
						...
						if (!outputSubFolder.equals(outputFolderString)) {
							...
							if (!osFile.exists()) {
								boolean created = osFile.mkdirs();
								if (!created) {
									System.out.println("Could not create output folder: " + outputSubFolder);
								}
							}
						}
...
//						}
	...
	private static void removeACMEditableObjects(HashMap<String, List<ExternalCV>> loadedMap) {
		Iterator<String> ecvIter = loadedMap.keySet().iterator();
	...
	/**
	 * Setter method for recursive variable
	 * @param recursive the recursive boolean variable
	 */
	...
	/**
	 * Returns whether annotation values are not to be changed.
	 * 

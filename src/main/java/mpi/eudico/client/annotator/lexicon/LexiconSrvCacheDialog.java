...
ActionListener, ListSelectionListener{
    ...
    private DefaultListModel<LexiconQueryBundle2> model;
    private JList<LexiconQueryBundle2> lexServiceList;
	...
	private void initComponents() {
        ...
        model = new DefaultListModel<LexiconQueryBundle2>();
        if (cacheBundles != null) {
        	for (int i = 0; i < cacheBundles.size(); i++) {
        		model.addElement(cacheBundles.get(i));
        	}
        }
        lexServiceList = new JList<LexiconQueryBundle2>(model);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == applyButton) {
			List<LexiconQueryBundle2> sels = lexServiceList.getSelectedValuesList();

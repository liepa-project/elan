...
public class AnnotationsFromGapsDlg extends ClosableDialog
    ...
    private JComboBox<String> onTierCB;
	...
	public void actionPerformed(ActionEvent e) {
        if (e.getSource() == selectAllButton) {
            ...
            //get selected tier names from the list
            String[] tierNames = tierList.getSelectedValuesList().toArray(new String[0]);
    ...
    /**
     * Event Listener for List components, currently only the tier list.
     *
     * @param event selection event
     */
    @Override
	public void valueChanged(ListSelectionEvent event) {
        if (event.getSource() == tierList) {
	        int nrSelectedItems = tierList.getSelectedIndices().length;
	        int firstSelectedIndex = tierList.getSelectedIndex();
	        ...
	        default:
	            ...
	            for (String name : tierList.getSelectedValuesList()) {
					onTierCB.addItem(name);
				}

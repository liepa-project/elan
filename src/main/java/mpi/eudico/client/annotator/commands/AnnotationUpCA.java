...
/**
 * Action that finds the annotation to activate on the tier visually above the tier the current active annotation is on (if
...
public class AnnotationUpCA extends CommandAction {
    ...
    public AnnotationUpCA(ViewerManager2 theVM) {
        ...
        getImageIconFrom(Constants.ICON_LOCATION + "GoToUpperAnnotation.gif").ifPresent(imageIcon -> {
            icon = imageIcon;
    ...
    protected Object[] getArguments() {
        ...
        if (currentActiveAnnot != null) {
            ...
            if ((index > 0) && (index <= (visibleTiers.size() - 1))) {
                ...
                TierImpl firstTier = getNextNonEmptyTier(visibleTiers.size() - 1);
            ...
            if ((activeTier != null) && (activeTier.getNumberOfAnnotations() > 0)) {
                newActiveAnnot = getAnnotationAtOrBefore((TierImpl) activeTier, vm.getMasterMediaPlayer().getMediaTime());
            } else {
                // use last visible tier
                TierImpl lastTier = getNextNonEmptyTier(visibleTiers.size() - 1);

                if (lastTier == null) {
                    newActiveAnnot = currentActiveAnnot;
                } else {
                    newActiveAnnot = getAnnotationAtOrBefore(lastTier, vm.getMasterMediaPlayer().getMediaTime());
                }
            }
        }
     ...
     */
    private Annotation getAnnotationAtOrBefore(final TierImpl tier, final long time) {

...
public class ClipMediaCA extends CommandAction implements SelectionListener {
    ...
    private final ClipWithScriptUtil scriptUtil;
    ...
    protected void newCommand() {
        command = ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.CLIP_MEDIA);
    }
    ...
    protected Object[] getArguments() {
        if (scriptFile == null) {
            return new Object[] {vm,
                                 new Exception(ElanLocale.getString("ExportClipDialog.Message.NoScript")
                                               + "\n"
    ...
    public void updateSelection() {
        ...
        setEnabled(vm.getSelection().getEndTime() > vm.getSelection().getBeginTime());
    }

    /**
     * Checks if a script has already been found and if so checks the last modified value. If needed the script is read
     * again.
     *

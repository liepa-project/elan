...
/**
 * A command to perform a find and replace in multiple eaf files. Builds on the single file and replace functionality.
 *
 * @author Han Sloetjes
 */
public class MultiFindAndReplaceCommand implements Command,
                                                   ProcessReporter {
     ...
     *
     * @param receiver null
     * @param arguments the arguments: <ul><li>arg[0] = the files to search (File[])</li> <li>arg[1] the tiers to search,
     *     in case of null or zero length all tiers will be searched (String[])</li>
     *     <li>arg[2] the search pattern (String)</li>
     *     <li>arg[3] the replace pattern (String)</li> <li>arg[4] flag
     *     whether it is a regular expression (Boolean)</li> <li>arg[5] flag whether to match case sensitive (Boolean)</li>
     *     </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        if ((searchPattern == null) || (searchPattern.length() == 0) || (replPattern == null)) {
    ...
    /**
     * Requests the running find and replace thread to stop. It depends on the current ongoing operation how long it will
     * take before execution stops.
     */
    ...
    private class ReplaceThread extends Thread {
        ...
        /**
         * Sets the flag that an interrupt request was received. The current find  and replace action will stop if it is not
         * just writing a changed transcription. This methods returns immediately.
         */
        @Override
        public void interrupt() {
        ...
        public void run() {
            ...
            for (int i = 0; i < searchFiles.length; i++) {
                ...
                if (!f.exists() || f.isDirectory()) {
                    ...
                    progressUpdate((int) (i * perFileProg), ("Skipping file: " + f.getAbsolutePath()));
                ...
                try {
                    ...
                    report("Cause: " + ((ex.getMessage() != null)
                                        ? ex.getMessage()
                                        : "Unknown file loading or parsing error..."));
                    progressUpdate((int) (i * perFileProg), ("Can not load file: " + f.getAbsolutePath()));
                ...
                if (!Objects.equals(selectedTiers[0], Constraint.ALL_TIERS)) {
                    ArrayList<String> validNames = new ArrayList<>(selectedTiers.length);
                    ...
                    if (validNames.isEmpty()) {
                        ...
                        numInspected++;
                        report("None of the selected tiers found \nin file: %s".formatted(f.getAbsolutePath()));
                        //finalReport(numInspected, numChanged, numFailed);
                        progressUpdate((int) (i * perFileProg), ("None of the tiers in file: " + f.getAbsolutePath()));
                        continue; // next file
                    }
                ...
                try {
                    ...
                    numInspected++;
                    report("Number of hits in " + f.getAbsolutePath() + " :  " + numHits);

                    if (numHits == 0) {
                        progressUpdate((int) (i * perFileProg), ("No hits in file: " + f.getAbsolutePath()));
                    ...
                    if (!interruptRequested) {
                        ...
                        try {
                            ACMTranscriptionStore.getCurrentTranscriptionStore()
                                                 .storeTranscription(trans, null, new ArrayList<TierImpl>(0), saveAsType);
                            numChanged++;
                            progressUpdate((int) (i * perFileProg), ("Processed succesfully: " + f.getAbsolutePath()));
                            report("Saved file successfully: " + f.getAbsolutePath());
                        } catch (IOException ioe) {
                            numFailed++;
                            report("Could not save the file: " + f.getAbsolutePath());
                            report("because: " + ((ioe.getMessage() != null) ? ioe.getMessage() : "unknown cause..."));
                        }
                    ...
                    finalReport(numInspected, numChanged, numFailed);
                    progressUpdate((int) (i * perFileProg), ("Cannot perform search in file: " + f.getAbsolutePath()));
                    ...
                    numFailed++;
                    numInspected++;
                    report("Exception while executing query: " + ex.getMessage() + "\nin file: " + f.getAbsolutePath());
                    progressUpdate((int) (i * perFileProg), ("Cannot perform search in file: " + f.getAbsolutePath()));
                }
            }

...
/**
 * Undoable Command that calculates overlaps of annotations on 2 tiers and creates a new annotation for each overlap found.
 * Position and duration of the new annotation corresponds to begin time and length of the overlap. Optionally the annotation
 * gets the duration of the overlap as value.
 *
 * @author Han Sloetjes
...
public class AnnotationsFromOverlapsClasCommand implements UndoableCommand {
    ...
    /**
     * Calculates the overlaps of the annotations on 2 source tiers and creates new annotation on the destination tier of the
     * duration of the overlaps and optionally adds the duration as the annotations' values.   <br> Oct 2008: a flag is added
     * that determines to only create annotation for overlapping annotations with the same value.
     * <b>Note: </b>it is assumed the types and order of the arguments are
     * correct.
     *
     * @param receiver the transcription
     * @param arguments the arguments: <ul><li>arg[0] = the first source tier (String)</li> <li>arg[1] the second source
     *     tier (String)</li>
     *     <li>arg[2] the destination tier name (new tier) (String)</li>
     *     <li>arg[3] the name of the LinguisticType for the new tier
     *     (String)</li> <li>arg[4] whether or not to make the duration of the overlap the value of  each annotation
     *     (Boolean)</li>
     ...
     *     <li>arg[7] a flag that determines that only one specific annotation value is considered
     *     (Boolean)</li>
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (destTier != null) {
            ...
            while (destTier != null) {
                ...
                destTier = transcription.getTierWithId(uniqueName);
                count++;
            }

            LOG.warning("Tier " + destName + " already exists, changed name to " + uniqueName);
            destName = uniqueName;
        }
        ...
        if (type == null) {
            ...
            for (int i = 0; i < transcription.getLinguisticTypes().size(); i++) {
                ...
                if (countType.getConstraints() == null) {
                    LOG.warning("LinguisticType "
                                + typeName
                                + " could not be found, using "
                                + countType.getLinguisticTypeName()
                                + " instead.");
                    type = countType;
        ...
        if ((transcription != null) && (destTier != null)) {
            ...
            if ((annRecords != null) && (annRecords.size() > 0)) {
                ...
                for (int i = 0; i < annRecords.size(); i++) {
                    ...
                    ann = destTier.createAnnotation(annotationDataRecord.getBeginTime(), annotationDataRecord.getEndTime());
    ...
    /**
     * A thread class that compares the annotations of two tiers, detects the ovelaps and  creates a new annotation on a
     * third tier, with the duration of the overlap.
     */
                    ...
                    if (et2 <= bt1) {
                        ...
                        if (matchingValuesOnly) {
                            if (!specificValueOnly) {
                                ...
                                if (ann1.getValue() != null
                                    && ann2.getValue() != null
                                    && ann1.getValue()
                                           .equals(specificValue)
                                    && ann2.getValue().equals(specificValue)) {
                                    overlaps.add(new long[] {obt, oet});
                                    if (!addContent) {
...
//                        }
    ...
    private static boolean isOverlapping(AbstractAnnotation ann1, AbstractAnnotation ann2) {
        String value1 = ann1.getValue();
        String value2 = ann2.getValue();
        return (value1 == null && value2 == null)
            || ((value1 != null && value2 != null) && (value1.isEmpty() && value2.isEmpty())

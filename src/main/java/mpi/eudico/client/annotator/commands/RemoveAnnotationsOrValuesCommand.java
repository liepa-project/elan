...
/**
 * Removes all annotations or annotation values from the selected tiers. Or removes only the annotation which matches with
 * the given value in the selected tiers.
 *
...
public class RemoveAnnotationsOrValuesCommand implements UndoableCommand {
    ...
    public void undo() {
        ...
        if (annotationValues) {
            ...
            if (tierNames != null) {
                if (records != null) {
                    ...
                    for (int i = 0; i < records.size(); i++) {
                        ...
                        if (tier != null) {
                            ann = (AbstractAnnotation) tier.getAnnotationAtTime(annotationValuesRecord.getBeginTime());

                            if ((ann != null) && (ann.getEndTimeBoundary() == annotationValuesRecord.getEndTime())) {
                                restoreValueAndRefs(ann, annotationValuesRecord);
                            } else {
            ...
            if (delAnnRecordsMap != null) {

                List<DefaultMutableTreeNode> delAnnRecords;
                String name = null;
                    ...
                    if (name != null) {
                        ...
                        if (allAnnotations) {
                            ...
                            // if the parent tier is already selected, then that will take care the depend
                                ...
                                AnnotationRecreator.createAnnotationsSequentially(transcription, delAnnRecords, true);
                            }
     ...
     *
     * @return hashMap with the tier and the list of annotations in that tier to be deleted
     */
            ...
            for (int i = 0; i < tierNames.size(); i++) {
                ...
                if (anns != null && anns.size() > 0) {
                    for (AbstractAnnotation ann : anns) {
                        ...
                        ann = (AbstractAnnotation) tier.getAnnotationAtTime((annotationValuesRecord.getBeginTime()
                                                                             + annotationValuesRecord.getEndTime()) / 2);
    ...
    /**
     * Currently removes all external references. It is unclear whether this is the best approach in all cases. It seems all
     * right for content related references (CV entry, lexical entry, ISOcat DC), for resource url's it might be different
     *
     * @param ann the annotation to remove the value from including external references
     */
     ...
     * @param annotationDataRecord the annotationDataRecord containing the information to restore
     */
    private void restoreValueAndRefs(AbstractAnnotation ann, AnnotationDataRecord annotationDataRecord) {
        ann.setCVEntryId(annotationDataRecord.getCvEntryId());
        ann.setExtRef(annotationDataRecord.getExtRef());

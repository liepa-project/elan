...
/**
 * A Command that filters the contents of the annotations of a source tier  to new annotations on a destination tier. Can
 * also be used to copy a tier. The destination is a tier of type symbolic association (one-to-one relationship).
 *
 * @author Han Sloetjes
 */
public class FilterTierCommand implements UndoableCommand {
        ...
        if (newAnnotations.size() > 0) {
            ...
            for (int i = 0; i < newAnnotations.size(); i++) {
                ...
                if (destAnn != null) {
                    ...
                    LOG.warning("Undo filter tier: could not remove annotation: "
                                + destRecord.getValue()
                                + " "
        ...
        if (!preserve && (existAnnotations.size() > 0)) {
            ...
            for (int i = 0; i < existAnnotations.size(); i++) {
                ...
                if (extAnn != null) {
                    ...
                    LOG.warning("Undo filter tier: could not restore annotation value: "
                                + extRecord.getValue()
                                + " "
                                + extRecord.getBeginTime()
                                + " - "
    ...
    public void redo() {
        ...
        if (newAnnotations.size() > 0) {
            ...
            for (int i = 0; i < newAnnotations.size(); i++) {
                ...
                long time = (destRecord.getBeginTime() + destRecord.getEndTime()) / 2;


                destAnn = (AbstractAnnotation) destTier.createAnnotation(time, time);

                if (destAnn != null) {
                    destAnn.setValue(destRecord.getValue());
                } else {
                    LOG.warning("Redo filter tier: could not recreate annotation: "
                                + destRecord.getValue()
                                + " "
                                + destRecord.getBeginTime()
                                + " - "
        ...
        if (!preserve && (existChangedAnnotations.size() > 0)) {
            ...
            for (int i = 0; i < existChangedAnnotations.size(); i++) {
                ...
                if (extAnn != null) {
                    ...
                    LOG.warning("Redo filter tier: could not recreate annotation value: "
                                + extRecord.getValue()
                                + " "
                                + extRecord.getBeginTime()
                                + " - "
     ...
     * @param arguments the arguments: <ul><li>arg[0] = the name of the source tier (String)<li> <li>arg[1] = the name of
     *     the destination tier (String)
     *     </li> <li>arg[2] = the filter strings (String[])</li> <li>arg[3] =
     *     a flag denoting whether or not to preserve existing annotations on the destination tier (Boolean)</li> <li>arg[4]
     *     = a flag denoting whether or not to create new annotations for empty source annotations (Boolean)</li></ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        existChangedAnnotations = new ArrayList<>();
    ...
    /**
     * Applies the specified filters to the source string.<br> The output is created by extracting/removing the strings in
     * the filters from the source.
     *
    ...
    public String applyFilters(String source, String[] filters) {
        ...
        for (int i = 0; i < filters.length; i++) {
            if ((filters[i] == null) || (filters[i].length() == 0) || (filters[i].length() > resultchars.length)) {
                continue;
            }
    ...
    private class FilterThread extends Thread {
        /**
         * Before using this inner class we must ensure none of the relevant fields (transcription, source tier and
         * destination tier) are null!
         */
        FilterThread() {
        }
        ...
        public void run() {
            final IndeterminateProgressMonitor monitor =
                new IndeterminateProgressMonitor(ELANCommandFactory.getRootFrame(transcription),
                                                 true,
            ...
            for (AbstractAnnotation srcAnn : sourceAnnos) {
                ...
                if ((childrenOnDest.size() > 0) && !preserve) {
                    ...
                    existAnnotations.add(new AnnotationDataRecord(childOnDest));

                    childOnDest.setValue(FilterTierCommand.this.applyFilters(srcAnn.getValue(),
                                                                             FilterTierCommand.this.filters));

                    // store the annotation with the new value for redo
                    existChangedAnnotations.add(new AnnotationDataRecord(childOnDest));
                }
                ...
                if (childrenOnDest.size() == 0) {
                    ...
                    if (srcValue.length() == 0) {
                        ...
                        if (createEmpty) {
                            long time = (srcAnn.getBeginTimeBoundary() + srcAnn.getEndTimeBoundary()) / 2;
                            Annotation ann = destTier.createAnnotation(time, time);

                            if (ann != null) {
                                newAnnotations.add(new AnnotationDataRecord(ann));
                            } else {
                                LOG.warning("Filter tier: could not create a new annotation for: "
                                            + srcValue
                                            + " "
                                            + srcAnn.getBeginTimeBoundary()
                                            + " - "
                                            + srcAnn.getEndTimeBoundary());
                            }
                        }
                    } else {
                        long time = (srcAnn.getBeginTimeBoundary() + srcAnn.getEndTimeBoundary()) / 2;
                        ...
                        if (ann != null) {
                            // apply filters
                            ann.setValue(FilterTierCommand.this.applyFilters(srcAnn.getValue(),
                                                                             FilterTierCommand.this.filters));
                            newAnnotations.add(new AnnotationDataRecord(ann));
                        } else {
                            LOG.warning("Filter tier: could not create a new annotation for: "

...
public class NgramStatsCommand implements Command {
    ...
    private final SimpleReport report = new SimpleReport(ElanLocale.getString("ProcessReport"));

    /**
     * Constructor to create the new NgramStatsCommand instance
     *
    ...
    private class NgramsThread extends Thread {
        ...
        public void run() {
            ...
            // [Error] 640_slave2.eaf:1562:92: cvc-attribute.3: The value 'a10' of attribute 'ANNOTATION_ID' on
            // element 'ALIGNABLE_ANNOTATION' is not valid with respect to its type, 'ID'.
            ...
            for (int i = 0; i < selectedFiles.size(); i++) {
                try {
                    ...
                    if (tier != null) {
                        ...
                        report.append("Selected tier("
                                      + ngramsResult.getTier()
                                      + ") not present in file: "
                                      + selectedFiles.get(i));
                    }
                    ...
                    progressUpdate((int) ((i + 1) * perFileProg - 1),
                                   "Unable to load file(" + selectedFiles.get(i) + "): " + ex.getMessage());
                    continue;
                }
            }
            ...
            report.prepend("----------\n Parsing run done in "
                           + ngramsResult.getSearchTime()
                           + " secs"
                           + "\n Selected Domain: "
                           + ngramsResult.getDomain()
                           + "\n Selected Tier: "
                           + ngramsResult.getTier()
                           + "\n N-gram Size: "
                           + ngramsResult.getNgramSize()
                           + "\n Files Inspected: "
                           + ngramsResult.getNumFiles()
                           + "\n Files Failed: "
                           + numFailed
                           + "\n Total Annotations: "
                           + ngramsResult.getNumAnnotations()
                           + "\n Total N-grams: "
                           + ngramsResult.getNumNgrams()
                           + "\n Collated N-grams: "
                           + ngramsResult.getNumCollectedNgrams()
                           + "\n----------\n");

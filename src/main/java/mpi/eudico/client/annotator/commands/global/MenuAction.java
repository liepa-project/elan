...
/**
 * The base class for action objects that are not connected to a particular document object (Transcription). Used for
 * JMenu's, that have an empty actionPerformed method, and a few JMenuItems that are present in e.g.  an empty Elan frame.
 * Useful for handling updateLocale and setting the mnemonic.
 *
 * @author Han Sloetjes, MPI
 * @version April 2009: keyboard shortcuts are now obtained via the ShortcutUtil class. The accelerator keys are now user
 *     definable.
 */
...
public class MenuAction extends AbstractAction implements ElanLocaleListener {
    ...
    public MenuAction(String name) {
        ...
        commandId = name;
        putValue(Action.ACCELERATOR_KEY, ShortcutsUtil.getInstance().getKeyStrokeForAction(commandId, null));
        updateLocale();
    }

    /**
     * Updates the NAME (what is shown in menu's and on buttons), tooltip  and eventually mnemonic.
     */
    @Override

...
/**
 * Undoable Command that merges the annotations on tiers and creates new annotations on a third tier. Overlapping annotations
 * are merged into one new annotation. Non overlapping annotations are copied as is. The content depends  on the user's
 * choice
 *
 * @author aarsom
 * @version Feb, 2014
    ...
    public void redo() {
        if (transcription != null) {
            ...
            if (dt == null) {
                ...
                TierImpl dTier = new TierImpl(parent, destTierName, tierParticipant, transcription, lType);
            ...
            for (int i = 0; i < createdAnnos.size(); i++) {
                annotationDataRecord = createdAnnos.get(i);
                ann = dt.createAnnotation(annotationDataRecord.getBeginTime(), annotationDataRecord.getEndTime());
                ...
                if (annotationDataRecord.getCvEntryId() != null) {
            ...
            if (destTierCreated) {
                ...
                if (st != null) {
                    if ((createdAnnos != null) && (createdAnnos.size() > 0)) {
                        ...
                        for (int i = 0; i < createdAnnos.size(); i++) {
                            annotationDataRecord = createdAnnos.get(i);
                            ann = st.getAnnotationAtTime((annotationDataRecord.getBeginTime()

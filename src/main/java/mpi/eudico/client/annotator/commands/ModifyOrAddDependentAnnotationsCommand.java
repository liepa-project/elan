...
/**
 * An undoable command that takes the active annotation as input, and creates new annotations on all the given tiers, using
 * the same start/end time as the active annotation. This is then one undoable command in the undo/redo list.
 *
 * @author Olaf Seibert
 */

public class ModifyOrAddDependentAnnotationsCommand implements UndoableCommand {
    ...
    /**
     * The execute method receives a variable number of arguments that must be a multiple of 4. arguments[4 * i + 0]: tier
     * name arguments[4 * i + 1]: desired annotation text arguments[4 * i + 2]: if not null: extRef arguments[4 * i + 3]: if
     * not null: cveId
     */
    @Override
    ...
    /**
     * Restore the old situation before adding/changing the annotations. This is done by means of the tier names which were
     * remembered, and the previous texts and external references of the annotations that were there. They are null if the

...
/**
 * Creates an input dialog to specify the amount of milliseconds to shift one or more annotations. Performs some checks.
 *
 * @author Han Sloetjes
...
public class ShiftAnnotationsDlgCommand implements Command {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (arguments != null) {
            ...
            if (beforeAnn != null) {
                min = beforeAnn.getEnd().getTime() - firstIn.getBegin().getTime(); // negative value
            } else {
                min = -firstIn.getBegin().getTime();
            }

            if (afterAnn != null) {
                max = afterAnn.getBegin().getTime() - lastIn.getEnd().getTime();
            } else {
            ...
            if (shiftValue != 0) {
                Command com = ELANCommandFactory.createCommand(transcription, ELANCommandFactory.SHIFT_ANNOTATIONS);
                com.execute(transcription, new Object[] {tier, bt, et, Long.valueOf(shiftValue)});
            }
        }
    }

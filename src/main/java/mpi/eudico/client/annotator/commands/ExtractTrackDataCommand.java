...
/**
 * A Command that extracts maximum, minimum or average from a timeseries track, based on intervals/annotations on one tier
 * and stores the information  in annotations on a dependent tier.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
public class ExtractTrackDataCommand implements UndoableCommand {
    ...
    /**
     * The undo action; newly created annotations are deleted, changed annotations are restored to their old state.
     */
    @Override
    public void undo() {
        if (transcription != null) {
            ...
            if (newAnnos != null) {
                for (int i = 0; i < newAnnos.size(); i++) {
                    annotationDataRecord = newAnnos.get(i);
                    ref = (RefAnnotation) tier.getAnnotationAtTime(annotationDataRecord.getBeginTime());

                    if (ref != null) {
                        tier.removeAnnotation(ref);
                    } else {
                        LOG.warning("Could not delete annotation: "
                                    + annotationDataRecord.getValue()
                                    + "bt: "
                                    + annotationDataRecord.getBeginTime());
                    }
                }
            }
            ...
            if (changedAnnos != null) {
                for (int i = 0; i < changedAnnos.size(); i++) {
                    ...
                    if (ref != null) {
                        ...
                        LOG.warning("Could not find annotation: " + valRec.oldValue + "bt: " + valRec.beginTime);
                    }
                }
            }
        ...
        if (transcription != null) {
            ...
            if (newAnnos != null) {
                for (int i = 0; i < newAnnos.size(); i++) {
                    ...
                    if (ref == null) {
                        mid = (annotationDataRecord.getBeginTime() + annotationDataRecord.getEndTime()) / 2;
                        ref = (RefAnnotation) tier.createAnnotation(mid, mid);

                        if (ref != null) {
                            ref.setValue(annotationDataRecord.getValue());
                        } else {
                            LOG.warning("Could not create a reference annotation at time: " + mid);
                        }
                    } else {
            ...
            if (changedAnnos != null) {
                for (int i = 0; i < changedAnnos.size(); i++) {
                    ...
                    if (ref != null) {
                        ...
                        LOG.warning("Could not find annotation: " + valRec.newValue + "bt: " + valRec.beginTime);
                    }
                }
            }
    ...
    /**
     * Extracts values from a time series track based on intervals of annotations on one tier and stores the values in
     * annotations on a dependent tier. <b>Note: </b>it is assumed the types and order of the arguments are correct.
     *
     * @param receiver the transcription
     * @param arguments the arguments: <ul><li>arg[0] the source tier (String)</li> <li>arg[1] the destination tier
     *     (String)</li>
     *     <li>arg[2] the track to extract the data from
     *     (AbstractTSTrack/Object)</li> <li>arg[3] what to extract maximum, minimum, average (String)</li> <li>arg[4]
     *     whether or not to overwrite existing annotation values (Boolean)</li> </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (sourceTier == null) {
            progressInterrupt(ElanLocale.getString("TimeSeriesViewer.Extract.NotFound") + " " + sourceTierName);
        ...
        if (destTier == null) {
            progressInterrupt(ElanLocale.getString("TimeSeriesViewer.Extract.NotFound") + " " + destTierName);
        ...
        if (track == null) {
            progressInterrupt(ElanLocale.getString("TimeSeriesViewer.Extract.NotFound") + " " + "track");
    ...
    /**
     * Class for minimal storage of a time value and an old and new textual value.
     *
    ...
    /**
     * The actual extraction is executed in a separate thread; a listener  with a user interface will be able to update
     * itself (progress bar)
     *
     * @author Han Sloetjes, MPI
     */
    private class ExtractThread extends Thread {
         ...
         * the source tier
         */
        private final TierImpl sourceTier;

        /**
         * the destination tier
         */
        private final TierImpl destTier;

        /**
         * the track
         */
        private final AbstractTSTrack track;
        ...
        ExtractThread(TierImpl sourceTier, TierImpl destTier, AbstractTSTrack track, String method, boolean overwrite) {
            ...
            String lMethod = method.toLowerCase(ElanLocale.getLocale());
        ...
        public void run() {
            ...
            for (int i = 0; i < annos.size(); i++) {
                ...
                try {
                    ...
                    transcription.setNotifying(true);
                    LOG.warning("Exception occured during calculation: " + ex.getMessage());
                    progressInterrupt("Exception occured during calculation: " + ex.getMessage());
                ...
                if (ref == null) {
                    ...
                    changedAnnos.add(new ValueRecord(begin, ref.getValue(), valString));

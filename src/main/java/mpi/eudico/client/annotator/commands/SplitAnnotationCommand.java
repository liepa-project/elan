...
/**
 * Command that splits the active annotation into two annotations if invoked from the context menu, then split is done at the
 * point where its invoked, otherwise it is split equally
 *
 * @author Aarthy Somasundaram
 * @version 2.0 HS August 2015: added two more, optional, arguments; text, which can be different from the original text of
 *     the annotation, and the index where the text should be split into a part for the current annotation and a part for the
 *     new annotation
 */
public class SplitAnnotationCommand implements UndoableCommand {
     ...
     *         <li>arg[3] = the position within the new text where the text should be split (Integer,
     *         optional)</li></ul>
     */
    @Override

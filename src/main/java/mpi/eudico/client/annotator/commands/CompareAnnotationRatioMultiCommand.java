...
/**
 * A command that calculates some kind of agreement value based on the ratio of the amount of overlap of two annotation and
 * the total extent of the two annotations when merged.
 */
public class CompareAnnotationRatioMultiCommand extends AbstractCompareCommand {
    ...
    /**
     * Compares the segmentation of two tiers by calculating the quotient of the time overlap of two annotations and the
     * total extent of the two (merged) annotations. This is similar to the Compare Annotators function that has been part of
     * ELAN for some time, but here only the average for two tiers is calculated and stored (no overview containing all
     * annotations).
     *
     * <p>Version: October 2014
     */
    @Override
    ...
    public void writeResultsAsText(File toFile, String encoding) throws
        ...
        try (BufferedWriter writer = new BufferedWriter(getOutputStreamWriter(encoding, new FileOutputStream(toFile)))) {
            ...
            writer.write(
                "The overlap/extent value is the amount of overlap of two matching annotations divided by the total "
                + "extent of those two annotations.");
            writer.write(NL);
            writer.write(
                "Unmatched annotations (annotations without a counterpart on the other tier) add 1 to the number of "
                + "comparisons, but 0 to the value.");
            writer.write(NL2);
            ...
            if (totalCount > 0) {
                ...
                writer.write("There is no overall average overlap/extent ratio available: no tier combinations found.");
            }
        } catch (Exception ex) {

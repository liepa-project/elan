...
import mpi.eudico.server.corpora.clomimpl.abstr.*;
...
/**
 * A Command to merge two transcriptions and write the result to file. Merging here means that a selected set of tiers with
 * their annotations from one transcription are added to another transcription. If one or more of the tiers already exist in
 * the destination transcription annotations will either be overwritten or preserved, whichever the user chooses. It is
 * assumed that the destination transcription may be altered, that the destination file path has been checked and that, if
 * the file already exists, it may be overwritten.
 */
public class MergeTranscriptionsCommand implements Command {
    ...
    /**
     * Merges two transcriptions. <b>Note: </b>it is assumed the types and order of the arguments are correct.
     *
    ...
    class MergeThread extends Thread {
        ...
        public void run() {
            try {
                ...
                if (copyAndRenameTiers) {
                    tierNameMap = mergeUtil.getRenamingTierMap(srcTrans, destTrans, selTiers);
                ...
                if (numIndivTiers > 0) {
                    ...
                    for (int i = 0; i < tiersToAdd.size(); i++) {
                        ...
                        if (!t.hasParentTier() || !tiersToAdd.contains(t.getParentTier())) {
                            addAnnotations(t);
                            progressUpdate(30 + count * progPerTier, "Merging of tier " + t.getName() + " done.");
                        }
                    }
                }
                ...
                */
                ...
                transcriptionStore.storeTranscription(destTrans, null, new ArrayList<TierImpl>(0), fileName, saveAsType);
        ...
        private List<TSSourceConfiguration> getTSConfigList(List<LinkedFileDescriptor> firstSrcDescriptors,
                                                            List<LinkedFileDescriptor> secondSrcDescriptors) {
        ...
        private void addConfs(List<LinkedFileDescriptor> descriptors, List<String> trackNamesList,
                              List<TSSourceConfiguration> confList) {
            int i = 0;
            ...
            if (parent != null) {
                ...
                if (lt.getConstraints().getStereoType() == Constraint.SYMBOLIC_ASSOCIATION) {
                    ...
                    for (AbstractAnnotation ann : annotations) {
                        List<Annotation> overlapAnn =
                            destTier.getOverlappingAnnotations(ann.getBeginTimeBoundary(), ann.getEndTimeBoundary());
                        if (overlapAnn.size() > 0) {
                            if (overwrite) {
                                ...
                                AnnotationRecreator.createAnnotationFromTree(destTrans, recordNode);
                ...
                for (AbstractAnnotation ann : annotations) {
                    if (overwrite
                        || destTier.getOverlappingAnnotations(ann.getBeginTimeBoundary(), ann.getEndTimeBoundary()).size()
                           == 0) {
                        recordNode = AnnotationRecreator.createTreeForAnnotation(ann);
                        AnnotationRecreator.createAnnotationFromTree(destTrans, recordNode);
                    }
        ...
        /**
         * Copies some preferences from the second file to the destination preferences. This probably should be moved to a
         * utility class.
         *
        ...
        private void mergePrefs(Map<String, Map> destPrefs, Map srcPrefs) {
            ...
            if (srcPrefMap instanceof Map) {
                @SuppressWarnings("unchecked") Map<String, Object> srcCVMap = (Map<String, Object>) srcPrefMap;
                ...
                for (String key : srcCVMap.keySet()) {
                    ...
                    if (!destCVMap.containsKey(key)) {
                    ...
                    } else { // TODO with the merging of transcriptions currently CV's are not merged when they have
                        // the same name
                        ...
                        if (curDestCVObject instanceof Map && curSrcCVObject instanceof Map) {
                            @SuppressWarnings("unchecked") Map<String, Object> curSrcCV =
                                (Map<String, Object>) curSrcCVObject;
                            @SuppressWarnings("unchecked") Map<String, Object> curDestCV =
                                (Map<String, Object>) curDestCVObject;
    ...
    private class TSConfigEncoder extends TSConfigurationEncoder {
         ...
         *
         * @param tsConfigs a collection of configuration objects of all linked timeseries sources
         */
        public void encodeAndSave(Collection<TSSourceConfiguration> tsConfigs) {
            ...
            if (doc != null) {
                ...
                try {
                    IoUtil.writeEncodedFile("UTF-8", configFile.substring(5), docElement);
                    ...
                    for (int i = 0; i < destTrans.getLinkedFileDescriptors().size(); i++) {
                        lfd = destTrans.getLinkedFileDescriptors().get(i);

                        //if (lfd.linkURL.equals(configFile)) {
                        if (lfd.linkURL.toLowerCase(Locale.getDefault()).endsWith(TimeSeriesConstants.CONF_SUFFIX)) {
                            return;
                        }
                    }
        ...
        protected Element createTrackElement(TSTrackConfiguration trConfig) {
            ...
            if (spos != null) {
                ...
                for (int i = 0; i < spos.getRows().length; i++) {
                    ...
                    pos.setAttribute(TimeSeriesConstants.ROW, String.valueOf(spos.getRows()[i]));
                    pos.setAttribute(TimeSeriesConstants.COL, String.valueOf(spos.getColumns()[i]));

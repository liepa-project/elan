...
/**
 * Converts segmentations produced by a recognizer to tiers and annotations. In fact this command receives a map with tier
 * name to annotation records, so it could be used by other operations as well. As long as all annotations are time-aligned
 * and there is no assumption of tier dependencies.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
public class SegmentsToTiersCommand implements UndoableCommand {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (!changeNames) {
            ...
            for (String name : segMap.keySet()) {
                ...
                if (tier != null) {
                    ...
                    while (count < 30) {
                        tier = transcription.getTierWithId(name + "-" + count);
        ...
        LinguisticType lt = transcription.getLinguisticTypeByName("default-lt");
        ...
        if ((lt != null) && (lt.getConstraints() == null)) {
            ...
            if ((lt != null) && (lt.getConstraints() == null)) {
                ...
                while (count < 30) {
                    lt = transcription.getLinguisticTypeByName(ltName + "-" + count);
    ...
    /**
     * Creates a new LinguisticType if needed and then creates new tiers and annotations.
     */
        ...
        for (String name : resolvedMap.keySet()) {
            ...
            if ((segments != null) && (segments.size() > 0)) {
                ...
                for (int i = 0; i < segments.size(); i++) {
                    AnnotationDataRecord annotationDataRecord = segments.get(i);

                    if (annotationDataRecord != null) {
                        AlignableAnnotation aa;
                        aa = (AlignableAnnotation) tier.createAnnotation(annotationDataRecord.getBeginTime(),
                                                                         annotationDataRecord.getEndTime());

                        if ((aa != null) && (annotationDataRecord.getValue() != null)) {
                            aa.setValue(annotationDataRecord.getValue());
                        }
                    }

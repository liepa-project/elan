...
/**
 * A command that creates one or two annotations in a gap on a particular (top level) tier. If an attempt is made to create
 * annotations where there is not a gap, nothing is actually changed (an error message should be shown probably). This
 * command is intended for use within a restricted, simplified turns-and-scene level transcription mode. It ignores, e.g.,
 * the time change propagation mode and several preference settings.
 *
 * @author Han Sloetjes
...
public class NewAnnotationsInGap implements UndoableCommand {
     ...
     *
     * @param receiver the tier the annotations are to be created on (assumed to be a top level tier for now) (TierImpl)
     * @param arguments <ul>
     *     <li>arg[0] = begin time of the total time interval (Long)</li>
     *     <li>arg[1] = end time of the total time interval (Long)</li>
     *     <li>arg[2] = the time at which to split the interval; if outside the range of the first 2
     *     arguments this is ignored (Long)</li>
     *     <li>arg[3] = the text for new annotation(s) (String)</li>
     *     <li>arg[4] = the position at which to split the text in case 2 annotations are made (Integer)
     *     </li>
     *     </ul>
     */
    @Override
    ...
    /**
     * Creates one or two annotations in a gap, a time span not containing annotations. it is assumed that checks have been

...
public class KioskModeCommand implements Command {
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = on or off flag (Boolean)</li> </ul>
     */
    @Override
    ...
    /**
     * A thread that regularly checks if the player is still playing;  when the player has stopped (= has reached the end) it
     * starts the  player again from the beginning.
     */
    private class LoopThread extends Thread implements ClientLogger  {
        /**
         * The actual work
         */
        @Override
        public void run() {
            while (vm.getMediaPlayerController().isPlaySelectionMode()) {
                ...
                try {
                    Thread.sleep(vm.getMediaPlayerController().getUserTimeBetweenLoops());
                } catch (InterruptedException ie) {
                    LOG.log(Level.WARNING, "Thread seems to be interrupted.", ie);
                }
            }

...
/**
 * An undoable command that builds on the create dependent annotation command. It takes the active annotation as input, and
 * creates new annotation recursively on all the dependent tiers This is then one undoable command in the undo/redo list.
 */
public class CreateDependentAnnotationsCommand implements UndoableCommand {
        ...
        if (numberOfannotationsCreated > 0) {
            if (MonitoringLogger.isInitiated()) {
                MonitoringLogger.getLogger(transcription)
                                .log(MonitoringLogger.CREATE_DEPENDING_ANNOTATIONS,
                                     tier.getName(),
                                     tier.getAnnotationAtTime(begin).getValue(),
                                     Long.toString(begin),
                                     Long.toString(end),
    ...
    public void redo() {
        ...
        if (numberOfannotationsCreated > 0) {
            if (MonitoringLogger.isInitiated()) {
                MonitoringLogger.getLogger(transcription)
                                .log(MonitoringLogger.REDO, MonitoringLogger.CREATE_DEPENDING_ANNOTATIONS);
            }
        }
    }
    ...
    public void undo() {
        ...
        if (MonitoringLogger.isInitiated()) {
            MonitoringLogger.getLogger(transcription)
                            .log(MonitoringLogger.UNDO, MonitoringLogger.CREATE_DEPENDING_ANNOTATIONS);
        }

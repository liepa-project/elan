...
public class RegularAnnotationCommand implements UndoableCommand {
    ...
    @Override
    public void undo() {
        if ((transcription == null) || (tiers == null) || (existAnnotationSets == null)) {
            return;
        }
        ...
        for (int tierIdx = 0; tierIdx < tiers.size(); tierIdx++) {
            ...
            if ((existAnn != null) && (existAnn.size() > 0)) {
                AnnotationRecreator.createAnnotationsSequentially(transcription, existAnn, true);
            }
        }
     ...
     * @param arguments the arguments: <ul><li>arg[0] = the start time (long)</li> <li>arg[1] = the end time (long) </li>
     *     <li>arg[2] = the annotations' size (long)</li> <li>arg[3] = the seleted tiers' names (Vector)</li> </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        LOG.info("User given end time is " + end + "ms; computed end time is " + realEnd + "ms");
    ...
    private class RegularAnnotationThread extends Thread {
        /**
         * Creates regular annotation for each selected tier and stores previously existing annotations in an element of
         * existAnnotationSets
         */
        ...
        public void run() {
            final IndeterminateProgressMonitor monitor =
                new IndeterminateProgressMonitor(ELANCommandFactory.getRootFrame(transcription),
                                                 true,
                                                 ElanLocale.getString("RegularAnnotationDialog.Message.Annotating"),
                                                 true,
            ...
            for (int tierIdx = 0; tierIdx < tiers.size(); tierIdx++) {
                ...
                while (annosIt.hasNext()) {
                    ...
                    if (annBT >= start && annET <= realEnd) { // inclusion
                        ...
                        if (!firstRun) {
                            partOverAnnos.add(AnnotationRecreator.createTreeForAnnotation(ann));
                        }
                    }
                }
                ...
                for (long time = start; time < realEnd; time = time + annSize) {
                    Annotation ann = tier.createAnnotation(time, time + annSize);
                }

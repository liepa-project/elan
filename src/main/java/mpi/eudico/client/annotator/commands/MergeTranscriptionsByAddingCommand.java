...
/**
 * This class merges two transcriptions without creating a third one, so the merging should be undoable. Note May 2015: this
 * is all only partially implemented. Several scenario's are not supported yet.
 */
public class MergeTranscriptionsByAddingCommand implements UndoableCommand {
    ...
    /**
     * A command to add the tiers and annotations from one transcription to another. This command does not create a copy of
     * the first transcription, but adds directly to the first transcription. This means that the action must be undoable.
     *
    ...
    /**
     * This method delegates the implementation of undo to the UndoableTranscriptionMerger instance that did the merging.
     */
    @Override
    public void undo() {
        if (merger != null) {
     ...
     *     <li>
     *     arguments[1] = a list of names of tiers to merge/import, can be null in which case all tiers
     *     are processed
     *     <li>

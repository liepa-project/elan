...
/**
 * Import tiers (if necessary with Linguistic Types and ControlledVocabularies) from an eaf or etf into a transcription.
 */
public class ImportTiersCommand implements UndoableCommand {
    ...
    /**
     * Constructor to initialize the command name
     *
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the fileName of an eaf or etf file (String)</li> </ul>
     */
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        List<ControlledVocabulary> currentCvs =
            new ArrayList<ControlledVocabulary>(transcription.getControlledVocabularies());
        ...
        if (fileName.toLowerCase(Locale.getDefault()).endsWith(".etf")) {
            Preferences.importPreferences(srcTrans, fileName.substring(0, fileName.lastIndexOf('.')) + ".pfsx");
        }
    ...
    private void addImportedPreferences() {
        if (tierColorsAdded != null) {
            Map<String, Color> colors = Preferences.getMapOfColor("TierColors", transcription);
            if (colors == null) {
                Preferences.set("TierColors", new HashMap<String, Color>(tierColorsAdded), transcription);
            } else {
        ...
        if (tierHighlightsAdded != null) {
            ...
            if (highlightColors == null) {
                Preferences.set("TierHighlightColors", new HashMap<String, Color>(tierHighlightsAdded), transcription);
            } else {
        ...
        if (tierFontsAdded != null) {
            Map<String, Font> fonts = Preferences.getMapOfFont("TierFonts", transcription);
            if (fonts == null) {
                Preferences.set("TierFonts", new HashMap<String, Font>(tierFontsAdded), transcription);
            } else {
    ...
    /**
     * Checks, loads and applies CV entry preferences and stores the preferences in a map (without applying them to
     * Preferences yet) for undo/redo.
     */
    @SuppressWarnings("unchecked")

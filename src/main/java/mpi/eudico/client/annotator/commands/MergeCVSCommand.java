...
/**
 * A Command to merge two Controlled Vocabularies from two different Transcriptions, i.e. entries present in the second CV
 * but not in the first are copied to the first CV.
 *
 * @author Han Sloetjes
 */
public class MergeCVSCommand implements UndoableCommand {
    ...
    /**
     * Removes the entries that have been added by the merging process. For a complicated merge, that model isn't sufficient,
     * and we just kept a copy of the old vocabulary.
     *
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the first Controlled Vocabulary (Controlled Vocabulary)</li>
     *     <li>arg[1] = the second Controlled Vocabulary (Controlled Vocabulary)</li> </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        if ((transcription.getControlledVocabulary(conVoc.getName()) != null) && (secConVoc != null)) {

            // Simple and common case:
            // both vocabularies have one language, and it is the same one.
            boolean simple;
            simple =
                conVoc.getNumberOfLanguages() == 1 && secConVoc.getNumberOfLanguages() == 1 && conVoc.getLongLanguageId(0)
                                                                                                     .equals(secConVoc.getLongLanguageId(
                                                                                                         0));
    ...
    /**
     * This function does a non-trivial merge between two vocabularies, making a new one. The new one has the merge of the
     * languages of the inputs. It has the name of the first input.
     *
     * @param one first CV
     * @param two second CV
     */
        ...
        for (int i = 0; i < nLangs2; i++) {
            ...
            if (langCache.containsKey(longId)) {
                ...
                while (one.getIndexOfLanguage(id) >= 0) {
                    id = RandomNumberGenerator.getInstance().postFixToMakeUnique(id, 10000);
                }
                int index = dest.addLanguage(id, longId, label);
    ...
    /**
     * Make a quick mapping from language index values in {@code src} to corresponding index values in {@code dest} instead

...
/**
 * Copies values of currently active annotation and every simultaneous annotations to next adjacent annotations. Sets the
 * annotation next to currently active  as the new active annotation. The copying takes place if and only if a simultaneous
 * annotation matches the time boundary of current annotation and next annotation is adjacent and of same length.
 *
...
public class CopyPreviousAnnotationCommand implements UndoableCommand {
    ...
    private final ArrayList<AnnotationValuesRecord> records = new ArrayList<AnnotationValuesRecord>();
    ...
    public void undo() {
        if (transcription != null) {
            ...
            AnnotationValuesRecord annotationValuesRecord;
            TierImpl tier;
            Annotation ann;

            while (annIt.hasNext()) {
                annotationValuesRecord = annIt.next();
                tier = transcription.getTierWithId(annotationValuesRecord.getTierName());
                if (tier != null) {
    ...
    /**
     * Copies values of annotations as current time to next adjacent and sets new active annotation
     *
     * @param receiver the viewer manager
     * @param arguments arg[0] = new active annotation arg[1] = currently active annotation arg[2] = annotations
     *     simultaneous to currently active
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        if ((curEndTime == newBeginTime) && (curLength == (newEndTime - newBeginTime))) {
            ...
            if (newActiveAnnot.getValue().trim().length() < 1) {
                AnnotationValuesRecord annotationValuesRecord = new AnnotationValuesRecord(newActiveAnnot);
                annotationValuesRecord.setNewLabelValue(currentActiveAnnot.getValue());
    ...
    /**
     * Copies annotation to next adjacent annotation on same tier, if its time boundaries match arguments and  if its length
     * is equal to the length of next adjacent annotation
     *
    ...
    private void copyToNextAnnotation(Annotation ann, long endTime, long length) {
        if (ann != null) {
            ...
            if (nextAnn != null) {
                ...
                if (((annET - annBT) == length) && ((nextET - nextBT) == length) && (annET == nextBT)) {
                    //Copy value to next annotation if next annotation has no value
                    if (nextAnn.getValue().trim().length() < 1) {
                        AnnotationValuesRecord annotationValuesRecord = new AnnotationValuesRecord(nextAnn);
                        annotationValuesRecord.setNewLabelValue(ann.getValue());
                        records.add(annotationValuesRecord);

...
/**
 * Undoable Command that merges 2 tiers with depending tiers, if possible. It creates new annotations on a third tier group.
 * Overlapping annotations are not merged, instead annotations of the second tier are create on top of those of the first
 * tier. Non overlapping annotations are copied as is. The mapping of tier names in the second group to tier names in the
 * first, is based on prefix or suffix.
 *
 * @author Han Sloetjes
...
public class MergeTierGroupCommand implements UndoableCommand {
    ...
    /**
     * Oct 2009: creates a new tier group based on tier 1 plus depending tiers. Copies annotations form tier 1 onto the new
     * group and then copies the annotations of tier 2 to the new group (whereby existing annotations can be changed).
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (destTier != null) {
            ...
            LOG.warning("Tier " + (sourceTierName1 + copySuffix) + " already exists, changed name to " + (sourceTierName1
                                                                                                          + cName));
            copySuffix = cName;
        }
            ...
            for (int i = 0; i < tierRecords.size(); i++) {
                ...
                if (transcription.getTierWithId(tr.getName()) == null) {
                    if (tr.getParentTier() != null) {
                        ...
                        if (pt == null) {
                            // error situation
                            LOG.severe("Cannot recreate tier: " + tr.getName() + " because the parent tier was not found");
                            continue;
                        }
                    }
                    lt = transcription.getLinguisticTypeByName(tr.getLinguisticType());
                    if (lt == null) {
                        LOG.severe("Cannot recreate tier: " + tr.getName() + " because the linguistic type was not found");
                        continue;
                    }
    ...
    /**
     * A thread class that compares the annotations of two tiers, detects the ovelaps and  creates new annotations on a third
     * tier.
     */
    private class MergeThread extends Thread {
        ...
        public void run() {
            ...
            List<AlignableAnnotation> sourceAnns1 =
                new ArrayList<AlignableAnnotation>(sourceTier1.getAlignableAnnotations());
            int numAnns1 = sourceAnns1.size();
            List<AlignableAnnotation> sourceAnns2 =
                new ArrayList<AlignableAnnotation>(sourceTier2.getAlignableAnnotations());

...
/**
 * CommandAction for copying values of annotations at current time to next adjacent annotations
 */
...
public class CopyToNextAnnotationCA extends CommandAction {
    ...
    public CopyToNextAnnotationCA(ViewerManager2 theVM) {
        ...
        //con = new ImageIcon(this.getClass().getResource
        // ("/mpi/eudico/client/annotator/resources/CopyToNextAnnotation.gif"));
    ...
    protected void newCommand() {
        ...
        if (currentAnnot != null) {
            command = ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.COPY_TO_NEXT_ANNOTATION);
        }
    ...
    /**
     * Retrieves current active annotation and simultaneous annotations on other tiers.
     *
     * @return Array containing next annotation, current annotation and simultaneous annotations
     */
    @Override
    ...
    /**
     * Returns the current active annotation and the next active annotation. If active annotation not found, returns an array
     * of null.
     *
     * @return the current active annotation and it's next adjacent annotation (if both exist)
     */
    private Annotation[] retrieveAnnotations() {
        ...
        if (currentActiveAnnot != null) {
            ...
            if (activeTier != null) {
                currentActiveAnnot = activeTier.getAnnotationAtTime(vm.getMasterMediaPlayer().getMediaTime());
    ...
    /**
     * Returns annotations on other top level tiers that are simultaneous with the current active annotation
     *
     * <p>Note: check if this is what is intended: probably better to check on begin
     * and end time, or even for symbolically associated annotations?? hier...
     *
     * @param currentActiveAnnot active annotation
     *
     * @return annotations on other tiers at the position of the active annotation begin time
     */
    private List<Annotation> getSimultaneousAnnotations(Annotation currentActiveAnnot) {
        ...
        if (currentActiveAnnot != null) {
            ...
            while (tiersIter.hasNext()) {
                ...
                Annotation ann = tier.getAnnotationAtTime(vm.getMasterMediaPlayer().getMediaTime());

...
/**
 * A command that calculates a value indicating the Degree of Organization of the segmentation of two tiers. The calculation
 * is based on / performed by the Staccato software. Reference:
 * {@code Luecking, A., Ptock, S., & Bergmann, K. (2011). Staccato: Segmentation Agreement Calculator according to Thomann.
 * In E. Efthimiou G. & Kouroupetroglou (Eds.), Proceedings of the 9th International Gesture Workshop: Gestures in Embodied
 * Communication and Human-Computer Interaction (pp. 50-53).
 *
 * <p>Thomann, B.: Observation and judgment in psychology: Assessing agreement among markings of behavioral events.
...
public class CompareAnnotationStaccatoCommand extends AbstractCompareCommand {
     ...
     * <p>{@link AbstractCompareCommand#execute(Object, Object[])}
     */
    @Override
    ...
    /**
     * Based on the tier combinations created bin the super class based on user input, Staccato objects are produced and the
     * StaccatoAnalyzer is called to do the actual calculations.
     */
    @Override
    ...
    /**
     * Creates a StaccatoData object containing the StaccatoNominations (equivalent to annotations) and the parameters for
     * the calculation (based on user input).
     *
    ...
    private StaccatoData createStaccatoData(CompareCombi cc) {
        ...
        long maxT = 0;
        Vector<StaccatoNomination> inputNominations = new Vector<>();

        for (AnnotationCore ac : cc.getFirstUnit().annotations) {
            // int start, int end, int id, String label, String annotator
            StaccatoNomination sn = new StaccatoNomination((int) ac.getBeginTimeBoundary(),
                                                           (int) ac.getEndTimeBoundary(),
                                                           idx++,
                                                           ac.getValue(),
                                                           cc.getFirstUnit().annotator);
    ...
    /**
     * Writes the results to a text file. Per tier pair some information of the files and the tiers is exported and the
     * information provided by Staccato: Degree of Organization, Fields, NucleusNominations, Nuclei. An average DO is
    ...
    public void writeResultsAsText(File toFile, String encoding) throws
        ...
        try (BufferedWriter writer = new BufferedWriter(getOutputStreamWriter(encoding, new FileOutputStream(toFile)))) {
            ...
            if (totalCount > 0) {
                ...
                writer.write("There is no overall average Degree of Organization available: no tier combinations found.");
            }
        } catch (Exception ex) {
     ...
     *
     * <p>While running the event reports progress from 0 to 100, an error is -1,
     * finished is 101.
     */
    private static class StacProcListener implements StaccatoListener {
        int lastEvent;

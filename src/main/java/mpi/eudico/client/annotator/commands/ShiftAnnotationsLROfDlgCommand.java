...
/**
 * Creates a dialog to specify how much the annotations on all tiers left or right from the crosshair should be shifted.
 *
 * @author Han Sloetjes
        ...
        if (arguments != null) {
            ...
            for (int i = 0; i < rootTiers.size(); i++) {
                ...
                while (annIt.hasNext()) {
                    ...
                    if ((ann.getEnd().getTime() <= bt) || ((ann.getBegin().getTime() < bt) && (ann.getEnd().getTime()
                                                                                               < et))) {
                        beforeAnn = ann;
                    ...
                    if ((ann.getBegin().getTime() <= bt && ann.getEnd().getTime() > et) || (ann.getBegin().getTime() < bt
                                                                                            && ann.getEnd().getTime()
                                                                                               >= et)) {
                        break;
                    }

                    if ((ann.getBegin().getTime() >= et) || ((ann.getBegin().getTime() > bt) && (ann.getEnd().getTime()
                                                                                                 > et))) {
                        afterAnn = ann;
                ...
                if (beforeAnn != null) {
                    min = Math.max(min, beforeAnn.getEnd().getTime() - firstIn.getBegin().getTime()); // negative value
                } else {
                    min = Math.max(min, -firstIn.getBegin().getTime());
                }

                if (afterAnn != null) {
                    max = Math.min(max, afterAnn.getBegin().getTime() - lastIn.getEnd().getTime());
                } else {
            ...
            if (shiftValue != 0) {
                Command com = ELANCommandFactory.createCommand(transcription, ELANCommandFactory.SHIFT_ALL_ANNOTATIONS_LROf);
                com.execute(transcription, new Object[] {bt, et, Long.valueOf(shiftValue)});
            }
        }
    }

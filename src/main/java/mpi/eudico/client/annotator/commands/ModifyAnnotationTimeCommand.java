...
public class ModifyAnnotationTimeCommand implements UndoableCommand {
    ...
    /**
     * note: this presumes that tier object references stay the same even if they have been removed from and re-added to the
     * transcription
     */
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the new begin time of the annotation (Long)</li> <li>arg[1] =
     *     the new end time of the annotation (Long)</li> </ul>
     */
    @Override
        ...
        if (annotation != null) {
            ...
            if (MonitoringLogger.isInitiated()) {
                MonitoringLogger.getLogger(transcription)
                                .log(MonitoringLogger.REDO, MonitoringLogger.CHANGE_ANNOTATION_TIME);
            }
        ...
        if (leftOffset < 0) {
            ...
            if (v.size() > 0) {
                ...
                if (ann.getBeginTimeBoundary() < newBeginTime) {
                    ...
                    changedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(ann));
                } else {
                    removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(ann));
                }

                for (int i = 1; i < v.size(); i++) {
                    removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation((AbstractAnnotation) v.get(i)));
                }
            }
        }
    ...
    private void restoreNormal() {
        ...
        if (rootTier != null) {
            ...
            if (aa != null) {
                ...
                AnnotationRecreator.createAnnotationFromTree(transcription, rootNode, true);
            }
        } else {
    ...
    /**
     * Restores information of all effected annotations in bulldozer time propagation mode. First restore the annotation that
     * was originally changed by the user,  next shift the annotations to the left and to the right back to their original
     * locations.
     */
    ...
    /**
     * Stores information on all effected annotations in shift propagation mode. On a time subdivision tier shift mode is
     * identical to normal mode. On root tiers the changes on the left side are treated the same way as in normal mode,
     * changes on the right side shift all time slots on  the right side of the selection to the right.
     */
    ...
    /**
     * Restores information of all effected annotations in shift time propagation mode. On root tiers first delete all
     * changed annotations, then  shift the time slots back and finally recreate/restore any removed annotations.
     */

...
/**
 * A command that creates annotation documents for all media files in a folder and optionally its sub-folders.
 *
 * @author Han Sloetjes
 */
public class CreateTranscriptionsCommand implements Command,
                                                    ProcessReporter {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (sourceFol == null || sourceFol.isEmpty()) {
     ...
     * @param combineVideos if true video files will be matched on the basis of prefix or suffix. E.g. AAA_1 and AAA_2
     *     might be in one and the same transcription. If false for each video a transcription file will be made.
     * @param prefixBased if true videos will be combined on the basis of different prefixes, otherwise on the basis of
     *     suffixes. This parameter is only considered if combineVideos is true
     */
    private void createTranscriptions(File source, String templatePath, String destinationPath, Boolean recursive,
                                      Boolean combineVideos, Boolean prefixBased) {
        ...
        if (templatePath != null && (templatePath.endsWith(".etf"))) {
        ...
        List<File> waveList = new ArrayList<>();
        List<String> exts = new ArrayList<>(5);
        exts.add(".txt");
        exts.add(".eaf");
        exts.add(".etf");
        exts.add(".pfsx");
        ...
        for (File f : files) {
             ...
             * <b>Description:</b> Usually the file names are case sensitive on some platforms.
             * So, why are we converting the case here ?
             * toLowerCase and toUpperCase can be searched to find more places which do this conversion.
             * <br />
             * Han <br />
             * The lowercase path is only used to filter out files with certain extensions, which are
             * defined above in lowercase. So, to lowercase is fine here.
             */
            String path = f.getName().toLowerCase(Locale.getDefault());
            if (path.startsWith(".")) {
        ...
        for (File f : fileList) {
            ...
            if (index > -1) {
                ...
                if (!waveList.contains(f)) {
                    ...
                    List<MediaDescriptor> mds = new ArrayList<>(6);
            ...
            if (saveToSave) {
                ...
                try {
                    ...
                    if (templatePrefsPath != null) {
                        ...
                        try {
                            // note: this leads to a mismatch in case the template has a different EAF format as the new
                            // transcription
    ...
    /**
     * Tries to find matching video files for the given first video file. Based on built in delimiters "-" and "_" or on a
     * character specified by the user.
     *
     * @param fileList the list of remaining video files
     * @param first the first video file
     * @param prefixBased if true the comparison is done on the basis of different prefixes, otherwise on different
     *     suffixes
     *
            ...
            if (prefixBased) {
                ...
                for (Character ch : delimiters) {
                    ...
                    if (hindex > -1 && hindex2 > -1) {
                        ...
                        if (!sub1.isEmpty() && !sub2.isEmpty() && sub1.equals(sub2)) {
                            if (result == null) {
    ...
    /**
     * Copies the preferences file of a template to the destination folder, one for each created eaf file.
     *
     ...
     */
    private void copyPreferences(String fromPath, String toPath) throws
                                                                 IOException,
                                                                 Exception {
        if (fromPath == null) {

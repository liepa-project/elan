...
/**
 * Iterates over top level tiers and shifts annotations within specified interval taking into account limits based on
 * existing annotations.
 *
 * @version Mar 2018 no longer uses one, overall available empty interval for all tiers anymore, but instead an interval per
 *     root tier, to prevent errors in the undo action
...
public class ShiftAnnotationsLROfCommand extends ShiftAnnotationsCommand {
     ...
     * @param arguments the arguments: <ul><li>arg[0] = the begin of the time interval the annotations in which are  to
     *     be shifted (Long)</li> <li>arg[1] = the end time of that interval (Long)</li> <li>arg[2] = the shift value
     *     (Long)</li> </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (arguments != null) {
            ...
            for (int i = 0; i < rootTiers.size(); i++) {
                ...
                spacePerRootTier.put(tier.getName(),
                                     new TimeInterval(getBoundaryBefore(tier, bt), getBoundaryAfter(tier, et)));
            }
    ...
    /**
     * @return if there is an overlapping annotation at the specified time return the end time of the annotation before the
     *     overlapping annotation, or 0 if there is none
     */
    @Override
    ...
    /**
     * @return if there is an overlapping annotation at the specified time return the begin time of the annotation after the
     *     overlapping annotation, or {@code Long.MAX_VALUE} if there is none
     */
    @Override

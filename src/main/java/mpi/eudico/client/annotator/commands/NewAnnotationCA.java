 ...
 * depending on selection, active annotation and active tier. Since there is no listener mechanism for active tier, this
 * action is no longer supposed to be visible as a menu item and no longer checks whether it should be enabled or disabled.
 * It simply checks conditions every time it is called and creates a command when appropriate. This Action is mainly intended
 * to provide a key shortcut for the creation of a new annotation.
 *
 * @author Han Sloetjes
 */
...
public class NewAnnotationCA extends CommandAction {
    ...
    /**
     * Before just creating a command check if it is possible to create a new annotation and if so, on which tier. If
     * receiver is <code>null</code> no command is created (since the command should be undoable we don't want to check in
     * the command itself).
     */
    @Override
        ...
        if (checkState()) {
            ...
            if (receiver != null) {
                ...
                if (val != null) {
                    if (val.booleanValue()) {
                        command =
                            ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.NEW_ANNOTATION_REC);
    ...
    /**
     * The receiver of this CommandAction is a TierImpl object on which the new annotation should be created. This can be
     * either the active tier or the tier the active annotation is on.
     *
    ...
    /**
     * First try to create an annotation on the active tier. When that is not possible check if it is possible to create an
     * annotation on a child tier of the tier the active annotation is on.
     *
     * @return {@code true} if it is possible to create a new annotation, {@code false} otherwise
     */
    protected boolean checkState() {
        ...
        if (vm.getMultiTierControlPanel() != null) {
            if (vm.getMultiTierControlPanel().getActiveTier() != null) {
                receiver = (TierImpl) vm.getMultiTierControlPanel().getActiveTier();
                ...
                if (vm.getSelection().getBeginTime() != vm.getSelection().getEndTime()) {
                    ...
                    if ((vm.getActiveAnnotation() != null) && (vm.getActiveAnnotation().getAnnotation() != null)) {
                        ...
                        if (depTiers == null || depTiers.isEmpty() || !depTiers.contains(receiver)) {
                            return false;
                        }
                        ...
                        if ((con != null) && ((con.getStereoType() == Constraint.SYMBOLIC_ASSOCIATION)
                                              || (con.getStereoType() == Constraint.SYMBOLIC_SUBDIVISION))) {
                            long time = (activeAnn.getBeginTimeBoundary() + activeAnn.getEndTimeBoundary()) / 2;
        ...
        /*
           ...
           while (tierIt.hasNext()) {
               ...
               if (con == null) {
                   ...
                   if (con.getStereoType() == Constraint.SYMBOLIC_ASSOCIATION || con.getStereoType() == Constraint

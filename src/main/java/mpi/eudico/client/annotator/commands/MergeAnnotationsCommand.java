...
/**
 * An undoable command that merges two adjacent annotations on a top level tier. Depending annotations are also merged, it
 * depends on the type of tier how annotations are added.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
public class MergeAnnotationsCommand implements UndoableCommand {
    ...
    /**
     * In the simple case revert the changes to the first annotation and recreate the second. In the complex case delete the
     * modified first annotation and recreate both original annotations.
     */
    @Override
    public void undo() {
        int curPropMode = 0;
        ...
        if (simpleMerge) {
            ...
            if (tier != null) {
                ...
                if (aa != null) {
                    aa.updateTimeInterval(aa.getBeginTimeBoundary(), activeAnnRecord.getEndTime());
                    aa.setValue(activeAnnRecord.getValue());
                    aa.setCVEntryId(activeAnnRecord.getCvEntryId());
                }

                AlignableAnnotation aa2 =
                    (AlignableAnnotation) tier.createAnnotation(mergeAnnRecord.getBeginTime(), mergeAnnRecord.getEndTime());

                if (aa2 != null) {
                    AnnotationRecreator.restoreValueEtc(aa2, mergeAnnRecord, true);
                }
            } else {
                LOG.warning("Cannot undo annotation merge: tier not found");
            }
        } else {
            // complex
            ((TranscriptionImpl) transcription).setNotifying(false);
            ...
            if (rootTier != null) {
                ...
                AnnotationRecreator.createAnnotationFromTree(transcription, activeNode, true);
                AnnotationRecreator.createAnnotationFromTree(transcription, mergeAnnNode, true);
            }
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the active annotation (Annotation)</li>
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if ((arguments.length > 0) && arguments[0] instanceof AlignableAnnotation aa) {
            ...
            if (mergeAnn != null && aa != null) {
                if ((aa.getParentListeners().size() == 0) && (mergeAnn.getParentListeners().size() == 0)) {
                    ...
                    mergeAnnNodeComp =
                        AnnotationRecreator.createTreeForAnnotation(mergeAnn); // modified for symbolic subdivisions
                    recalculateSymbolicSubdivisions(activeNode,
                                                    mergeAnnNodeComp,
    ...
    /**
     * Merge operation with child annotations. First the first root annotation's time interval is updated,  then the
     * information of the next annotation tree is processed; either new annotations are created (subdivision and included in)
     * or the values of existing annotations are updated (root and sym associated children of the root).
     */
                    ...
                    if (tier.isTimeAlignable()) {
                        ...
                        if (linStereoType == Constraint.SYMBOLIC_SUBDIVISION) {
                            ...
                            if (node.getParent() == root) {
                                ...
                                if (tier.getParentTier().getLinguisticType().getConstraints().getStereoType()
                                    == Constraint.SYMBOLIC_ASSOCIATION) {
                                    begin =
                                        ((AnnotationDataRecord) ((DefaultMutableTreeNode) node.getParent()).getUserObject()).getEndTime()
                                        - 1;
                                }
                            }
    ...
    /**
     * Recalculates virtual begin and end times of sym. subdivision annotations of the second node, based on the (new) total
     * of subdivision annotations and the new interval. Recursive.
     *
     ...
     */
    private void recalculateSymbolicSubdivisions(DefaultMutableTreeNode node1, DefaultMutableTreeNode node2, long begin,
                                                 long end) {
    ...
    private void checkChildNodes(DefaultMutableTreeNode node2, Map<String, MutableInt> changedTiers,
                                 boolean isInSecondNode) {
        DefaultMutableTreeNode node = null;
        AnnotationDataRecord annData = null;
        TierImpl tier = null;
    ...
    /**
     * Updates child annotations of symbolic subdivision annotations, both symbolic association and symbolic subdivisions.
     *

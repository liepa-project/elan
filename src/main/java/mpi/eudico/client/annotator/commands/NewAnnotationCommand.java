...
/**
 * A Command for the creation of new annotations on a tier.<br> Should become undoable eventually. Because existing
 * annotations  can be destroyed when creating a new annotation undo /redo is not  yet implemented.
 *
 * @author Han Sloetjes
 */
public class NewAnnotationCommand implements UndoableCommand {
    ...
    long end;
    /**
     * new annotation begin time
     */
    protected long newAnnBegin;
    /**
     * new annotation end time
     */
        ...
        if ((tier != null) && (newAnnotation != null)) {
            ...
            if (aa != null) {
                ...
                if (vm.getActiveAnnotation().getAnnotation() == aa) {
                    ...
                    Command c = ELANCommandFactory.createCommand(transcription, ELANCommandFactory.ACTIVE_ANNOTATION);
     ...
     * correct.<br> July 2006: removed the ViewerManager as one of the objects in the arguments array
     *
     * @param receiver the TierImpl
     * @param arguments the arguments:  <ul> <li>arg[0] = the begin time of the annotation (Long)</li> <li>arg[1] = the
     *     end time of the annotation (Long)</li>
        ...
        if (boolPref != null) {
            if (boolPref.booleanValue()) {
                if (vm.getMediaPlayerController().getSelectionMode()) {
                    vm.getSelection()
                      .setSelection(vm.getMediaPlayerController().getMediaTime(),
    ...
    /**
     * A method for storing information for the undo operation. Can be called or overridden by sub-classes.
     */
    ...
    /**
     * A method for restoring the original situation in the undo operation. Can be called or overridden by subclasses.
     */
    ...
    void newAnnotation() {
        ...
        if (newAnnotation != null) {
            ...
            if (MonitoringLogger.isInitiated()) {
                MonitoringLogger.getLogger(transcription)
                                .log(MonitoringLogger.NEW_ANNOTATION,
                                     tier.getName(),
                                     Long.toString(begin),
                                     Long.toString(end));
            }
            ...
            AlignableAnnotation aa = (AlignableAnnotation) tier.getParentTier().getAnnotationAtTime((begin + end) / 2);
    ...
    /**
     * Returns the start time of the video frame at the specified media time.
     *
    ...
    /**
     * Returns the end time of the video frame at the specified media time.
     *
    ...
    /**
     * Stores information of all effected annotations in normal time propagation mode. Assumption: no annotations on
     * parenttiers will be effected.
     */
    private void storeNormal() {
        if (rootTier != null) {
            List<Annotation> possiblyEffectedAnn = rootTier.getOverlappingAnnotations(begin, end);
            ...
            for (int i = 0; i < possiblyEffectedAnn.size(); i++) {
                ...
                changedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(aa));
            }
        } else {
            ...
            for (int i = 0; i < effectedAnn.size(); i++) {
                ...
                if (aa.getBeginTimeBoundary() < begin) {
                    ...
                    removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(aa));
                }
            }
        }
    }
    ...
    private void restoreNormal() {
        ...
        if (rootTier != null) {
            ...
            for (int i = 0; i < changedAnnotations.size(); i++) {
                ...
                if ((annRecord.getBeginTime() <= mid) && (annRecord.getEndTime() >= mid)) {
                    break;
                }
            }
    ...
    /**
     * Stores information of all effected annotations in bulldozer time propagation mode. Assumption: no annotations on
     * parent tiers will be effected.<br> On root tiers: <br> if the selection begin is within the boundaries of an existing
     * annotation this annotation will be shifted to the left; all other overlapped  annotations will be shifted to the
     * right. Information on effected  annotations is stored in this order: first the annotation that is shifted  to the
     * left, then all other annotations on the left side that will be  bulldozered (descending), then all annotations that
    ...
    private void storeBulldozer() {
        if (rootTier != null) {
            ...
            if (affectedAnn.size() > 0) {
                ...
                if ((aa.getBeginTimeBoundary() <= begin) && (aa.getEndTimeBoundary() > begin)) {
                ...
                if (leftOffset < 0) {
                    ...
                    changedAnnotations.add(new TimeShiftRecord(cur.getBeginTimeBoundary(),
                                                               cur.getEndTimeBoundary(),
                                                               leftOffset));
                    removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(cur));

                    for (int i = index - 1; i >= 0; i--) {
                        prev = allAnn.get(i);

                        int dist = (int) (cur.getBeginTimeBoundary() - prev.getEndTimeBoundary());
                        gapBridged += dist;

                        if (gapBridged < gapToBridge) {
                            changedAnnotations.add(new TimeShiftRecord(prev.getBeginTimeBoundary(),
                                                                       prev.getEndTimeBoundary(),
                                                                       -(gapToBridge - gapBridged)));
                            removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(prev));
                            cur = prev;
                        } else {
                            break;
                        }
                ...
                if (rightOffset > 0) {
                    ...
                    for (int i = index + 1; i < allAnn.size(); i++) {
                        ...
                        int dist = (int) (next.getBeginTimeBoundary() - cur.getEndTimeBoundary());
                        gapBridged += dist;

                        if (gapBridged < gapToBridge) {
                            changedAnnotations.add(new TimeShiftRecord(next.getBeginTimeBoundary(),
                                                                       next.getEndTimeBoundary(),
                                                                       gapToBridge - gapBridged));
                            removedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(next));
                            cur = next;
                        } else {
                            break;
                        }
    ...
    /**
     * Restores information of all effected annotations in bulldozer time propagation mode. First deletes the annotation that
     * was originally created by the user,  next deletes the shifted annotations and then recreates the original
     * annotations.
     */
    ...
    /**
     * Stores information on all effected annotations in shift propagation mode.
     */
    ...
    /**
     * Restores information of all effected annotations in shift time- propagation mode.
     */

...
public class SplitAnnotationCA extends CommandAction implements ActiveAnnotationListener {
    ...
    protected void newCommand() {
        command = ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.SPLIT_ANNOTATION);
    }
    ...
    private List<Annotation> getDependingAnnotation(TierImpl tier) {
        ...
        for (int i = 0; i < dependentTiers.size(); i++) {
            if (dependentTiers.get(i).getLinguisticType().getConstraints().getStereoType()
            ...
            if (tier.isTimeAlignable() && !tier.hasParentTier()) {
                ...
                if (childTiers != null) {
                    ...
                    for (int i = 0; i < childTiers.size(); i++) {
                        ...
                        if (childTier.getLinguisticType().getConstraints().getStereoType()
                            != Constraint.SYMBOLIC_ASSOCIATION) {
                            break;
                        }
                        valid = true;

...
/**
 * Stores the command history, for undo and redo actions. Executed commands are added at the end of the history. When
 * executing an undo, a pointer is moved back from the end, indicating which command can now be re-done, and which is the
 * next command to undo.
 *
 * <p>Once some commands are undone, and a new command is executed, all remaining
...
public class CommandHistory {
    ...
    private void adjustCurrentCommand() {
        if (nextCommand > 0) {
            ...
            undoString += " ";
            undoString += ElanLocale.getString((history.get(nextCommand - 1)).getName());

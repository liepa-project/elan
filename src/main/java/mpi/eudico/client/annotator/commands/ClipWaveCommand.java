...
/**
 * A command to create an audio clip from a segment of a wave file using javax.sound.sampled classes.
 *
 * @author Han Sloetjes
 */
public class ClipWaveCommand implements Command {
    ...
    /**
     * Creates a file path for the clip, either by prompting the user or by constructing it from the source name and the
     * begin and end time.
     *
    ...
    private String getOutputFile(String sourcePath, long begin, long end) {
        ...
        Boolean promptObj = Preferences.getBool("Media.PromptForFilename", transcription);
        if (promptObj != null) {
    ...
    private void clipFile(String inputFile, String outputFile, long beginTime, long endTime) {
        ...
        try {
            boolean clipSuccess = clipper.exportClip(inputFile, outputFile, beginTime * 1000, endTime * 1000);
            ...
            if (clipSuccess) {
                ...
                // warning
                showMessage(ElanLocale.getString("ClipMedia.Error.Message.Unknown"), JOptionPane.ERROR_MESSAGE);
            }
        } catch (IOException ioe) {
            showMessage(ElanLocale.getString("ClipMedia.Error.Message.IOError"), JOptionPane.ERROR_MESSAGE);
        } catch (UnsupportedAudioFileException uafe) {
            showMessage(ElanLocale.getString("ClipMedia.Error.Message.NotSupported"), JOptionPane.ERROR_MESSAGE);
        } catch (Throwable t) {
            showMessage("An unknown error occured while creating the audio clip", JOptionPane.ERROR_MESSAGE);
        }
    }


    private void showMessage(String message, int type) {
        JOptionPane.showMessageDialog(ELANCommandFactory.getRootFrame(transcription), message, "", type);
    }

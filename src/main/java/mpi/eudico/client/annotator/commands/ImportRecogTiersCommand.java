...
public class ImportRecogTiersCommand implements Command {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (f.exists() && f.canRead()) {
            ...
            if (segm != null && segm.size() > 0) {
                Command cc = ELANCommandFactory.createCommand(trans, ELANCommandFactory.SEGMENTS_2_TIER_DLG);
                cc.execute(trans, new Object[] {segm});
            } else {
                // warning no tiers
                JOptionPane.showMessageDialog(ELANCommandFactory.getRootFrame(trans),
    ...
    /**
     * Prompts the user to specify a location from where to load tiers. The tiers can be in a csv file or in an xml file
     *

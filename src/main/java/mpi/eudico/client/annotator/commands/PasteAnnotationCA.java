...
/**
 * A CommandAction to paste an annotation (i.e. a transferable AnnotationDataRecord) from the System's Clipboard to a
 * receiving transcription.
 *
 * <p>Registration as MenuListener to the Menu for en/disabling the action is not sufficient since there
 * is an acceleration key. What is needed is to register a Clipboard listener, which is possible in j2se 1.5 or higher.
 */
public class PasteAnnotationCA extends CommandAction {
    ...
    protected void newCommand() {
        command = ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.PASTE_ANNOTATION);
    }
    ...
    /**
     * Don't create a command if there is no access to the clipboard. Or if there is no or unusable contents on the
     * clipboard.
     *

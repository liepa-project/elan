...
public class ExportHTMLDlgCommand implements Command {
     ...
     *     Selection object (Selection)</li> </ul>
     */
    @Override
    public void execute(Object receiver, Object[] arguments) {
        TranscriptionImpl transcription = (TranscriptionImpl) arguments[0];
        Selection selection = (Selection) arguments[1];
        Interlinear inter = new Interlinear(transcription, Interlinear.HTML);

        if ((selection != null) && (selection.getBeginTime() != selection.getEndTime())) {
            inter.setSelection(new long[] {selection.getBeginTime(), selection.getEndTime()});
        }

        new InterlinearPreviewDlg(ELANCommandFactory.getRootFrame(transcription), true, inter).setVisible(true);
    }

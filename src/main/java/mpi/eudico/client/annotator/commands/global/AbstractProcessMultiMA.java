...
/**
 * Base class for processing multiple eaf files. This part deals with selecting multiple files and directories and creating a
 * list of unique, existing files.
 *
 * @author Han Sloetjes
 * @version 1.0
 */
...
public abstract class AbstractProcessMultiMA extends FrameMenuAction {
    ...
    /**
     * Shows a multiple file chooser to select multiple eaf files and or folders.
     *
    ...
    protected List<File> getMultipleFiles(JFrame parent, String title) {
        ...
        // "new domain prompt"
        MFDomainDialog mfDialog = new MFDomainDialog(parent, ElanLocale.getString("ExportDialog.Multi"), true);
    ...
    protected void addFiles(File dir, List<File> files) {
        ...
        if (allSubs != null) {
            for (File sub : allSubs) {
                if (sub.canRead()) {
                    if (sub.isDirectory()) {
                        addFiles(sub, files);
                    } else if (sub.getName().toLowerCase().endsWith(FileExtension.EAF_EXT[0])) {

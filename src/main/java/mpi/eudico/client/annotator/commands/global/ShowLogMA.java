...
public class ShowLogMA extends FrameMenuAction {
        ...
        try {
            ...
            while (en.hasMoreElements()) {
                ...
                for (int i = 0; i < h.length; i++) {
                    if (h[i] instanceof ErrOutLogFileHandler target) {
                        String fileName = "";
                        handlerFound = true;
                        try {
                            ...
                            LOG.warning("Cannot read from the log file: " + fileName + " : " + ioe.getMessage());
                            ...
                            LOG.warning("Cannot read from the log file: " + fileName + " : " + ex.getMessage());
                            showMessage(ElanLocale.getString("ExportDialog.LogView.Message")
                                        + "\n"
                                        + fileName
                                        + " : "
    ...
    private void showMessage(String message) {
        JOptionPane.showMessageDialog(frame, message, ElanLocale.getString("Warning"), JOptionPane.WARNING_MESSAGE);
    }

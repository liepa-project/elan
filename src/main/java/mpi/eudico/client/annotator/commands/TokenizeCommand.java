...
/**
 * A Command that tokenizes the contents of the annotations a source tier new annotations on a destination tier.
 *
 * @author Han Sloetjes
 */
public class TokenizeCommand implements UndoableCommand,
    ...
    @Override
    public void undo() {
        if (transcription == null || sourceTier == null || destTier == null || completedTokenizations == null) {
            return;
        }
        int curPropMode = 0;
    ...
    /**
     * Wipe annotations from the destination tier in some time periods, because we're going to put different annotations
     * there. Used for both undo (to remove the token-annotations and  make room for the originals if any) and redo (to make
     * room for the token-annotations).
     */
    ...
    private class TokenizeThread extends Thread {
        ...
        public void run() {
            ...
            for (AbstractAnnotation srcAnn : sourceAnnos) {
                ...
                @SuppressWarnings("unused") Annotation parentAnn;
                if (destTierIsRootTier) {
                    parentAnn = null;
                    childrenOnDest =
                        destTier.getOverlappingAnnotations(srcAnn.getBeginTimeBoundary(), srcAnn.getEndTimeBoundary());
                } else {
                    parentAnn = srcAnn;
                    childrenOnDest = srcAnn.getChildrenOnTier(destTier);
                }
                ...
                if (childrenOnDest.size() == 0 || !preserve) {
                    ...
                    for (String nextToken : tokens) {
                        prevAnn = createAnnotation(destTier,
                                                   prevAnn,
    ...
    /**
     * Creates one of several annotations in a time period. Works for all 4 of the stereotypes. (SYMBOLIC_ASSOCIATION allows
     * only the first annotation to be made)
     *
     ...
     *
     * <p>The whole time period is from {@code bt} to {@code et}.
     *
     * <p>The period is subdivided in {@code numAnns} sub-periods, and the desired new
     * annotation occupies the {@code seq}th of that (0-based).
     *
     * <p>If the time period cannot be evenly divided into numAnns parts (the resolution
     * is in whole milliseconds) then some created annotations will be longer than others, to compensate for the accumulated
     * truncation loss.
     *
     * <p>Each annotation will start at the same moment that the previous one ends.
     * The first one will always start at moment {@code bt}. The last one will always end at moment {@code et}.
     *
    ...
    private static Annotation createSubperiodAnnotation(TierImpl t, long bt, long et, int numAnns, int seq) {
        ...
        //System.out.printf("createSubperiodAnnotation: seq=%d numAnns=%d perAnn=%f bt=%d et=%d start=%d end=%d, (%f,
        // %f, %f)\n",

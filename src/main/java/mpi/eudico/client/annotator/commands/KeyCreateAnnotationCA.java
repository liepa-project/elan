...
/**
 * A CommandAction to create an annotation by pressing a key combination alternatively at the  begin time and the end time of
 * the desired new annotation.
 */
...
public class KeyCreateAnnotationCA extends CommandAction {
    ...
    /**
     * Returns the current interval, begin and end time and resets the internal first  key stroke and second key stroke
     * values.
     *
    ...
    /**
     * Checks whether it is the first or the second key stroke, if the active tier is time alignable, if at the position of
     * the interval an annotation can be created etc.
     *
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (firstTime == -1) {
            ...
            if ((vm.getMultiTierControlPanel() != null) && (vm.getMultiTierControlPanel().getActiveTier() != null)) {
                TierImpl ti = (TierImpl) vm.getMultiTierControlPanel().getActiveTier();
                ...
                if (ti.isTimeAlignable()) {
                    if (!ti.hasParentTier()) {
                        ...
                        if ((ti.getParentTier().getAnnotationAtTime(firstTime) != null) || (ti.getParentTier()
                                                                                              .getAnnotationAtTime(st)
                                                                                            != null)) {

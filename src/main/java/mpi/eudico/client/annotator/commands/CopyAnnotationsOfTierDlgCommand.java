...
/**
 * Creates the dialog to copy annotations of one tier to another tier, with filtering.
 *
 * @author Han Sloetjes
 */
public class CopyAnnotationsOfTierDlgCommand implements Command {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        JDialog dialog = pane.createDialog(ELANCommandFactory.getRootFrame(trans),
                                           ElanLocale.getString("Menu.Tier.CopyAnnotationsOfTierDialog"),
                                           true);
        // make the window a bit bigger
        Rectangle bounds = dialog.getBounds();
        final int RES = 60;
        dialog.setBounds(Math.max(bounds.x - RES, 0),
                         Math.max(bounds.y - RES, 0),
                         bounds.width + 2 * RES,
                         bounds.height + 2 * RES);

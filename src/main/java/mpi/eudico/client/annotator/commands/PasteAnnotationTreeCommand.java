 ...
 *
 * <p>Note June 2006 transferal of application/x-java-serialized-object objects between jvm's
 * doesn't seem to work on Mac OS X. The Transferable sun.awt.datatransfer.ClipboardTransferable@8b4cb8 doesn't support any
 * flavor with human presentable name "application/x-java-serialized-object"
 */
public class PasteAnnotationTreeCommand extends NewAnnotationCommand {
    protected DefaultMutableTreeNode node;
    protected AnnotationDataRecord annotationDataRecord;
     ...
     * correct. The arguments parameter can be null, of length 1 or of length 2<br> If the arguments parameter is null the
     * following order is used to decide on which tier the annotation has te be pasted:<br> - the tier of the same name as of
     * the tier of the top level annotation in the tree, if it is present in the destination transcription<br> - the current
     * active tier
     *
     * <p>If there isn't an annotation tree (in the form of a DefualtMutableTreeNode) on the clipboard but
     * an AnnotationDataRecord instead this single toplevel annoation is pasted.
     *
     * @param receiver a transcription
     * @param arguments the arguments:  <ul><li>null</li></ul> or
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (canAccessSystemClipboard()) {
            ...
            if (contents instanceof DefaultMutableTreeNode) {
                ...
                annotationDataRecord = (AnnotationDataRecord) contents;
            } else {
                return;
            }

            destTierName = annotationDataRecord.getTierName();
            ...
            if (arguments != null && arguments.length > 0) {
                ...
                if (arguments.length > 1) {
                    ...
                    if (node != null && newBeginTime != annotationDataRecord.getBeginTime()) {
                        // adjust times if necessary
                        adjustTimes(node, newBeginTime - annotationDataRecord.getBeginTime());
                    }
                }
            }
            TierImpl tier = null;
            if (destTierName != null) {
                tier = transcription.getTierWithId(destTierName);
                ...
                if (tier != null && node != null && !destTierName.equals(annotationDataRecord.getTierName())) {
                    // if source and destination tier contains same affix, update tier names
                    TierNameCompare tnc = new TierNameCompare();
                    int[] indices = tnc.findCorrespondingAffix(annotationDataRecord.getTierName(), destTierName);
                    if (indices != null && indices[0] > -1) {
                        char del = annotationDataRecord.getTierName().charAt(indices[0]);
                        String affix = "";
            ...
            if (tier == null) {
                tier =
                    (TierImpl) ELANCommandFactory.getViewerManager(transcription).getMultiTierControlPanel().getActiveTier();
                if (tier != null) {
            ...
            if (newBeginTime != null) {
                args[0] = newBeginTime;
                args[1] = Long.valueOf(newBeginTime.longValue() + (annotationDataRecord.getEndTime()
                                                                   - annotationDataRecord.getBeginTime()));
            } else {
                args[0] = Long.valueOf(annotationDataRecord.getBeginTime());
                args[1] = Long.valueOf(annotationDataRecord.getEndTime());
            }
            transcription.setNotifying(false);
    ...
    void newAnnotation() {
        if (node != null) { // annotation tree
            ...
            if (annotationDataRecord != null) { // record is the root node object
                if (!annotationDataRecord.getTierName().equals(destTierName)) {
                    super.newAnnotation();

                    if (newAnnotation != null) {
                        AnnotationRecreator.restoreValueEtc((AbstractAnnotation) newAnnotation, annotationDataRecord, false);
        ...
        } else if (annotationDataRecord != null) { // single annotation
            super.newAnnotation();

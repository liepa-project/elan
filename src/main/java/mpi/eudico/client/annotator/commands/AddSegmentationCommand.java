...
/**
 * A Command that adds a group of segments/annotations to a tier in one, undoable, action.
 *
 * @author Han Sloetjes
 */
public class AddSegmentationCommand implements UndoableCommand {
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the name of the Tier, which should be a root tier
     *     (TierImpl)</li> <li>arg[1] = a list of time intervals (ArrayList containing  TimeInterval objects)</li> </ul>
     */
    @Override
    ...
    /**
     * Sort the intervals and correct overlapping intervals.<br> This is done by changing the end time of one annotation to
     * the begin time of the next annotation, in case the old end time  is greater than the next begin time.
     */
    ...
    /**
     * Makes a backup of the effected annotations of this tier, including child annotations (for undo).
     */
    private void storeAnnotations() {
        if ((segments != null) && (segments.size() > 0)) {
            ...
            for (int i = 0; i < segments.size(); i++) {
                ...
                List<Annotation> effectedAnn =
                    tier.getOverlappingAnnotations(curInterval.getBeginTime(), curInterval.getEndTime());
    ...
    private void restoreAnnotations() {
        ...
        for (int i = 0; i < changedAnnotations.size(); i++) {
            ...
            List<Annotation> v = tier.getOverlappingAnnotations(annRecord.getBeginTime(), annRecord.getEndTime());

...
/**
 * An undo-able command for copying annotations of one tier to another tier. The destination tier must either be an
 * independent, top-level tier or a direct child tier of the source tier. The implementation currently only supports copying
 * within a single transcription, there are preparations for supporting this kind of copying in multiple files.
 *
 * @author Han Sloetjes
 */
public class CopyAnnotationsOfTierCommand extends AbstractProgressCommand implements UndoableCommand {
    ...
    /**
     * @param receiver the Transcription in case of single file mode, null in case of multiple file mode
     ...
     *     <li>arguments[6]: the list of files to process (in case of multiple file mode) (List of
     *     String)</li>
     *     </ul>
     */
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (arguments != null && arguments.length >= 6) {
            try {
                ...
                // by default copy all
                copyAllAnnotations = !"WithValue".equals(cpMode);
                ...
                if (isRegex) {
                    ...
                    try {
                        // add some other flags to change default behavior? CANON_EQ,...
                        pattern = Pattern.compile(queryValue, Pattern.UNICODE_CHARACTER_CLASS);
                    } catch (PatternSyntaxException pse) {
                        if (LOG.isLoggable(Level.WARNING)) {
                            LOG.warning(String.format(
                                "Cannot execute the Copy Annotations command, invalid regular expression: %s",
                                pse.getMessage()));
                        }
                        progressInterrupt(String.format(
                            "Cannot execute the Copy Annotations command because of an invalid regular expression: %s",
                            pse.getMessage()));
                        return;
                    }
                }
                ...
                if (LOG.isLoggable(Level.WARNING)) {
                    LOG.warning(String.format("Cannot execute the Copy Annotations command: %s", t.getMessage()));
                }
                progressInterrupt(String.format(
                    "Cannot execute the Copy Annotations command because of a problem with the arguments: %s",
                    t.getMessage()));
                return;
            }
        } else {
    ...
    public void undo() {
        if (transcription != null) {
            if (copyAnnotations != null && !copyAnnotations.isEmpty()) {
                ...
                for (AnnotationDataRecord annotationDataRecord : copyAnnotations) {
                    long mid = (annotationDataRecord.getBeginTime() + annotationDataRecord.getEndTime()) / 2;
                ...
                } else if ((deletedAnnotations != null && !deletedAnnotations.isEmpty()) || (modifiedAnnotations != null
                                                                                             && !modifiedAnnotations.isEmpty())) {
    ...
    /**
     * Collects the annotations that are going to be copied. This depends on several settings: whether all annotations have
     * to be copied or just those with specific values and whether or not existing annotations on the destination tier can be
     * overwritten.
     *
    ...
    /**
     * Stores, for undo, records of annotations that are going to be removed or modified by the copy operation. annotations
     * on depending tiers are also immediately removed here.
     *
     * @param t1 the source tier
     * @param t2 the destination tier
                    ...
                    for (Annotation a : overlappingAnn) {
                        ...
                        if (TimeRelation.isInside(ovAnn, bt, et)) {
                            ...
                            deletedAnnotations.add(AnnotationRecreator.createTreeForAnnotation(ovAnn));
                                   ...
                                   || TimeRelation.overlapsOnRightSide(ovAnn,
                                                                       bt,
                                                                       et)
    ...
    /**
     * Checks the data records to be copied and removes CV entry references if the destination tier is not linked to a
     * controlled vocabulary or not to the same CV as the source tier. This should probably be extended to other types of
     * external references (e.g. lexicon fields).
     *
     ...
     * @param destinationCVName the name of the CV linked to the destination tier, can be null
     */
    private void updateAnnotationRecords(List<AnnotationDataRecord> copyAnnotations, String sourceCVName,
                                         String destinationCVName) {
     ...
     *
     * @param t2 the destination tier
     * @param copyAnnotations a list of records to be used for the creation of new annotations on the destination tier
     */
    private void createCopiesOfAnnotations(TierImpl t2, List<AnnotationDataRecord> copyAnnotations) {
        ...
        for (AnnotationDataRecord annotationDataRecord : copyAnnotations) {
            long bt = annotationDataRecord.getBeginTime();
            long et = annotationDataRecord.getEndTime();
            if (symbolicTier) {
                bt = (bt + et) / 2;
                et = bt;
            }
            ...
            if (na != null) {
                AnnotationRecreator.restoreValueEtc(na, annotationDataRecord, false);
            }
        }
    }

    /**
     * The steps involved in the copy operation are performed on a separate thread so that progress can be monitored. This
     * will be more useful in the multiple file situation than in the single transcription situation.
     */
    private class CopyThread extends Thread {
        ...
        public void run() {
            if (transcription != null) {
                ...
                progressUpdate(20,
                               String.format("Collected annotation information, %d annotations to copy",

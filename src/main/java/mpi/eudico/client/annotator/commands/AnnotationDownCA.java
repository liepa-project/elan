...
/**
 * Action that finds the annotation to activate on the tier visually below the tier the current active annotation is on (if
 * there is an active annotation). Searches recursively.
 */
@SuppressWarnings("serial")
public class AnnotationDownCA extends CommandAction implements ClientLogger {
    private Icon icon;
    ...
    public AnnotationDownCA(ViewerManager2 theVM) {
        ...
        getImageIconFrom(Constants.ICON_LOCATION + "GoToLowerAnnotation.gif").ifPresent(imageIcon -> {
            icon = imageIcon;
    ...
    /**
     * Tries to find the annotations "below" the current active annotation. If possible this is an overlapping annotation on
     * a tier which is visually below, in the user interface, the current active annotation.
     *
    ...
    protected Object[] getArguments() {
        ...
        if (currentActiveAnnot != null) {
            ...
            if ((mediaTime >= currentActiveAnnot.getBeginTimeBoundary()) && (mediaTime
                                                                             <= currentActiveAnnot.getEndTimeBoundary())) {
            ...
            if ((index > -1) && (index < (visibleTiers.size() - 1))) {
                ...
                for (int i = index; i < (visibleTiers.size() - 1); i++) {
                    ...
                    if (newActiveAnnot != null) {
                        ...
                        if ((newBeginTime >= beginTime && newBeginTime <= endTime) || (newBeginTime <= beginTime
                                                                                       && newBeginTime >= (beginTime
                                                                                                           - 1000L)) || (
                                newBeginTime >= endTime
                                && newBeginTime <= (endTime + 1000L))) {
                            break;
                        } else if (newBeginTime <= beginTime && newEndTime >= endTime) {

                            // if the annotation interval is larger than the active annotation but if it overlaps the active
                            // annotation, and if it is within the time interval of +/- 5 sec and the duration of the
                            // annotation should be less than 10 sec, then jumps to that annotation
            ...
            if (activeTier != null) {
                newActiveAnnot = getAnnotationAtOrBefore((TierImpl) activeTier, vm.getMasterMediaPlayer().getMediaTime());
            } else {
                ...
                if (firstTier == null) {
                    ...
                    newActiveAnnot = getAnnotationAtOrBefore(firstTier, vm.getMasterMediaPlayer().getMediaTime());
                }
            }
        }
    ...
    /**
     * If there is an annotation at the specified time return it, otherwise return the closest annotation before this time
     *
     ...
     */
    private Annotation getClosestAnnotation(final TierImpl tier, Annotation ann) {

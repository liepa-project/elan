...
/**
 * A command that creates a dialog to type/modify start and end time of an annotation and that creates a
 * {@link ModifyAnnotationTimeCommand} when the changes in the dialog are applied.
 *
 * @author Han Sloetjes
 */
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (ann instanceof AlignableAnnotation) {
            TimeIntervalEditDialog dialog =
                new TimeIntervalEditDialog(ELANCommandFactory.getRootFrame(vm.getTranscription()), true);
            ...
            if (nextBT > -1 && nextET > nextBT && !(nextBT == ann.getBeginTimeBoundary()
                                                    && nextET == ann.getEndTimeBoundary())) {
                // create an undoable command to change the time interval
                Command com = ELANCommandFactory.createCommand(ann.getTier().getTranscription(),

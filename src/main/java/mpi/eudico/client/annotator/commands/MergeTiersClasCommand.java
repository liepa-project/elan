...
/**
 * Undoable Command that merges the annotations on 2 tiers and creates new annotations on a third tier. Overlapping
 * annotations are merged into one new annotation. Non overlapping annotations are copied as is. The content depends  on the
 * user's choice: either concatenated annotation values or the duration of  the new annotation.
 *
 * @author Han Sloetjes
...
public class MergeTiersClasCommand implements UndoableCommand {
    ...
    /**
     * Oct 2008: a flag is added that determines to only create an annotation for overlapping annotations with the same
     * value. <b>Note: </b>it is assumed the types and order of the arguments are correct.
     *
    ...
    private class MergeThread extends Thread {
        ...
        public void run() {
            ...
            if (matchingValuesOnly) {
                    ...
                    for (; j < numAnns2; j++) {
                        ...
                        if (et2 <= bt1) {
                            ...
                            if (!specificValueOnly) {
                                if ((ann1.getValue() == null && ann2.getValue() == null) || ((ann1.getValue() != null
                                                                                              && ann2.getValue() != null)
                                                                                             && (ann1.getValue().length()
                                                                                                 == 0
                                                                                                 && ann2.getValue().length()
                                                                                                    == 0) || (ann1.getValue()
                                                                                                                  .equals(
                                                                                                                      ann2.getValue())))) {
                                    overlaps.add(new AnnotationValuesRecord(sourceTier1.getName(),
                                                                            ann1.getValue(),
                                                                            obt,
                                                                            oet));
                                }
                ...
                // progress updates
                int annCount = 2 * (sourceTier1.getNumberOfAnnotations() + sourceTier2.getNumberOfAnnotations());
                if (annCount > 0) {
            ...
            for (int i = 0; i < overlaps.size(); i++) {
                curRecord = overlaps.get(i);
                ann = destTier.createAnnotation(curRecord.getBeginTime(), curRecord.getEndTime());

                if (ann != null) {
                    if (concatenate) {
                        ann.setValue((curRecord.getNewLabelValue() != null)
                                     ? curRecord.getNewLabelValue()
                                     : curRecord.getValue());
                    } else {
                        switch (timeFormat) {
                            case Constants.MS:

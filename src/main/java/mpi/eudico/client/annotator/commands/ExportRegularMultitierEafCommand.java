...
/**
 * A command to save a transcription produced in Simple-ELAN with separated tiers for translation and speakers (in case there
 * are special markers for that within annotations).
 *
 * <p>Current implementation is not optimized; existing annotations are checked
 * several times for the special markers, first to accommodate the creation of new tiers, then to split annotations into text
 * and translations, possibly per speaker
 */
public class ExportRegularMultitierEafCommand implements Command {
    private final String name;
    private final String translationTypeName = "translation";
    private final String transSuffix = "trans";
    private final String unknownSpeakerName = "unknown";
    ...
    /**
     * Shows a file chooser, converts tiers with compound annotations (transcription and translation or speaker markers) to
     * separate tiers and saves the result as a regular eaf file.
     *
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        File origFile = null;
        //chooser.createAndShowFileDialog(title, dialogType, extensions, mainFilterExt,
        // prefStringToLoadtheCurrentPath, selectedFileName);
        ...
        if (f != null) {
            ...
            if (nextTranscription != null) {
                ...
                if (nextTranscription.isChanged()) {
                    try {
                        ACMTranscriptionStore.getCurrentTranscriptionStore()
                                             .storeTranscriptionIn(nextTranscription,
                                                                   null,
                                                                   null,
                                                                   nextAbsPath,
                                                                   ACMTranscriptionStore.getCurrentEAFParser()
                                                                                        .getFileFormat());
                    } catch (IOException ioe) {
                        JOptionPane.showMessageDialog(ELANCommandFactory.getRootFrame(transcription),
                                                      ElanLocale.getString("Message.Error.Save"),
    ...
    /**
     * Checks the contents of tiers to see if a translation tier needs to be added and/or if tiers per speaker need to be
     * created
     *
    ...
    /**
     * If an annotation contains translation markers and/or speaker markers, the contents is split up and distributed over
     * several annotations, on several tiers.
     *
     ...
     */
    private void splitAnnotations(TranscriptionImpl tr, Map<TierImpl, List<TierImpl>> speakerTierMap,
                    ...
                    while (speakIt.hasNext()) {
                        ...
                        if (transIndices != null) {
                            ...
                            if (transIndices[0] + TaSSpecialMarkers.TRANS_MARKER.length() < valLen - 1) {
                                trans = value.substring(transIndices[0] + TaSSpecialMarkers.TRANS_MARKER.length()).trim();
                            }
     ...
     * @param speakerTierMap a map with as keys tiers with at least one annotation with a speaker / tier indication and
     *     as values lists of new tiers derived from the encountered speaker / tier names
     *
     * @return a map of source tiers to dependent translation tiers
     */
    private Map<TierImpl, TierImpl> createNewTranslationTiers(TranscriptionImpl tr, List<TierImpl> tiersWithTrans,
        ...
        for (TierImpl t : tiersWithTrans) {
            ...
            boolean derivedTiersExist = (derivedSpeakTiers != null && !derivedSpeakTiers.isEmpty());
            ...
            if (derivedTiersExist) {
                ...
                for (TierImpl derTier : derivedSpeakTiers) {
                    ...
                    TierImpl derTransTier =
                        new TierImpl(derTier, derTierName, derTier.getParticipant(), tr, translationType);

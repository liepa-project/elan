...
/**
 * An undoable command that builds on the new annotation command. It takes the new annotation as input, and creates new
 * annotation recursively on all the dependent tiers This is then one undoable command in the undo/redo list.
 */
public class NewAnnotationRecursiveCommand extends NewAnnotationCommand {
    ...
    public void execute(Object receiver, Object[] arguments) {
        ...
        if (numberofRecAnnotations > 0 && MonitoringLogger.isInitiated()) {
            MonitoringLogger.getLogger(transcription)
                            .log(MonitoringLogger.RECURSIVE_ANNOTATIONS,
                                 "number of annotations created recursively : " + numberofRecAnnotations);
        }
    }

    /**
     * The creation of the new annotation recursively on all dependent tiers
     */
    ...
    private void createNewAnnotationsRecursively() {
        ...
        if (dependentTiers != null && dependentTiers.size() > 0) {
            ...
            tier.getTranscription()
                .notifyListeners(tier, ACMEditEvent.ADD_ANNOTATION_HERE, tier.getAnnotationAtTime(newAnnBegin));
        }
    }

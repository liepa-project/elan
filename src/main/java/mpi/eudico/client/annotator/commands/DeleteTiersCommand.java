...
public class DeleteTiersCommand implements UndoableCommand {
            ...
            for (int i = 0; i < tiers.length; i++) {
                ...
                if (annotationsNodes[i].size() > 0) {
                    ...
                    if (tiers[i].hasParentTier()) {
                        AnnotationRecreator.createAnnotationsSequentially(transcription, annotationsNodes[i], true);
                    } else {
                        for (int j = 0; j < annotationsNodes[i].size(); j++) {
                            node = annotationsNodes[i].get(j);
     ...
     * @param arguments the arguments:  <ul><li>arg[0] = the tier to remove (TierImpl)</li> </ul>
     */
    @Override
        ...
        for (int i = 0; i < numTiers; i++) {
            ...
            if (tiers[i] != null) {
                ...
                while (anIter.hasNext()) {
                    ann = anIter.next();
                    annotationsNodes[i].add(AnnotationRecreator.createTreeForAnnotation(ann));
                }

...
/**
 * A Command that copies a tier or changes the parent of a Tier by making a copy of the Tier 'under' the new parent.
 * Optionally dependent Tiers will also be copied.
 *
 * @author Han Sloetjes
 */
public class CopyTierCommand implements UndoableCommand {
    ...
    // if original tiers are renamed, instead of the new tiers having new names
    ...
    /**
     * Creates copies of the tier and sub-tiers as a dependent tier of the new parent. <b>Note: </b>it is assumed the types
     * and order of the arguments are correct.
     *
    ...
    /**
     * Undo the changes made by this command, i.e. removes the created/copied tier(s).
     */
    @Override
    public void undo() {
        ...
        if ((transcription != null) && (newTiers != null)) {
            ...
            if (tierCopy != null && !copiedAnnotationsNodes.isEmpty()) {
                ...
                if (tier.hasParentTier()) {
                    AnnotationRecreator.createAnnotationsSequentially(transcription, copiedAnnotationsNodes, true);
                } else {
    ...
    /**
     * Creates a copy of the tier and its depending tiers and tries to re-create  the annotations on the copied tiers. Since
     * the LinguisticType might have been changed, success is not guaranteed.
     *
     * @author Han Sloetjes
     * @version 1.1
     */
    class CopyThread extends Thread {
        /**
         * Depending on the copying mode ('groupwise' or not), this ArrayList contains either DefaultMutableTreeNode(with
         * AnnotationDataRecord)s, or List&lt;DefaultMutableTreeNode>s.
         */
        ...
        public void run() {
            try {
                ...
                // This might happen:
                // java.lang.ClassCastException: mpi.eudico.server.corpora.clomimpl.abstr.RefAnnotation cannot be
                // cast to mpi.eudico.server.corpora.clomimpl.abstr.AlignableAnnotation
        ...
        private void checkTransitionType() {
            ...
            if (tier.getLinguisticType().getConstraints() != null) {
                oldStereo = tier.getLinguisticType().getConstraints().getStereoType();
            }
            ...
            if (parent != null && parent != tier) {
                ...
                if (((transitionType == TierCopier.SAME) && ((stereo == Constraint.TIME_SUBDIVISION) || (stereo
                                                                                                         == Constraint.SYMBOLIC_SUBDIVISION)))
                    || ((transitionType != TierCopier.ANY_TO_ROOT) && (transitionType != TierCopier.ROOT_TO_ASSOC) && (
                    transitionType
                    != TierCopier.ROOT_TO_SYMSUB) && (transitionType != TierCopier.ROOT_TO_TIMESUB) && (transitionType
                                                                                                        != TierCopier.ROOT_TO_INCLUDED_IN))) {
        ...
        /**
         * Create copies of the tier(s) involved. A unique name is generated and  'proper' Linguistic Types are applied to
         * the new tiers.
         */
        private void copyTiers() {
            ...
            while (en.hasMoreElements()) {
                ...
                if (pt != null) {
                    tn = new TierImpl(pt, newName, to.getParticipant(), transcription, nextType);
                } else {
                    tn = new TierImpl(newName, to.getParticipant(), transcription, nextType);
                }
        ...
        /**
         * Store information on the annotations of the tier to reparent (and depending tiers).
         */
        ...
        /**
         * Creates copies of the annotation: success might depend on the new LinguisticType  and on the annotations on the
         * (new) parent tier.
         */
                ...
                if (parent != null) {
                    ...
                    if (groupWiseCopying) {
                        ...
                        for (int i = 0; i < numAnnotationsNodes; i++) {
                            ...
                            copier.createAnnotationFromTree(transcription, node, oldNameToNewName);
                            count++;
                            progressUpdate((int) (50 + (count * perAnn)), null);
                        }
                    }
                    ...
                    for (int i = 0; i < numAnnotationsNodes; i++) {
                        node = (DefaultMutableTreeNode) annotationsNodes.get(i);
                        copier.createRootAnnotationFromTree(transcription, node, oldNameToNewName);
                        count++;
                        progressUpdate((int) (50 + (count * perAnn)), null);
                    }
                }

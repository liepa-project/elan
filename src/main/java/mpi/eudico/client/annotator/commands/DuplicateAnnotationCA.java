...
/**
 * A command action for duplicating (copy and paste in a single operation) an annotation. Based on the active annotation and
 * the active tier in TimeLineViewer or InterlinearViewer.
 *
 * @author Han Sloetjes
 */
public class DuplicateAnnotationCA extends CommandAction implements ActiveAnnotationListener {
    ...
    protected void newCommand() {
        command = ELANCommandFactory.createCommand(vm.getTranscription(), ELANCommandFactory.DUPLICATE_ANNOTATION);
    }
    ...
    protected Object[] getArguments() {
        return new Object[] {vm.getMultiTierControlPanel().getActiveTier(), vm.getActiveAnnotation().getAnnotation()};
    }

    /**
     * Override ationPerformed to perform some initial checks before calling super.actionPerformed, where a new Command is
     * created etc.
     *
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (c != null && (c.getStereoType() == Constraint.SYMBOLIC_ASSOCIATION
            ...
            long mid = (vm.getActiveAnnotation().getAnnotation().getBeginTimeBoundary() + vm.getActiveAnnotation()
                                                                                            .getAnnotation()
                                                                                            .getEndTimeBoundary()) / 2;
            if (t.getAnnotationAtTime(mid) != null) {
                return;
            } else {
                TierImpl par = t.getParentTier();

                if (par.getOverlappingAnnotations(vm.getActiveAnnotation().getAnnotation().getBeginTimeBoundary(),
                                                  vm.getActiveAnnotation().getAnnotation().getEndTimeBoundary()).size()
                    == 0) {
                    return;
                }
            }
        } else if (c != null && (c.getStereoType() == Constraint.TIME_SUBDIVISION
            ...
            if (ann1 != null && ann2 != null && ann1 != ann2) {
                //return; // don't choose?
                       ...
                       && ann2 == null
                       && par.getOverlappingAnnotations(vm.getActiveAnnotation().getAnnotation().getBeginTimeBoundary(),
                                                        vm.getActiveAnnotation().getAnnotation().getEndTimeBoundary()).size()
                          == 0) {
                return;
            }

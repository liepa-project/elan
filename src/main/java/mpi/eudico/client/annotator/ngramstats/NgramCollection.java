...
public class NgramCollection {
	...
	/**
	 * Returns the average duration as a string.
	 * 
	...
	/**
	 * Returns the minimum duration as a string.
	 * 
	...
	/**
	 * Returns the maximum duration as a string.
	 * 
	...
	/**
	 * Returns the average annotation duration as a string.
	 * 
	...
	/**
	 * Returns the average interval time as a string.
	 * 
	 * @return the average interval time of the annotations in the collection
	 * or {@code null} for {@code unigrams}
	 */
	...
	private void calcDescriptiveStats(String name, List<Double> data) {
...

		{
			...
			if (data.size() > 2) {
				if (data.size() % 2 == 0) { // even-sized array
					...
					// feb 2024 changed to include the center in both halves
					iq1 = calcMedian( data.subList( 0, (int)Math.ceil(data.size() / 2d) ) );
					iq3 = calcMedian( data.subList( (int)Math.floor(data.size() / 2d), data.size() ) );
				}
			} else {
		...
		
		{
			...
			int count = 0;
			for (Map.Entry<Double, Integer> modeEntry : modeMap.entrySet()) {
				Double x = modeEntry.getKey();
				Integer cValue = modeEntry.getValue();
				...
				if (cValue == count && x > mode) {
					mode = x;
				} else 	if (cValue > count) {
					mode = x;
					count = cValue;
				}
			}

...
public class CommentViewer extends  AbstractViewer
	...
	private static Color modifiableColor = Constants.LIGHTBACKGROUNDCOLOR;
	...
	/** re-pop up milliseconds identifier */
	...
	public CommentViewer(TranscriptionImpl transcription) {
		...
		if (Constants.DARK_MODE) {
			filteredLightColor = new Color(
					Constants.LIGHTBACKGROUNDCOLOR.getRed(), 
					Math.min(255, Constants.LIGHTBACKGROUNDCOLOR.getGreen() + 30), 
					Math.min(255, Constants.LIGHTBACKGROUNDCOLOR.getBlue() + 50));
		}
...
//		rightPanel.add(menuBar, gbc);
	...
	public void setRegexFilter(String match, boolean caseSensitive) {
		...
		if (match != null && !match.isEmpty()) {
			...
			String flags = caseSensitive ? "(?U)" : "(?iU)";
			try {
...
//				}
	...
	/** postpone option constant. */
	public static final int POSTPONE_OPTION = 0;
	/** keep local as new option constant. */
	public static final int KEEP_LOCAL_AS_NEW_OPTION = 1;
	/** reject option constant. */
	public static final int REJECT_OPTION = 2;
	/** accept option constant. */
	public static final int ACCEPT_OPTION = 3;

	/** import as new option constant. */
	public static final int IMPORT_AS_NEW_OPTION = -99; // not recommended it seems...
	
	/**
	 * Option dialog for postpone, keep as new, reject, accept options
	 * @param msg the string mesage
	 * @return the constant to indicate which option was choosen
	 */
	...
	private final class RedTextDocumentListener implements
		...
		public void insertUpdate(DocumentEvent e) {
			if (commentOfText == null || !commentOfText.getMessage().equals(text.getText())) {
				...
				text.setForeground(Constants.DEFAULTFOREGROUNDCOLOR);
			}
		}

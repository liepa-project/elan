...
public class SingleTierViewerPanel extends JPanel implements ACMEditListener,
	...
	public SingleTierViewerPanel(ViewerManager2 viewerManager) {
        ...
        tierComboBox.setRenderer(new EmptyRenderer((ListCellRenderer<String>)tierComboBox.getRenderer()));
        /**
            ...
            try {
            	if (Constants.DARK_MODE) {
                	icon = new ImageIcon(this.getClass().getResource(
                			"/mpi/eudico/client/annotator/resources/dark/triangle_down.gif"));
            	} else {
                	icon = new ImageIcon(this.getClass().getResource(
                			"/mpi/eudico/client/annotator/resources/triangle_down.gif"));
            	}
    ...
    private void tierChanged(TierImpl tier) {
        ...
        if (tier == currentTier) {
            ...
            if ( !tierTable.containsKey(newLabel) ) {
            	...
            	for (Map.Entry<String, Tier> tierEntry : tierTable.entrySet()) {
            		String name = tierEntry.getKey();
            		
            		if (tierEntry.getValue() == tier) {
    ...
    private void selectTier(Tier tier) {
    	if (tier == null) {
    		...
    		for (Map.Entry<String, Tier> tierEntry : tierTable.entrySet()) {
				Tier tierIt = tierEntry.getValue();
				if (tierIt == tier) {
					tierComboBox.setSelectedItem(tierEntry.getKey());
					break;
				}
			}
    	}
...
//			}
	 ...
	 *
	 */
	private class EmptyRenderer implements ListCellRenderer<String> {
		private ListCellRenderer<String> mainRenderer;

		/**
		 * Constructor with the real renderer as the parameter.
		 */
		EmptyRenderer (ListCellRenderer<String> renderer) {

...
public class TimeLineViewer extends TimeScaleBasedViewer
    ...
    private JCheckBoxMenuItem editBoxAlwaysInViewMI;
    ...
    private JMenuItem modifyAnnoResURLMI;
    ...
    private boolean editBoxRemainsInView = true;
    private final int EDITBOX_MARGIN = 5;
    ...
    private boolean viewPortZoomSliderVisible = true;
    private JCheckBoxMenuItem zoomSliderVisibleMI;
    private ReentrantLock scrollLock = new ReentrantLock();
    private ReentrantLock viewPortLock = new ReentrantLock();
    ...
    private void initViewer() {
        ...
        Color lineCol = new Color (
        		(Constants.DEFAULTFOREGROUNDCOLOR.getRed() + Constants.DEFAULTBACKGROUNDCOLOR.getRed()) / 2, 
        		(Constants.DEFAULTFOREGROUNDCOLOR.getGreen() + Constants.DEFAULTBACKGROUNDCOLOR.getGreen()) / 2,
        		(Constants.DEFAULTFOREGROUNDCOLOR.getBlue() + Constants.DEFAULTBACKGROUNDCOLOR.getBlue()) / 2);
        zoomSlider.setBorder(new LineBorder(lineCol, 1)); 
        add(zoomSliderPanel);
        corner = new JPanel();
        add(corner);
    ...
    private void paintBuffer() {
    	...
    	// remove defBarWidth from the test and image width?
    	int bw = viewPortMode ? 0 : defBarWidth;
        if (((curWidth - bw) != imageWidth) ||
                (imageHeight != ((visibleTiers.size() * pixelsForTierHeight) +
                rulerHeight))) {
            imageWidth = curWidth - bw;
            imageHeight = (visibleTiers.size() * pixelsForTierHeight) +
                rulerHeight;
        ...
        if (intervalEndTime > dur) {
        	if (viewPortMode) xx -= xAt(intervalBeginTime);
        ...
        if (intervalBeginTime < dur) {
	        ...
	        for (int i = 0; i < numStripes; i++) {
	            if (i % 2 != 0) {
	                big2d.fillRect(0, y + i * pixelsForTierHeight, xx, pixelsForTierHeight);
	            }
	        }
        }
        big2d.translate(-ax, 0);
        //paint selection
...
//            big2d.setColor(selectionColorAlpha4);
        ...
        synchronized (tierLock) {
            Iterator<Tier2D> visIt = visibleTiers.iterator();
            	...
            	Font tf = getFontForTier(tier2d.getTier());
                big2d.setFont(tf);
...
//                	big2d.setColor(activeTierAlpha7);
        ...
        if (timeRulerVisible) {
        	big2d.setColor(Constants.DEFAULTBACKGROUNDCOLOR);
        	big2d.fillRect(0, viewPortY, imageWidth, verticalScrollOffset + rulerHeight);
	        big2d.setColor(Constants.SELECTIONCOLOR);
	        big2d.drawLine(0, verticalScrollOffset + viewPortY + rulerHeight, imageWidth, 
	        		verticalScrollOffset + viewPortY + rulerHeight);
	        big2d.translate(-((int)(intervalBeginTime / msPerPixel)), verticalScrollOffset + viewPortY);
	        //paint selection
	        if (selectionBeginPos != selectionEndPos) {
	            int beginPos = timeToPixels(getSelectionBeginTime());
	            int endPos = timeToPixels(getSelectionEndTime());
...
//        Toolkit.getDefaultToolkit().sync();
    ...
    private void paintCommentIndicators(Graphics2D gr2d, boolean buffered) {
    	...
    	if (cv == null) {
       	...
       	int rulerY = buffered ? verticalScrollOffset
       			              : (viewPortMode ? viewPort.getViewPosition().y: 0);
       	int rulerH = timeRulerVisible ? rulerHeight : 0;       	
       	if (viewPortMode && buffered) gr2d.translate(-xAt(intervalBeginTime), 0);
       	...
       	if (someMoreToTheLeft) {
   			...
   			ruler.setColor(COMMENT_INDICATOR_EDGE_COLOR);
   			int left = 0;
   			if (viewPortMode) left = xAt(intervalBeginTime);
	...
	public void paintComponent(Graphics g) {
        ...
        synchronized(getTreeLock()) {
	        ...
	        g2d.fillRect(scrollViewBX, 0, imageWidth, h);
	        ...
	        int xOff = viewPortMode ? 0 : (int) (intervalBeginTime / msPerPixel);
	        if ((cursorTag2D != null) &&
	                visibleTiers.contains(cursorTag2D.getTier2D())) {
	        	int x =  cursorTag2D.getX() - xOff;   	
	...
	private void paintUnbuffered(Graphics2D g2d) {
    	int vx = 0, y = 0, w = 0, h = 0;
        // scrolling related fill
        g2d.setColor(Constants.DEFAULTBACKGROUNDCOLOR);
        
    	if (viewPortMode) {
    		// paint a bit/lot more than just the visible area? Less glitches while scrolling
    		//vx = xAt(intervalBeginTime);
    		//w = getViewWidth();
    		w = viewPort.getViewSize().width;
    		//h = getViewHeight();
    		h = viewPort.getViewSize().height;
    		g2d.fillRect(vx, y, w, h);
    		y = rulerHeight;
    	} else {
    		w = getWidth();
    		h = getHeight();
    		g2d.fillRect(vx, y, w, h);
    		y = rulerHeight - verticalScrollOffset;
    	}
        
        // paint a slightly different background color for every other tier
        g2d.setColor(Constants.LIGHTBACKGROUNDCOLOR);
        for (int i = 0; i < visibleTiers.size(); i += 2) {
        	g2d.fillRect(vx, y + (i * pixelsForTierHeight), w, pixelsForTierHeight);
        }
...
//            g2d.setColor(selectionColorAlpha4);
        ...
        int ax = viewPortMode ? 0 : timeToPixels(intervalBeginTime);
        ...
        synchronized (tierLock) {
        	...
        	while (visIt.hasNext()) {
        		...
        		if (tier2d.isActive()) {    
        			...
        			//                	big2d.setColor(activeTierAlpha7);
        			g2d.fillRect(ax, y, w, pixelsForTierHeight);
        			g2d.setComposite(AlphaComposite.SrcOver);
        		}
        ...
        if (timeRulerVisible) {
	        g2d.setColor(Constants.DEFAULTBACKGROUNDCOLOR);
	        g2d.fillRect(ax, viewPortY, w, rulerHeight);
	        g2d.setColor(Constants.DEFAULTFOREGROUNDCOLOR);
	        g2d.translate(0, viewPortY);
        ...
        if (selectionBeginPos != selectionEndPos && timeRulerVisible) {
            ...
            g2d.setComposite(alpha04);
//        	big2d.setColor(selectionColorAlpha4);
            g2d.fillRect(selectionBeginPos, viewPortY, (selectionEndPos - selectionBeginPos), rulerHeight);
            g2d.setComposite(AlphaComposite.SrcOver);
        }
        if (timeRulerVisible) {
	        g2d.setColor(Constants.SELECTIONCOLOR);
            g2d.drawLine(0, viewPortY + rulerHeight, w, viewPortY + rulerHeight);
        }
        // from paintComponent
        //g2d.setFont(font);
        if ((cursorTag2D != null) &&
                visibleTiers.contains(cursorTag2D.getTier2D())) {
        	int cx =  cursorTag2D.getX();
        	if (!viewPortMode) cx -= (int) (intervalBeginTime / msPerPixel);
        	int ww = cursorTag2D.getWidth();
            int yy = (rulerHeight + (cursorTierIndex * pixelsForTierHeight) +
                pixelsForTierHeightMargin) - verticalScrollOffset;
            int he = pixelsForTierHeight - (2 * pixelsForTierHeightMargin);
            paintCursorTag2D(g2d, cursorTag2D, cx, yy, ww, he);
        }
        ...
        if (dragEditTag2D != null) {
            ...
            int dx = (int) dragEditTag2D.getX();
            if (!viewPortMode) dx -= (int) (intervalBeginTime / msPerPixel);
            ...
            int he = pixelsForTierHeight - (2 * pixelsForTierHeightMargin);
            paintDragEditTag2D(g2d, dx, yy, ww, he);
        }
...
//                        (intervalBeginTime / msPerPixel));
                	int x = xAt(a.getBeginTimeBoundary());
    		...
    		for (Tag2D t2d : selectedAnnotations) {
    			if (visibleTiers.contains(t2d.getTier2D())) {
    				g2d.drawRect(t2d.getX() - xOff, 
    						viewPortY + rulerHeight + (getTierIndexForAnnotation(t2d.getAnnotation()) * pixelsForTierHeight) + 1 - verticalScrollOffset, 
    ...
    public int xAt(long t) {
    	...
    	if (viewPortMode) {
    		return (int) Math.round(t / msPerPixel);
    	} else {
    		return (int) ((t - intervalBeginTime) / msPerPixel);
    	}
    }
    ...
    public long timeAt(int x) {
        ...
        if (viewPortMode) {
        	return (long) Math.round(x * msPerPixel);
    	} else {
     ...
     * {@code media duration in ms / ms per pixel}. Therefore the return value does
    ...
    private int timeToPixels(long theTime) {
        return (int) Math.round(theTime / msPerPixel);
    }
    ...
    public int calcTierHeight() {
    	int th = (int)((font.getSize() * 2.5d) + (36.0d / font.getSize()));
        if (reducedTierHeight) {
        	th = (th * 2) / 3;
        }
        return th;
...
*/
        ...
        if (viewPortMode) {
        	oldScreenPos -= viewPortX;//viewPort.getViewPosition().x 
        ...
        //new values
        long newIntervalBeginTime = (long) (((numScreens * imageWidth) - diff) * msPerPixel);

        if (newIntervalBeginTime < 0) {
        	newIntervalBeginTime = 0;
        }
        
        recalculateTagSizes();

        if (viewPortMode) {
        	viewPort.setViewSize(this.getPreferredSize());
        	setIntervalBeginTime(newIntervalBeginTime);
        	viewPortX = viewPort.getViewPosition().x;
        } else {
        	intervalBeginTime = newIntervalBeginTime;
...
//        int zoom = (int) (100f * (10f / msPerPixel));
        ...
        if ((!viewPortMode || viewPortZoomSliderVisible) && zoomSlider != null) {
    ...
    private void setLocalTimeScaleIntervalBeginTime(long begin) {
        ...
        try {
	        if (scrollLock.tryLock() || scrollLock.tryLock(20, TimeUnit.MILLISECONDS)) {
	        	try {
			        ...
			        selectionEndPos = xAt(getSelectionEndTime());
			        if (viewPortMode) {
			        	// instead of calling scrollRectToVisible, set the value of the horizontal scrollbar directly
			        	int curHorValue = scrollPane.getHorizontalScrollBar().getValue();
			        	int nextHorValue = xAt(intervalBeginTime);
			        	if (nextHorValue != curHorValue) {
			        		scrollPane.getHorizontalScrollBar().setValue(nextHorValue);
			        	}
//			        	scrollRectToVisible(new Rectangle(xAt(intervalBeginTime), viewPort.getViewPosition().y, 
...
//        repaint();
    ...
    private void ensureVerticalVisibilityOfActiveAnnotation() {
        ...
        if (viewPortMode) {
        	...
        	} else if (cy > viewPortY + viewPort.getViewRect().height 
        			- pixelsForTierHeight - rulerHeight) {
        		Point p = viewPort.getViewPosition();
        		p.y = cy - viewPort.getViewRect().height + pixelsForTierHeight + rulerHeight;
        		viewPort.setViewPosition(p);
        	}
        } else {
...
//        }
    ...
    private void notifyMultiTierControlPanel() {
        ...
        if (tierYPositions.length > 0) {
            ...
            tierYPositions[0] = (bh + (pixelsForTierHeight / 2)) -
                Math.max(verticalScrollOffset, viewPortY);
    ...
    public int getRightMargin() {
    	if (viewPortMode) {
    		if (viewPort.getParent() instanceof JScrollPane) {
    			return ((JScrollPane) viewPort.getParent()).getVerticalScrollBar().getWidth();
    		}
    	} else {
    ...
    private void createPopupMenu() {
        ...
        zoomSliderVisibleMI = new JCheckBoxMenuItem(ElanLocale.getString("TimeLineViewer.ZoomSliderVisible"));
        zoomSliderVisibleMI.setSelected(viewPortZoomSliderVisible);
        zoomSliderVisibleMI.addActionListener(this);
        zoomSliderVisibleMI.setVisible(viewPortMode);
        popup.add(zoomSliderVisibleMI);
        
        editBoxAlwaysInViewMI = new JCheckBoxMenuItem(ElanLocale.getString(
        		"TimeLineViewer.EditBoxAlwaysInView"));
        if (viewPortMode) {
	        editBoxAlwaysInViewMI.setSelected(editBoxRemainsInView);
	        editBoxAlwaysInViewMI.addActionListener(this);
	        popup.add(editBoxAlwaysInViewMI);
        }
        popup.addSeparator();
        ...
        modifyAnnoResURLMI = new JMenuItem(ELANCommandFactory.getCommandAction(
        		transcription, ELANCommandFactory.MODIFY_ANNOTATION_RESOURCE_URL));
        popup.add(modifyAnnoResURLMI);
        contextSensitiveItems.add(modifyAnnoResURLMI);
...
//    	}
    ...
    private void zoomToShowEntireMedia() {
    	...
    	if (viewPortMode) {
    		areaWidth = viewPort.getExtentSize().width - SCROLL_OFFSET;
    	}
    	float nextMsPP = getMediaDuration() / (float) areaWidth;
    	if (Math.abs(nextMsPP - msPerPixel) > 0.00000001f) { // i.e. (nextMsPP != msPerPixel)
	    	setMsPerPixel(nextMsPP);
...
//        mergeAnnoBeforeMI.setEnabled(false);	// enabled/disabled from its Action
        ...
        if (timeRulerVisible && (p.y < rulerHeight)) {
            ...
            try {
                ...
                Tag2D tag2d = getTagAt(inverse, tierIndex);
                ...
                if ((tag2d != null) && (tag2d == cursorTag2D)) {
                    ...
                    if (UrlOpener.hasBrowserLink((AbstractAnnotation)tag2d.getAnnotation())) {
                    	showInBrowserMI.setEnabled(true);
//                    	URI mediaURI = UrlOpener.getBrowserURIFrom((AbstractAnnotation)tag2d.getAnnotation());
//                    	if (mediaURI != null && UrlOpener.isRemoteAVMediaLink(mediaURI.toString())) {
                    		// this doesn't work via Desktop.open(File f), the basis of ShowInDefaultApplication
//                    		showInBrowserMI.setText(ElanLocale.getString("Menu.Annotation.ShowInDefaultApplication"));
//                    	} else {
                    		showInBrowserMI.setText(ElanLocale.getString("Menu.Annotation.ShowInBrowser"));
//                    	}
                    } else if (UrlOpener.hasLocalExternalResourceRef((AbstractAnnotation)tag2d.getAnnotation())) {
                    	showInBrowserMI.setEnabled(true);
                    	showInBrowserMI.setText(ElanLocale.getString("Menu.Annotation.ShowInDefaultApplication"));
                    }
                } else {
    ...
    /**
     * Sets the list of keystrokes not to be consumed by this viewer.
     * 
     * @param ksList the keystrokes not to consume
     */
    ...
    private void updateEditBoxLocation(InlineEditBox editBox, int x, int y) {
    	...
    	if (!viewPortMode || !editBoxRemainsInView) {
    		editBox.setLocation(x, y);
    	} else {
    		// try to keep the edit box inside the view
    		int x2 = x; 
    		int y2 = y;
    		int w2 = editBox.getWidth();
    		int h2 = editBox.getHeight();
    		if (x2 < viewPortX) {
    			x2 = viewPortX + EDITBOX_MARGIN;
    		}
    		if (y2 < viewPortY) {
    			y2 = viewPortY + EDITBOX_MARGIN;
    		}
    		if (w2 > getViewWidth()) {
    			w2 = getViewWidth() - 2 * EDITBOX_MARGIN;
    		}
    		if (x2 + w2 > viewPortX + getViewWidth()) {
    			x2 = viewPortX - EDITBOX_MARGIN + getViewWidth() - w2;
    		}
    		if (y2 + h2 > viewPortY + getViewHeight()) {
    			y2 = viewPortY - EDITBOX_MARGIN + getViewHeight() - h2;
    		}
    		
    		editBox.setLocation(x2, y2);
    		editBox.setSize(w2, h2);
...
alltierloop: 
     ...
     * 
     * Runs on a separate thread, make it run on the AWT event dispatch thread.
     */
    @Override
    public void controllerUpdate(ControllerEvent event) {
    	EventQueue.invokeLater(() -> {
    		controllerUpdate0(event);
		});
    }
    
	private void controllerUpdate0(ControllerEvent event) {
	...
	public void updateLocale() {
        if (popup != null) {
            ...
            shiftActiveAnnMI.setText(ElanLocale.getString("Menu.Annotation.Shift") + " "
            		+ ElanLocale.getString("Menu.Annotation.ShiftActiveAnnotation"));
            editBoxAlwaysInViewMI.setText(
            		ElanLocale.getString("TimeLineViewer.EditBoxAlwaysInView"));
        }
    ...
    public void updateFont(Font f) {
        ...
        for (Map.Entry<String, Font> fontEntry : prefTierFonts.entrySet()) {
        	Font prFont = fontEntry.getValue();
        	if (prFont != null) {
        		prefTierFonts.put(fontEntry.getKey(), new Font(prFont.getName(), Font.PLAIN, 
        				font.getSize()));
        	}
        }
	...
	/**
	 * Zoom slider listener and scrollpane listener in case the viewer is in a
	 * scrollpane.
	 */
	@Override
	public void stateChanged(ChangeEvent e) {
		if (e.getSource() == zoomSlider) {
			//if (!zoomSlider.getValueIsAdjusting()) {
				...
				if (Math.abs(nmspp - msPerPixel) > 0.00000001f) { // i.e. if (nmspp != msPerPixel) {
					setMsPerPixel(nmspp);
				}
				zoomSlider.setToolTipText(String.valueOf(zoomValue));
			//}
		} else if (e.getSource() == viewPort) {
			// recalculate size and position
			try {
				if (viewPortLock.tryLock() || viewPortLock.tryLock(20, TimeUnit.MILLISECONDS)) {
					try {
						int oldPortH = imageHeight;
						imageWidth = viewPort.getExtentSize().width;
						imageHeight = viewPort.getExtentSize().height;
						int oldPortY = viewPortY;
						int oldPortX = viewPortX;
						viewPortX = viewPort.getViewPosition().x;
						viewPortY = viewPort.getViewPosition().y;
						verticalScrollOffset = 0;
						horizontalScrollOffset = 0;
						if (oldPortX != viewPortX) {
							setIntervalBeginTime(pixelToTime(viewPort.getViewPosition().x));
						}
						notifyMultiTierControlPanel();
						
						// update the position of the zoom slider if necessary					
						if (viewPortZoomSliderVisible && (oldPortX != viewPortX)
								|| (oldPortY != viewPortY || oldPortH != imageHeight)) {
							zoomSliderPanel.setBounds(
									viewPort.getExtentSize().width - ZOOMSLIDER_WIDTH + viewPort.getViewPosition().x,
									viewPort.getExtentSize().height - defBarWidth + viewPort.getViewPosition().y,
									ZOOMSLIDER_WIDTH, defBarWidth);
							zoomSlider.setBounds(0, 0, ZOOMSLIDER_WIDTH, defBarWidth);
						}

						if (oldPortY != viewPortY && useBufferedImage) {
							paintBuffer();
						} else {
							repaint(viewPortX, viewPortY, imageWidth, imageHeight);
						}
						
						if (editBox.isVisible() && editBoxRemainsInView) {
							// the x coordinate is updated in setLocalTimeScaleIntervalBeginTime
							int ny = editBox.getY();
							if (oldPortY != viewPortY && getActiveAnnotation() != null) {
								int tierIndex = getTierIndexForAnnotation(getActiveAnnotation());
								ny = (rulerHeight + (tierIndex * pixelsForTierHeight)) - verticalScrollOffset;
							}
							// add an update of the inline-editor location to the AWT event queue
							final int nny = ny;
							EventQueue.invokeLater(() -> {
								updateEditBoxLocation(editBox, editBox.getX(), nny);
							});
						}
					...
					if (LOG.isLoggable(Level.FINE)) {
						LOG.log(Level.FINE, "Failed to acquire the viewport scroll lock");
					}
				}
			} catch (InterruptedException ie) {
				// ignore
	...
	public void componentResized(ComponentEvent e) {
        ...
        if (!viewPortMode) {
        	...
        	if (viewPortZoomSliderVisible) {
        		if (viewPort.getExtentSize().width > 2 * ZOOMSLIDER_WIDTH) {
	        		if (!zoomSliderPanel.isVisible()) {
	        			zoomSliderPanel.setVisible(true);
	        		}
        			
        			zoomSliderPanel.setBounds(viewPort.getExtentSize().width - ZOOMSLIDER_WIDTH + 
...
//        	splitTime =  (int) (p.x * msPerPixel);
        ...
        if ((e.getClickCount() == 1) && e.isShiftDown()) {
            ...
            if (getSelectionBeginTime() != getSelectionEndTime()) {
        		...
        		(e.isControlDown() && !SystemReporting.isMacOS()))) {
        	// open an external resource by cmd/ctrl+click
            Point inverse = new Point(pp);
            dismissEditBox();
            if (!viewPortMode) {
                inverse.x += timeToPixels(intervalBeginTime);
            ...
            Tag2D selTag = getTagAt(inverse, getTierIndexForPoint(inverse));
            if (selTag != null) {
            	AbstractAnnotation ann = (AbstractAnnotation) selTag.getAnnotation();
            	if (UrlOpener.hasAnyExternalResourceRef(ann)) {
            		ELANCommandFactory.createCommand(transcription, 
            				ELANCommandFactory.SHOW_IN_BROWSER).execute(null, new Object[]{ann});
            	}
            }
        } else {
            Point inverse = new Point(pp);
        ...
        if (e.isAltDown() && timeRulerVisible &&
            ...
            if ((cursorTag2D != null) && (dragEditTag2D == null)) {
                if (getHoverTag(new Point(e.getPoint())) == cursorTag2D && 
                		cursorTag2D.getAnnotation() instanceof AlignableAnnotation) {
...
//                int x = (int) ((dragEditTag2D.getBeginTime() / msPerPixel) -
//                    (intervalBeginTime / msPerPixel));
                int x = xAt (dragEditTag2D.getBeginTime()); 
                int x2 = x +
	...
	public void mouseDragged(MouseEvent e) {
        ...
        int rb = viewPortMode ? viewPort.getViewPosition().x + viewPort.getExtentSize().width : 
        	getWidth() - defBarWidth;
        int lb = viewPortMode ? viewPort.getViewPosition().x : 0;
        /*e.getPoint can be outside the image size*/
        if ((dragEndPoint.x <= lb) ||
                (dragEndPoint.x >= rb)) {
            stopScroll();
        ...
        //auto scroll first
        if ((dragEndPoint.x < lb + SCROLL_OFFSET) && (dragEndPoint.x > 0)) {
            /*
            ...
            if (scroller == null) {
                ...
                if ((dragStartPoint.x < lb + SCROLL_OFFSET) &&
                        (dragStartPoint.x > lb)) {
            ...
            return;
        } else if ((dragEndPoint.x > (rb - SCROLL_OFFSET)) &&
                (dragEndPoint.x < rb)) {
            /*
               long begin = intervalBeginTime + SCROLL_OFFSET * msPerPixel;
               setIntervalBeginTime(begin);
            ...
            if (scroller == null) {
                // if the dragging starts close to the edge call setSelection
                if ((dragStartPoint.x > (rb - SCROLL_OFFSET)) &&
                        (dragStartPoint.x < rb)) {
                    setSelection(dragStartTime, dragStartTime);
                }
     ...
     * 
     * When the viewer is in a ViewPort the scroll behavior of the scrollpane
     * takes precedence and interferes with the {@code Ctrl} zoom and {@code Alt}
     * scroll behavior. 
     *  
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
    	...
    	if (e.isControlDown()) {
    		...
    		// horizontal scrolling with track pad uses a mouse wheel event with Shift modifier   		
    		if (!viewPortMode) {
    	...
    	// in viewPort mode, let the scrollpane take care of vertical scrolling
    	if (!viewPortMode) {
//	        if (e.getUnitsToScroll() > 0) {
...
//	            scrollBar.setValue(scrollBar.getValue() - pixelsForTierHeight);
//	        }
	        scrollBar.setValue(scrollBar.getValue() + Integer.signum(
	        		e.getUnitsToScroll()) * pixelsForTierHeight);
    	}
    }
	...
	public void keyPressed(KeyEvent e) {
    	int downMask = InputEvent.ALT_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK | 
    			Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx();
        if ((e.getModifiersEx() & downMask) == downMask) {
            if (!showEmptySlots) {
                showEmptySlots = true;
                repaint();
            }
	...
	public void actionPerformed(ActionEvent e) {
    	if (e.getSource() == copyAnnoMI) {
    		...
    		setPreference("TimeLineViewer.TimeRulerVisible", Boolean.valueOf(timeRulerVisible), 
    				transcription);
    	} else if (e.getSource() == zoomSliderVisibleMI) {
    		viewPortZoomSliderVisible = zoomSliderVisibleMI.isSelected();
    		zoomSliderPanel.setVisible(viewPortZoomSliderVisible);
    		zoomSlider.setVisible(viewPortZoomSliderVisible);
    		if (viewPortZoomSliderVisible) {
				zoomSliderPanel.setBounds(viewPort.getExtentSize().width - ZOOMSLIDER_WIDTH + 
    					viewPortX, 
    					viewPort.getExtentSize().height - defBarWidth + viewPortY, 
    					ZOOMSLIDER_WIDTH, defBarWidth);
				zoomSlider.setBounds(0, 0, ZOOMSLIDER_WIDTH, defBarWidth);
    		}
    		//paintBuffer();//??
    		setPreference("TimeLineViewer.ZoomSliderVisible", Boolean.valueOf(viewPortZoomSliderVisible), 
    				transcription);
    	} else if (e.getSource() == editBoxAlwaysInViewMI) {
            editBoxRemainsInView = editBoxAlwaysInViewMI.isSelected();
    		setPreference("TimeLineViewer.EditBoxAlwaysInView", Boolean.valueOf(editBoxRemainsInView), 
    				transcription);
        } 
	...
	public void preferencesChanged() {
		...
		Boolean zoomSliderVis = Preferences.getBool("TimeLineViewer.ZoomSliderVisible",
				transcription);
		if (zoomSliderVis == null) {
			zoomSliderVis = Preferences.getBool("TimeLineViewer.ZoomSliderVisible",
					null);
		}
		if (zoomSliderVis != null) {
			viewPortZoomSliderVisible = zoomSliderVis.booleanValue();
			zoomSliderPanel.setVisible(viewPortZoomSliderVisible);
			if (zoomSliderVisibleMI != null) {
				zoomSliderVisibleMI.setSelected(viewPortZoomSliderVisible);
			}
		}
		...
		Boolean editBoxViewPref = Preferences.getBool("TimeLineViewer.EditBoxAlwaysInView", transcription);
		if (editBoxViewPref != null) {
			editBoxRemainsInView = editBoxViewPref.booleanValue();
		}
		// preferred fonts
...
*/
	...
	public void hostMediaDisplayer(Object[] arguments, Rectangle sourceBounds) {
        ...
        if(mediaDisplayer != null) {
           ...
           // Determine whether the tag2d is above or below the center
            ...
            int viewportCenterY = viewPortMode ? (viewPortY + viewPort.getExtentSize().height / 2) 
            		: imageHeight/2;
            ...
            int viewportCenterX = viewPortMode ? (viewPortX + viewPort.getExtentSize().width / 2)
            		: imageWidth/2;
	...
	//###### Scrollable implementation and related methods for the situation in     ####
	//###### which the viewer is in a scrollpane and doesn't use its own scrollbars ####
	
	@Override
	public Dimension getPreferredScrollableViewportSize() {
		// same as in signal viewer
	...
	public int getScrollableUnitIncrement(Rectangle visibleRect, int orientation, int direction) {
		if (orientation == SwingConstants.VERTICAL) {
			if (pixelsForTierHeight > 4) {
				return pixelsForTierHeight / 4;
			} else {
				return 10;
			}
		} else {
	...
	public int getScrollableBlockIncrement(Rectangle visibleRect, int orientation, int direction) {
		if (orientation == SwingConstants.VERTICAL) {
			if (viewPort != null) {
				return viewPort.getExtentSize().height - rulerHeight;
			}
		} else {
			if (viewPort != null) {
				return viewPort.getExtentSize().width;
	...
	/**
	 * Checks if the parent is a JViewPort (i.e. if we are in s scroll pane)
	 * and removes the custom scroll bars.<p>
	 * 
	 * This is still based on a hybrid implementation, where the viewer can be
	 * added to a panel or container and maintains its own scroll bars or can
	 * be added to a scroll pane and interacts with that scroll pane when size
	 * and location are or need to be updated. Eventually a switch scroll pane
	 * only mode could be considered and a clean up of code would be possible.
	 */
	@Override
	public void addNotify() {
		...
		if (getParent() instanceof JViewport) {
			...
			remove(hScrollBar);
			if (hScrollBarVisMI != null) {
				hScrollBarVisMI.setVisible(false);
			}
			if (!viewPortZoomSliderVisible) {
				zoomSlider.removeChangeListener(this);
				remove(zoomSliderPanel);
			}
			// not removing the mousewheel listener disables vertical scrolling
...
//			if (viewPort.getParent() instanceof JScrollPane) {
//				((JScrollPane) viewPort.getParent()).removeMouseWheelListener(this);
//			}
		}
	}
	
	/**
	 * Make a distinction between the size when in a scroll pane and when used
	 * "standalone".
	 * 
	 * @return when in a scroll pane returns the required width for the entire duration 
	 * of the main media file (or of the collection of annotations) given the current 
	 * zoom level (plus some extra) and the required height for all visible tiers.
	 * Otherwise return the preferred size as it is set by the layout manager.
	 */
	@Override
	public Dimension getPreferredSize() {
		if (viewPortMode) {
			int h = Math.max(getViewHeight(), (rulerHeight + visibleTiers.size() * pixelsForTierHeight));
			return new Dimension((int) (
					(getMediaDuration() / msPerPixel) + (getViewWidth() / 2f)), 
						h);
		}

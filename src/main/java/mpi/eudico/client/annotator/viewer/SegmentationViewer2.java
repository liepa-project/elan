...
MultiTierViewer, AdjustmentListener, ACMEditListener, ChangeListener {
    ...
    private static final int ZOOMSLIDER_WIDTH = 100;
    ...
    private static final int DRAG_EDIT_MARGIN = 8;
    private static final int DRAG_EDIT_CENTER = 0;
    private static final int DRAG_EDIT_LEFT = 1;
    private static final int DRAG_EDIT_RIGHT = 2;
	...
	// a hard-coded action
	private AbstractAction deleteAction2;
	private static final String deleteKey2 = "DeleteAction2";
	...
	private void initViewer2() {
        ...
        deleteAction2 = new DeleteAnnotationAction(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
        
        mainInputMap.put((KeyStroke) deleteAction2.getValue(Action.ACCELERATOR_KEY), deleteKey2);
        mainActionMap.put(deleteKey2, deleteAction2);
        getInputMap(WHEN_FOCUSED).put((KeyStroke) deleteAction2.getValue(Action.ACCELERATOR_KEY), deleteKey2);
        getInputMap(WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put((KeyStroke) deleteAction2.getValue(Action.ACCELERATOR_KEY), deleteKey2);
        // initially add the default segmentation action
		String key = ELANCommandFactory.SEGMENT;
        mainInputMap.put((KeyStroke) segmentTypeAction.getValue(Action.ACCELERATOR_KEY), key);
        mainActionMap.put(key, segmentTypeAction);
        actionMap.put(key, segmentTypeAction);
	...
	private void updateShortcutMaps(){
		...
		InputMap rootInMap = null;
		ActionMap rootActionMap = null;
		if (getRootPane() != null) {
			rootInMap = getRootPane().getInputMap(WHEN_IN_FOCUSED_WINDOW);
			rootActionMap = getRootPane().getActionMap();
		}
		...
		while(it.hasNext()){
			...
			if(ca != null){
	    		...
	    		if (rootInMap != null && rootActionMap != null) {
	    			rootInMap.put((KeyStroke) ca.getValue(Action.ACCELERATOR_KEY), actionName);
	    			rootActionMap.put(actionName, ca);
	    		}
			}			
		}
		// special case for DELETE key
		if (rootInMap != null && rootActionMap != null && rootActionMap.get(deleteKey2) == null) {			
			rootInMap.put((KeyStroke) deleteAction2.getValue(Action.ACCELERATOR_KEY), deleteKey2);
			rootActionMap.put(deleteKey2, deleteAction2);
		}
	}
	
	/**
	 * Ensure key strokes and actions are added to the root pane's maps.
	 */
    @Override
...
//        	editTier2d = allTiers.get(0);
    ...
    private void paintBuffer() {
        ...
        synchronized (paintlock) {
	        ...
	        big2d.setColor(Constants.DARK_MODE ? Constants.DEFAULTBACKGROUNDCOLOR :
	...
	//                    (float) (y + ((height / 2) - descent + 1)));
	        ...
	        big2d.setColor(Constants.DARK_MODE ? Constants.DEFAULTFOREGROUNDCOLOR :
    ...
    private void paintUnbuffered(Graphics2D g2d) {
        ...
        g2d.setColor(Constants.DARK_MODE ? Constants.DEFAULTBACKGROUNDCOLOR :
        	Constants.DEFAULTFOREGROUNDCOLOR);
        int height = pixelsForTierHeight - (2 * pixelsForTierHeightMargin);
        y = rulerHeight + pixelsForTierHeightMargin + (pixelsForTierHeight / 2);
...
//                    (float) (y + ((height / 2) - descent + 1)));
        ...
        g2d.setColor(Constants.DARK_MODE ? Constants.DEFAULTFOREGROUNDCOLOR :
        	Constants.SHAREDCOLOR6);
        y = rulerHeight + editTierHeight + pixelsForTierHeightMargin - verticalScrollOffset;
        // iterate over tiers
        for (int i = 0; i < visibleTiers.size(); i++) {
        	Tier2D tier2d = visibleTiers.get(i);
...
//        }       
	...
	/**
     * Sets the control panel for this viewer, ignored here.
     *
    ...
    /**
     * Sets the {@code SegmentationControlPanel} for this viewer.
     * 
     * @param segment the control panel for this viewer
     */
    ...
    private void switchActions() {
    	...
    	if (segmentMode == TWO_TIMES_PRESS_RELEASE_SEGMENTATION) {
    		...
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.remove((KeyStroke) segmentTypeAction.getValue(Action.ACCELERATOR_KEY));
    			rootActionMap.remove(ELANCommandFactory.SEGMENT);
    		}
    		...
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.put((KeyStroke) segmentPressAction.getValue(Action.ACCELERATOR_KEY), key);
    			rootActionMap.put(key, segmentPressAction);
    		}
    		...
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.put((KeyStroke) segmentReleaseAction.getValue(Action.ACCELERATOR_KEY), key);
    			rootActionMap.put(key, segmentReleaseAction);
    		}
    	} else {
    		String key = "SegDown";
    		...
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.remove((KeyStroke) segmentPressAction.getValue(Action.ACCELERATOR_KEY));
    			rootActionMap.remove(key);
    		}
    		...
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.remove((KeyStroke) segmentReleaseAction.getValue(Action.ACCELERATOR_KEY));
    			rootActionMap.remove(key);
    		}
            ...
            getInputMap(WHEN_ANCESTOR_OF_FOCUSED_COMPONENT).put((KeyStroke) segmentTypeAction.getValue(Action.ACCELERATOR_KEY), key);
    		if (rootInMap != null && rootActionMap != null) {
    			rootInMap.put((KeyStroke) segmentTypeAction.getValue(Action.ACCELERATOR_KEY), key);
    			rootActionMap.put(key, segmentTypeAction);
    		}
    	}
    	if (editTier2d != null) {
    		setActionsForTier(editTier2d.getTier());
    	}
    }
	 ...
	 *  
	 * @param nextDelayDuration the new delay value
	 */
	public void setDelayDuration(long nextDelayDuration) {
		delayDuration = nextDelayDuration;
		if (delayDuration < 0) {
        ...
        if (segmentMode == TWO_TIMES_SEGMENTATION) {
            if (cur != lastSegmentTime) {
                ...
                if ((timeCount % 2) != 0) {
                	...
                	if (cve != null) {
                		...
                		if (cur > lastSegmentTime) {
                			adr = new AnnotationDataRecord("", cve.getValue(
                					cve.getParent().getDefaultLanguageIndex()), lastSegmentTime, cur);
                		} else {
                			adr = new AnnotationDataRecord("", cve.getValue(
                					cve.getParent().getDefaultLanguageIndex()), cur, lastSegmentTime);
                		}
                		adr.setCvEntry(cve);
        	...
        	if (cve != null) {
                ...
                if (singleStrokeIsBegin) {
                    adr = new AnnotationDataRecord("", cve.getValue(
                    		cve.getParent().getDefaultLanguageIndex()), cur, cur + fixedDuration);
                } else {
                    ...
                    adr = new AnnotationDataRecord("", cve.getValue(
                    		cve.getParent().getDefaultLanguageIndex()), bb, cur);
                }
        		adr.setCvEntry(cve);
     ...
     * Might need revision? 
     * 
     * @param cve a {@code CVEntry} instance or {@code null}
     ...
     */
    private void addPressReleaseTime(CVEntry cve, boolean release) {
        long cur = getMediaTime() - delayDuration;
        if (cur < 0) {
        	cur = 0;
        }
...
//        }
    ...
    /**
     * Sets the {@code local} begin time of the visible interval (i.e. without 
     * notifying the {@code global} time scale object connecting multiple 
     * viewers).
     */
	@Override
    ...
    /**
     * Sets the {@code local} resolution or number of milliseconds per pixel 
     * (i.e. without notifying the {@code global} time scale object connecting
     * multiple viewers).
     */
	@Override
	...
	/*
	 * Called on the AWT Event Dispatching Thread from the super implementation
	 * of {@code controllerUpdate(ControllerEvent)} method.
	 * 
...
//            System.out.println("IE time: " + intervalEndTime);
    ...
    /**
     * Updates user interface elements and notifies the control panel.
     *
	...
	/**
	 * Notification that shortcuts have been changed.
	 */
            	...
            	if (cv != null) {
            		...
            		for (CVEntry e : cv) {
            			...
            			if (segmentMode != TWO_TIMES_PRESS_RELEASE_SEGMENTATION) {
            				...
            				PressReleaseSegmentAction spa = new PressReleaseSegmentAction(e, false);
            				String nextId = "dcve-" + i;
	            			cvInputMap.put((KeyStroke) spa.getValue(Action.ACCELERATOR_KEY), nextId);
	            			cvActionMap.put(nextId, spa);
	            			
	            			PressReleaseSegmentAction sra = new PressReleaseSegmentAction(e, true);
	            			nextId = "ucve-" + i;
	            			cvInputMap.put((KeyStroke) sra.getValue(Action.ACCELERATOR_KEY), nextId);
	            			cvActionMap.put(nextId, sra);
            			}
            			i++;
            		}
            	}
	...
	/**
	 * Activates the next or previous tier in the list of visible tiers.
	 * 
	 * @param down if {@code true} activates the next tier in the list, 
	 * otherwise activates the previous tier
	 */
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource() == zoomSlider) {
			//if (!zoomSlider.getValueIsAdjusting()) {
				...
				if (Math.abs(nmspp - msPerPixel) > 0.00000001f) { // i.e. (nmspp != msPerPixel)
					setMsPerPixel(nmspp);
				}
...
//        }
    ...
    /**
     * Merge an annotation with the next or previous annotation.
     * 
     * @param next if {@code true} merge with next annotation, otherwise merge
    ...
    /**
     * Splits an annotation at the current split time, as set by the user.
     */
    ...
    /**
     * Shows a modify annotation time dialog for a selected annotation.
     */
	...
	//#### addition for key press key release segmentation ####
	/**
	 * Action triggered by a key press (down) or key release (up) event.
	 */
	public class PressReleaseSegmentAction extends AbstractAction {
		private boolean up;
		
		/**
		 * Creates a new segment action.
		 * 
		 * @param up if {@code true} it triggers key released events, otherwise
		 * key pressed events
		 */
		...
		/**
		 * Creates a new segment action with a controlled vocabulary entry with
		 * a keyboard shortcut as parameter.
		 * 
		 * @param cve a {@code CVEntry} object, should not be {@code null} and 
		 * should have a keyboard shortcut defined
		 * @param up if {@code true} it triggers key released events, otherwise
		 * key pressed events
		 */
		public PressReleaseSegmentAction(CVEntry cve, boolean up) {
			this.up = up;
			putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(cve.getShortcutKeyCode(), 0, up));
...
//			System.out.println("Segment " + (up ? "Up" : "Down") + "   Is key down: " + pressReleaseKeyDown);
			...
			if (val instanceof CVEntry) {
				SegmentationViewer2.this.addPressReleaseTime((CVEntry) val, up);
			} else {
				SegmentationViewer2.this.addPressReleaseTime(null, up);
			}	
		}
	...
	/**
     * An action class to handle the enter-key-typed (or whatever key has been
     * set as segmentation key) event.
     *
     * @author Han Sloetjes
     */
    public class SegmentAction extends AbstractAction {
        /**
         * Constructor, sets the accelerator key to the VK_ENTER key or to the
         * key set as a preference by the user.
         */
        public SegmentAction() {        	
            putValue(Action.ACCELERATOR_KEY, ShortcutsUtil.getInstance().getKeyStrokeForAction(
            		ELANCommandFactory.SEGMENT, ELANCommandFactory.SEGMENTATION_MODE));              
            putValue(Action.DEFAULT, null);
        }
        
        /**
         * Constructor with a controlled vocabulary entry as parameter.
         * 
...
//        	System.out.println("Segment action");
	...
	public class ActiveTierAction extends AbstractAction {
		 ...
		 * Constructor.
		 * 
		 * @param down if {@code true} the direction of activation is down, 
		 * if {@code false} the direction is up
		 */
	...
	public class DeleteAnnotationAction extends AbstractAction {
		...
		/**
		 * Creates a new delete action with default shortcut key.
		 */
		...
		/**
		 * Creates a new delete action with the specified shortcut key.
		 * 
	...
	public class MergeAnnotationAction extends AbstractAction {
		...
		/**
		 * Constructor.
		 * @param next if {@code true} merge with next annotation,
	...
	/**
	 * An action that shows a modify times dialog for the active annotation.
	 */

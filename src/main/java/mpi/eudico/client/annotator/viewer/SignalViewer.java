...
public class SignalViewer extends TimeScaleBasedViewer
    ...
    MouseWheelListener, ActionListener, Scrollable {
     ...
     * current interval */
    private static final int SCREEN_BUFFER = 1;

    private WAVSamplesProvider samp;

    /** The initial number of milliseconds per pixel */
    public static final int DEFAULT_MS_PER_PIXEL = 10;
    ...
    public static final int SCROLL_OFFSET = 16;
    ...
    private static final int MAX_READ_NUM_SAMPLES = (int) Math.pow(2, 22); //power 22 = 4194304, 23 = 8388608, 24 = 16777216
...
//        }
     ...
     */
    public void setMediaDescriptor(MediaDescriptor md) {
    	if (md != null) {
    ...
    private void initSampler(String sourcePath) {
        ...
        try {
			...
			if (compr != WAVHeader.WAVE_FORMAT_UNCOMPRESSED && 
            		...
            		compr != WAVHeader.WAVE_FORMAT_IEEE_FLOAT) {
...
//        	System.out.println("Wave part ET: " + currentPart.getStopTime());
        ...
        //paint the background color
        conG2d.setComposite(AlphaComposite.Src);
        conG2d.setColor(Constants.LIGHTBACKGROUNDCOLOR);
        conG2d.fillRect(scrollViewBX, 0, contextWidth, contextHeight);
...
//          ALBERT
        ...
        // draw a line at the end of the media time
        if (intervalEndTime > getMediaDuration()) {
            int xx = xAt(getMediaDuration()) + waveIntervalX1;
            conG2d.setComposite(AlphaComposite.Src);
            conG2d.setColor(Color.LIGHT_GRAY);
            conG2d.drawLine(xx, 0, xx, contextHeight);
        }
...
//        }
    ...
    */
    ...
    /**
     * Returns whether the viewer is in the Transcription mode layout.
     * 
     * @return {@code true} if the viewer is part of the Transcription mode
	...
	/**
	 * Sets whether the viewer is in the Transcription mode layout.
	 * 
	 * @param loadTranscriptionMode if {@code true} the viewer is part of the
	 * Transcription mode
	 */
    ...
    private void setLocalTimeScaleIntervalBeginTime(long begin) {
        ...
        try {
        	if (paintLock.tryLock() || paintLock.tryLock(paintLockTimeOut, TimeUnit.MILLISECONDS)) {
		        try {
...
//    	}
    ...
    private void updateVertZoomPopup( int vertZoom) {
        if (popup != null) {
            Enumeration<AbstractButton> en = vertZoomGroup.getElements();
            while (en.hasMoreElements()) {
            	AbstractButton rbmi = en.nextElement();

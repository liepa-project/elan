...
public class TextViewer extends AbstractViewer implements SingleTierViewer,
    ...
    public TextViewer() {
        ...
        try {
            ...
            fontSize = Constants.DEFAULTFONT.getSize();
...
//            ex.printStackTrace();
        ...
        try {
            ...
            for (Highlight element : h_arr) {
                if (element.getPainter() == currentPainter) {
					...
					Rectangle2D rect = taText.modelToView2D(idx);
					Rectangle2D endRect = taText.modelToView2D(ide);
					if (rect == null || endRect == null) {
						return;
					}
					Rectangle2D union2D = rect.createUnion(endRect);
					Rectangle union = new Rectangle();
					union.setRect(union2D);
    ...
    private void updateAnnotations() {
        ...
        // note: updateAnnotations already runs on the AWT EventThread, so invokeLater() may not be necessary here
	...
	private void paintSpellErrorUnderline() {
		...
		if (tier == null || annotations == null || annotations.size() == 0) {
			return;
		}
	...
	public void setTier(Tier tier) {
		...
		// call doUpdate after the TextArea has finished updating itself
        // note: setTier already runs on the AWT EventThread, so invokeLater() may not be necessary here
		SwingUtilities.invokeLater(new Runnable(){
			@Override
			public void run() {
				doUpdate();
		...
		public void mouseReleased(MouseEvent e) {
            ...
            if (!taText.getText().equals("")) {
                ...
                if ((selectionStartPosition == selectionEndPosition) &&
                    ...
                    index = (int) Math.ceil((double)indexSelectionStart / 2);

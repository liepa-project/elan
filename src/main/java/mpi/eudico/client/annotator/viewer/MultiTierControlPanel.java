...
public class MultiTierControlPanel extends JPanel implements MouseListener,
     ...
     */
    private static final String TIER_AC = "@&@";
    
    /**
     * Add a marker to the action command of a tier to avoid collisions in  the
     * handling of menuitem's action events
     */
    private static final String TIERSET_AC = "@TS@";
        ...
        for (MenuElement item : items) {
            if (item instanceof JCheckBoxMenuItem) {
        		...
        		if (visTiers.contains(transcription.getTierWithId(tierName))){
        			...
        			checkBoxMI.setForeground(Constants.DEFAULTFOREGROUNDCOLOR);
...
//    }
	...
	/**
	 * Creates a lighter variant of label colors for a {@code dark} theme.
	 * @param c the original color
	 * @return a lighter variant of the color
	 */
	private Color toThemeColor(Color c) {
		if (Constants.DARK_MODE) {
			// this is probably a too simplistic transformation
			return new Color(
					Math.min(c.getRed() + 128, 255), 
					Math.min(c.getGreen() + 128, 255), 
					Math.min(c.getBlue() + 128, 255));
		}
		return c;
	}
...
//    		}
    ...
    public void isClosing(){		
		...
		for (Map.Entry<TierImpl, Color> prefEntry : prefTierColors.entrySet()) {
			prefMap.put(prefEntry.getKey().getName(), prefEntry.getValue());
		}
    ...
    /**
     * A comparator class that compares two tiers base on their name, ignoring
     * case.
     */
    public static class TierComparer implements Comparator<TierImpl>, Serializable {
		private static final long serialVersionUID = 4220005121638049231L;
    ...
    /**
     * Activates or selects the next tier set, if it exists, selects the first
     * set otherwise.
     */

...
/**
 * A class that provides and configures user interface components for  editing annotation values. Elan Viewer components that
 * offer the possibility of editing annotation values can use this class to get a suitable editor component. Depending on the
 * language (locale) of the annotation it will activate  special input method components.
 *
 * @author MPI
 * @version sep 2005 when an annotation's Locale is the system default and the the edit box's is also the system default, the
 *     language isn't set anymore. This way it is possible to use other system specific im's / keyboards on MacOS as well.
 */
...
public class InlineEditBox extends JPanel implements ActionListener,
    ...
    /**
     * a focus listener that lets the attached-mode textarea request the keyboard focus
     */
    ...
    /**
     * a focus listener that lets the detached-mode textarea request the keyboard focus
     */
    ...
    private final JPopupMenu popupMenu = new JPopupMenu("Select Language");
    ...
    private final List<KeyStroke> keyStrokesNotToBeConsumed = new ArrayList<KeyStroke>();
    private final List<KeyStroke> defaultRegisteredKeyStrokes = new ArrayList<KeyStroke>();
    /**
     * this field can be either a JPanel (this), a JScrollPane, a JTextArea , a JComboBox or any other component that can be
     * added to the layout of a viewer (component)
     */
    ...
    static {
    	...
    	int numberOfSpellCheckers = SpellCheckerRegistry.getInstance().getSpellCheckerCount();
    	if (LOG.isLoggable(Level.INFO)) {
    		LOG.log(Level.INFO, "Number of available spell checkers: " + numberOfSpellCheckers);
    	}
    }

    /**
     * When this editor is not created by a viewer, it will always be created as a  "detached" dialog.
     *
     * @param attachable whether or not this editor can be attached to a viewer component.
     */
     ...
     *
     * @param ksList list of keystrokes
     */
    ...
    public void init() {
        ...
        textArea.addFocusListener(new FocusAdapter() {
            @Override
            public void focusGained(FocusEvent e) {
                if (annotationLocale != null && !annotationLocale.equals(Locale.getDefault()) && !annotationLocale.equals(
                    textArea.getLocale())) {
    ...
    public void createExternalDialog() {
        try {
            externalDialog = new JDialog(ELANCommandFactory.getRootFrame(annotation.getTier().getTranscription()),
                                         ElanLocale.getString("InlineEditBox.Title"),
                                         true);
            ...
            toggleSuggestMI.setVisible(isUsingControlledVocabulary);
    ...
    public void createPopupMenu() {
        ...
        detachPUMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, ActionEvent.SHIFT_MASK));
        ...
        pastePUMI = new JMenuItem(ElanLocale.getString("InlineEditBox.Edit.Paste"));
    ...
    private JMenuBar createJMenuBar() {
        ...
        selectLanguageMenu = new JMenu(ElanLocale.getString("InlineEditBox.Menu.Select"));

        if (attachable) {
            attachMI = new JMenuItem(ElanLocale.getString("InlineEditBox.Attach"));
            attachMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, ActionEvent.SHIFT_MASK));
        ...
        toggleSuggestMI = new JMenuItem(ElanLocale.getString("InlineEditBox.ToggleSuggestPanel"));
    ...
    /**
     * Returns whether or not the current annotation's value is restricted by a ControlledVocabulary.
     *
    ...
    /**
     * Overrides setSize(Dimension) in Component by performing a check to guarantee a minimal  size.
     *
    ...
    /**
     * Overrides setSize(Dimension) in Component by performing no check to guarantee a minimal  size.
    ...
    /**
     * Overrides setFont(Font) in Component by also setting the font for the textareas.
     *
    ...
    /**
     * TEMP!! A method to get the width of the current textual contents of the (attached) text area. For testing "live"
     * resizing while typing.
     *
    ...
    /**
     * Sets the annotation that is to be edited. When <code>forceOpenCV</code> is true an 'open' text edit box will be used
     * even if the linguistic type has an associated ControlledVocabulery.
     *
     * @param ann the annotation to be edited
     * @param forceOpenCV if true the associated CV will be ignored, editing will be open
     */
    public void setAnnotation(Annotation ann, boolean forceOpenCV) {
        ...
        try {
            ...
            if (forceOpenCV) {
                ...
                isUsingControlledVocabulary = annotation.getTier().getLinguisticType().isUsingControlledVocabulary();
            }
        } catch (Exception e) {
    ...
    public boolean annotationModified() {
        return attached ? (!oldText.equals(textArea.getText())) : (!oldText.equals(exttextArea.getText()));
    }

    /**
     * Paints the squiggly line underneath spell errors in the text area. To prevent unnecessary creation of a
     * {@code SwingWorker} for the spell check, a few shallow tests are performed.
     *
    ...
    private void paintSpellErrorUnderline(final JTextArea textAreaToCheck) {
        if (textAreaToCheck.getText().isEmpty()) {
            return;
        }

        if (SpellCheckerRegistry.getInstance().getSpellCheckerCount() == 0) {
            return;
        }

        String langRef = annotation.getTier().getLangRef();
        if (langRef == null || !SpellCheckerRegistry.getInstance().hasSpellCheckerLoaded(langRef)) {
            return;
        }
    ...
    /**
     * Sets internal TextArea (resp. "this") visible(false) and opens external TextArea (resp. "externalDialog")
     */
    public void detachEditor() {
        ...
        if (dialogBounds != null) {
            ...
            if (boundsPref != null) {
                ...
                externalDialog.setLocation((screenDim.width - frameDim.width) / 2, (screenDim.height - frameDim.height) / 2);
            }
        }
    ...
    /**
     * Returns the editor the editor to the previous attached state and dispose the external dialog.
     */
    ...
    /**
     * Resets elements and cleans up without applying any changes in the  value of the annotation.  HB, 11 oct 01, changed
    ...
    /**
     * Checks for modifications, applies the modification if any, resets and cleans  up a bit.
     */
                ...
                if (Character.isValidCodePoint(codePoint)) {
                    ...
                    if (bad) {
                        ...
                        JOptionPane.showMessageDialog(this,
                                                      ...
                                                      // "Text started with a character that is meant to modify the one
                                                      // before it.\n"
    ...
    /**
     * Restores the default locale (because of InputMethod stuff) of the textareas, unless the language hasn't been changed
     * when the annotation was set.
     */
    ...
    /**
     * Enables/disables edit menu items in the popup menu.<br> Check the contents of the <code>textArea</code> component and
    ...
    private void updatePopup() {
        ...
        pastePUMI.setEnabled(isTextOnClipboard());

        selectAllPUMI.setEnabled(textArea.getText() != null && textArea.getText().length() != 0);
    }
    ...
    private void updateMenuBar() {
        if ((exttextArea.getSelectedText() == null) || (exttextArea.getSelectedText().length() == 0)) {
        ...
        pasteMI.setEnabled(isTextOnClipboard());

        selectAllMI.setEnabled(exttextArea.getText() != null && exttextArea.getText().length() != 0);
    }

    /**
     * Checks whether the contents of the system clipboard can be paste into  a textcomponent.
     *
    ...
    /**
     * Returns the (configured) editor component, ready to be used in a viewer's  layout.
     *
    ...
    private Dimension getCVCompSize(Dimension size) {
        ...
        minCVHeight =
            cvEntryComp.getEditorComponent().getFontMetrics(cvEntryComp.getEditorComponent().getFont()).getHeight();
    ...
    /**
     * Makes the editorComponent visible and tries to grabFocus.<br> This should be called after configuring and getting the
     * editor component
     *
     * @see #configureEditor(Class, Font, Dimension)
     * @see #getEditorComponent()
    ...
    /**
     * M_P all e.consume() calls outcommented because the events are needed to deselect a newly  made / edited annotation in
     * EudicoAnnotationFrame with the Escape key M_P 25 june 2003 Just the first outcommented. The last when has to be
     * consumed. OS May 2016: not consuming ESC leads to undesired effects when this editbox is embedded in a table (or maybe
     * in a window that closes on ESC). Whoever wants a notification that editing has ended had better become an
     * InlineEditBoxListener. EudicoAnnotationFrame does not exist anymore so the previous must be rather out of date
     * anyway.
     *
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (keyStrokesNotToBeConsumed.contains(ks)) {
            ...
            //temp
            restoreOriValue = e.getKeyCode() == KeyEvent.VK_SPACE && ((e.getModifiersEx() & KeyEvent.SHIFT_DOWN_MASK) != 0
                                                                      // e.isShiftDown()
    ...
    public void updateLocale() {
        ...
        if (menuBar != null) {
            ...
            selectLanguageMenu.setText(ElanLocale.getString("InlineEditBox.Menu.Select"));
    ...
    /**
     * A class that provides a component for the selection of an entry from  a ControlledVocabulary.<br> The current possible
     * delegate components are a JScrollPane containing a JList or a JComboBox
     *
     * @author Han Sloetjes
     */
    class CVEntryComponent implements KeyListener,
                                      ActionListener,
                                      DocumentListener {
        /**
        ...
        /**
         * the component to use for editing, either a scrollpane containing a JList  or a JComboBox
         */
        private JComponent delegate;

        /**
         * the array of CV entries from the ControlledVocabulary referenced by the  LinguisticType in use by the Tier
         * containing the current annotation
         */
         ...
         * the argument.
         *
         * @param componentClass the type of component to use for edit operations
         */
        ...
        /**
         * Sets which type of component should be used for editing. Can depend on the kind of viewer that created the
         * InlineEditBox  and of the attached / detached state.
         *
        ...
        /**
         * Tries to ensure that the selected item is visible in the scrollpane's viewport. Applies only to the JList
         * component.
         */
        ...
        /**
         * When this list is in a detached dialog it doesn't need the popup because all options are in the menu bar. Applies
         * only to the JList component.
         */
        ...
        /**
         * When this list is not in a dialog menu items for detaching, committing and canceling need to be provided.  Applies
         * only to the JList component.
         */
        ...
        public void scrollIfNeededAutomatically() {
            ...
            Rectangle rect = entryTable.getCellRect(entryTable.getSelectedRow(), entryTable.getSelectedColumn(), true);
        ...
        /**
         * Initializes either a list in a scrollpane (with a popup menu etc) or a combo box.  Adds listeners.
         *
                ...
                if (entryTable == null) {
                    ...
                    entryTable.addMouseMotionListener(new MouseMotionListener() {
                        ...
                        public void mouseMoved(MouseEvent e) {
                            ...
                            if ((row > -1 && row < entryTable.getRowCount()) && (col > -1
                                                                                 && col < entryTable.getColumnCount())) {
        ...
        /**
         * Gets the entry array with the entries in the cv referenced by the linguistic type of the tier. HS Jan 2011: try to
         * re-use the (potentially long) list of cv entries Sep 2018: re-implementation of caching of the entries of the last
         * used (E)CV to prevent that the list of entries is not refreshed after a change of the (E)CV for a tier
         *
        ...
        /**
         * Fills the model of either the combo box or the list with the entries of the current Controlled Vocabulary.
         */
                ...
                if (reload) {
                    ...
                    if (cveId != null) {
                        for (int i = 0; i < entryBoxModel.getSize(); i++) {
                            SimpleCVEntry entry = entryBoxModel.getElementAt(i);
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getSource() == detachMI) {
                ...
                if ((ae.getID() == ActionEvent.ACTION_PERFORMED) && (ae.getModifiers() == InputEvent.BUTTON1_DOWN_MASK)) {
        ...
        private void createPopupMenu() {
            ...
            detachMI = new JMenuItem(ElanLocale.getString("InlineEditBox.Detach"));
            detachMI.addActionListener(this);
            detachMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, ActionEvent.SHIFT_MASK));
            popup.add(detachMI);
            commitMI = new JMenuItem(ElanLocale.getString("InlineEditBox.Commit"));
            commitMI.addActionListener(this);
            commitMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER,
        ...
        /**
         * Finds entries that (partially) match contents of the text field of the suggest panel. Reason for extending
         * Runnable: if the number of entries is very large, searching would interfere typing.
         *
         * <p>
            ...
            public void run() {
                ...
                if (part != null && !part.equals("")) {
                    ...
                    if (suggestIgnoreCaseFlag) {
                        part = part.toLowerCase(ElanLocale.getLocale());
                    }
                    while (entriesIndex < entries.length && !Thread.currentThread().isInterrupted()) {
        ...
        private String getDescription(SimpleCVEntry next) {
            ...
            if (d == null) {
                 ...
                 * <b>Task Creation Date:</b> 2023-10-27 <br />
                 * <b>Description:</b> Do we want to use Elan's locale or CV's language specific locale?<br />
                 * Han: ELAN's Locale is fine here, I think.
                 */
                return d.toLowerCase(ElanLocale.getLocale());
            }
        }
    ...
    /**
     * Sets media displayer host
     *
     * @param mediaDisplayerHost the media displayer host
     */

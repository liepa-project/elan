...
import mpi.eudico.client.annotator.lexicon.*;
...
public class EditLexSrvcDialog extends ClosableDialog implements ActionListener,
                                                                 ItemListener {
    ...
    private void warnIfNoExtensions() {
        if (transcription.getLexiconServiceClientFactories() == null || transcription.getLexiconServiceClientFactories()
                                                                                     .isEmpty()) {
            addServiceButton.setEnabled(false);
            JOptionPane.showMessageDialog(this,
                                          ElanLocale.getString("LexiconServiceClient.NoClient"),
                                          "Warning",
    ...
    private void updateUIforSelectedService() {
        if (serviceNameBox.getSelectedIndex() > -1) {
            ...
            serviceInfoText.setText(typeStr
                                    + ": "
                                    + link.getLexSrvcClntType()
                                    + "\n"
                                    + urlStr
                                    + ": "
                                    + link.getUrl()
            ...
            editServiceButton.setEnabled(link.getSrvcClient() != null);
    ...
    /**
     * Checks whether there are service configurations stored on the system, loads them if there are any and enables the
     * import button.
     */
    ...
    private void importServices() {
        ...
        for (LexiconQueryBundle2 bundle : selBundles) {
            ...
            if (!alreadyThere) {
                ...
                Command com = ELANCommandFactory.createCommand(transcription, ELANCommandFactory.ADD_LEX_LINK);
    ...
    private void deleteService(LexiconLink link) {
        Command com = ELANCommandFactory.createCommand(transcription, ELANCommandFactory.DELETE_LEX_LINK);
        Object[] args = new Object[1];
        args[0] = link;

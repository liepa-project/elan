...
/**
 * Extends the {@code EditCVPanel} with a popup for input methods and activates data category related UI elements.
 */
...
public class ElanEditCVPanel extends EditCVPanel {
        ...
        entryValueTextField.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent event) {
                if (SwingUtilities.isRightMouseButton(event) || event.isPopupTrigger()) {
                    ...
                    if (popup != null) {
                        popup.show(entryValueTextField, event.getX(), event.getY());
    ...
    /**
     * Set the DCR UI elements visible. Set the table cell renderer.
     *
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if (source instanceof JMenuItem) {
            ...
            if (currentEntry != null) {
                Window ancestor = SwingUtilities.getWindowAncestor(this);
                if (ancestor instanceof JDialog) {
                    CVEntryOptionsDialog dialog =
                        new CVEntryOptionsDialog((JDialog) ancestor, true, currentEntry, getSelectedColumn());
    ...
    protected void updateLabels() {
        moveToTopButton.setToolTipText(ElanLocale.getString("EditCVDialog.Button.Top"));
        moveUpButton.setToolTipText(ElanLocale.getString("EditCVDialog.Button.Up"));
        moveDownButton.setToolTipText(ElanLocale.getString("EditCVDialog.Button.Down"));
        moveToBottomButton.setToolTipText(ElanLocale.getString("EditCVDialog.Button.Bottom"));
        ...
        entryDescLabel.setText(ElanLocale.getString("EditCVDialog.Label.EntryDescription"));
        ...
        invalidValueMessage = ElanLocale.getString("EditCVDialog.Message.EntryValidValue");
        valueExistsMessage = ElanLocale.getString("EditCVDialog.Message.EntryExists");
    ...
    protected void addEntry() {
        ...
        if (cv.containsValue(language, entry)) {
            ...
            if (dcrField.getText() != null && dcIdField.getText().length() > 0 /*&& currentDC != null*/) {
                //newEntry.setExternalRef(currentDC.getId()); // or be on the safe side and check the contents of the
                // field?
            ...
            if ((newDescription != null) && !newDescription.equals(currentEntry.getDescription(language))) {
                ...
                if (dcId != null && dcId.length() > 0) {
                    ...
                    if (currentEntry.getExternalRef() instanceof ExternalReferenceImpl) {
                        if (!dcId.equals(currentEntry.getExternalRef().getValue())) {
    ...
    protected void updateTextFields() {
        ...
        if (selEntry == null) {
            ...
            //CVEntry selEntry = getSelectedEntry();
            if (selEntry.getExternalRef() instanceof ExternalReferenceImpl eri) {
            ...
            for (CVEntry cve : cv) {
                ...
                if (cve.getShortcutKeyCode() == copyEntry.getShortcutKeyCode() && cve != currentEntry) {
                    ...
                    int option = JOptionPane.showOptionDialog(this,
                                                              ElanLocale.getString("EditCVDialog.Message.ShortcutUsed")
                                                              + " "
                                                              + cve.getValue(0),
                                                              "ELAN",
                                                              ...
                                                              null,
                                                              new String[] {ElanLocale.getString(
                                                                  "EditCVDialog.Message.ShortcutChange"),
                                                                            ElanLocale.getString(
                                                                                "EditCVDialog.Message.ShortcutDontChange")},
                                                              ElanLocale.getString("EditCVDialog.Message.ShortcutChange"));

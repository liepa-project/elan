...
/**
 * A pane with a message label and a table with 2 columns; a checkbox column and a column with strings identifying the
 * transcriptions or the windows. The (string) values of the checked rows will be returned by getSelectedValues().
 *
...
public class ChangedTranscriptionsPane extends JPanel implements ListSelectionListener {
     ...
     *
     * @param changedTrans string representation of the transcriptions (name, path, url) or their windows
     */
    ...
    private void initComponents() {
        messageLabel =
            new JLabel("<html>" + ElanLocale.getString("Frame.ElanFrame.UnsavedMultiple1") + "<br>" + ElanLocale.getString(
                "Frame.ElanFrame.UnsavedMultiple2") + "</html>");
        ...
        transTable.getColumnModel().getColumn(0).setCellEditor(new DefaultCellEditor(new JCheckBox()));
        transTable.getColumnModel().getColumn(0).setCellRenderer(new CheckBoxTableCellRenderer());
        transTable.getColumnModel().getColumn(0).setMaxWidth(checkW);
        transTable.getColumnModel().getColumn(0).setMinWidth(checkW);
        transTable.getColumnModel().getColumn(1).setPreferredWidth(tableW - checkW - 4);
    ...
    /**
     * Adds a row to the table for each transcription with unsaved data. The first column of each row is a Boolean, defaults
     * to true, the second contains the transcription/frame name. After the rows have been added the (minimum) width of the
     * name column is calculated.
     *
    ...
    private void fillTable(List<String> changedTrans) {
        if (changedTrans != null) {
            ...
            if (ws.length() > 0) {
                ...
                int w = transTable.getFontMetrics(transTable.getFont()).stringWidth(ws);
            ...
            for (int i = b; i <= e; i++) {
                if (transTable.isRowSelected(i)) {
                    ...
                    if (curValue instanceof Boolean) {
                        if (Boolean.TRUE.equals(curValue)) {

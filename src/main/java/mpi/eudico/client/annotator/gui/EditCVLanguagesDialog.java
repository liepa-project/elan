...
public class EditCVLanguagesDialog extends AbstractEditLanguagesDialog implements ActionListener {
    private final ControlledVocabulary cv;
    ...
    /**
     * Enable/disable the buttons based on the current state. For instance, the last language cannot be deleted, and the
     * first language must be changed before you can add more.
     */
    @Override
    protected void updateButtons() {
        super.deleteButton.setEnabled(getNumberOfLanguages() > 1);
        boolean adding =
            getNumberOfLanguages() > 1 || !getLongLanguageId(0).equals(BasicControlledVocabulary.DEFAULT_LANGUAGE_DEF);
        super.addButton.setEnabled(adding);
    }

    /**
     * In the editor for the languages to be used in the CV, we don't want to have the whole official collection of languages
     * available. Therefore we just allow to select from the "recent languages" list, with no editing or other option.
     *

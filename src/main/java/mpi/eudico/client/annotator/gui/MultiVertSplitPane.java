...
/**
 * Experimental splitpane for more than two components. Not used at the moment.
 */
...
public class MultiVertSplitPane extends JComponent implements ComponentListener,
    ...
    private final List<Container> components;
    private final List<Container> dividers;
    ...
    /**
     * gets component present at the index
     *
     * @param index the index
     *
    ...
    public Component getComponentAt(int index) {
        if (index < 0 || index > components.size() - 1) {
            throw new ArrayIndexOutOfBoundsException("Index: "
                                                     + index
                                                     + " < 0 or >= than number of components: "
                                                     + components.size());
        }
    ...
    private JPanel createDivider() {
        ...
        try {
            ImageIcon icon = new ImageIcon(this.getClass()
                                               .getResource("/mpi/eudico/client/annotator/resources/ResizeDivider
                                               .gif"));
    ...
    public void componentResized(ComponentEvent e) {
        ...
        if (curSPH == -1) { // distribute equally
            ...
            for (int i = components.size() - 1; i > -1; i--) {
                components.get(i)
                          .setBounds(border,
                                     (int) (i * cHeight) + (i * DIV_HEIGHT) + border,
                                     getWidth() - (2 * border),
                                     (int) cHeight);
            ...
            for (int i = 0; i < dividers.size(); i++) {
                dividers.get(i)
                        .setBounds(border,
                                   (int) ((i + 1) * cHeight) + (i * DIV_HEIGHT) + border,
                                   getWidth() - (2 * border),
                                   DIV_HEIGHT);
        ...
        components.get(components.size() - 1).setBounds(components.get(components.size() - 1).getX(), 0, lastWidth,
        getHeight());

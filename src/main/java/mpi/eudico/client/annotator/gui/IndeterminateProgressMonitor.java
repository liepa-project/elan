...
/**
 * A ProgressMonitor that displays an indeterminate JProgressBar. It can display a message above the progressbar and a cancel
 * button below the progressbar (both optional).  The monitor can be modal or non-modal, and cancellable or non-cancellable.
 * It is the caller's responsibility to make sure the monitor is closed at the end of the process or when an exception occurs
 * during the process. Cancelling only sets the boolean <code>cancelled</code> to true. It does not stop the process it is
 * monitoring; the caller should use
...
public class IndeterminateProgressMonitor {
    ...
    /**
     * Creates a modal or non-modal progress monitor positioned relative to  the frame containing <code>parent</code> or
     * parent itself when it is a frame.<br> The constructor does not make the monitor visible. <code>show</code> should  be
     * called to make it visible.
     *
     ...
     */
    public IndeterminateProgressMonitor(Component parent, boolean modal, String message, boolean cancellable,
                                        String cancelText) {
        this.parent = parent;
        this.modal = modal;
    ...
    /**
     * Creates the dialog, adds the proper components and makes the dialog visible. It may be necessary to use a separate
     * thread for the process and/or the monitor to make it visible.
     */
    public void show() {
        if (cancelled) {
            return;
    ...
    /**
     * When called before {@link #show()} this flag determines whether the dialog will be undecorated or not.
     *

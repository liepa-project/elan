...
public class FormattedMessageDlg {
     ...
     *
     * @param mes a two-dimensional array of Strings, that will be shown in  a html table in a message dialog
     */
    ...
    /**
     * Creates a new FormattedMessageDlg instance that extracts information from the player's media descriptor.
     *
    ...
    private String[][] createMediaInfo(ElanMediaPlayer player) {
        ...
        info[4][1] = String.format("%.4f", 1000 / player.getMilliSecondsPerSample());
    ...
    private void showMessage(ElanMediaPlayer player, String[][] mes) {
        ...
        if (cc != null) {
            JOptionPane.showMessageDialog(SwingUtilities.getWindowAncestor(cc),
                                          label,
                                          "ELAN",
                                          JOptionPane.INFORMATION_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(null, label, "ELAN", JOptionPane.INFORMATION_MESSAGE);
        }
    }
}

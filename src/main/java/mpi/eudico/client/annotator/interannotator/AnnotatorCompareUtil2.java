...
/**
 * Extends the AnnotatorCompareUtil class with a method that calculates an average for two lists (corresponding to two tiers)
 * of AnnotationCore objects.
 */
public class AnnotatorCompareUtil2 extends AnnotatorCompareUtil {
    ...
    /**
     * Calculates the average overlap / extent ratio for the entire combination of the segments of two tiers. Doesn't store
     * and preserve the ratio of individual combinations.
     *
    ...
    /**
     * Creates and returns a map of annotations with the largest overlap. Annotations not in the map don't have a counterpart
     * in the other group
     *
     ...
     *
     * @param compareCombi the two lists of annotations
     * @param minimalOverlapPercentage the minimal required overlap, the overlap as a percentage of the largest interval
     *
     * @return a map of annotations with the largest overlap
     */
        ...
        while (allOverlaps.size() > 0) {
            ...
            for (int k = curGroup.size() - 1; k >= 0; k--) {
                ...
                if (iterOP.rawOverlap > bestCandidate.rawOverlap) {
                    ...
                    if (minST < bestCandidate.ac1.getBeginTimeBoundary()
                        || minST < bestCandidate.ac2.getBeginTimeBoundary()) {
                        bestCandidate = iterOP;
                    }
                }
            }
    ...
    /**
     * Calculates the percentage of the overlap of two intervals of the longest of the two intervals.
     *
    ...
    public double calculateOverlapPercentage(long bt1, long et1, long bt2, long et2) {
        return (Math.min(et1, et2) - Math.max(bt1, bt2)) / (double) Math.max(et1 - bt1, et2 - bt2);
    }

    /**
     * A class to hold two annotations/segments, the percentage of their overlap and the raw overlap in milliseconds.
     */
    private static class OverlapPair implements Comparable<OverlapPair> {
        ...
        public int compareTo(OverlapPair op) {
            int result = Double.compare(percentage, op.percentage);

            if (result == 0) {
                result = Long.compare(rawOverlap, op.rawOverlap);
            }
        ...
        public boolean equals(Object o) {
            ...
            OverlapPair that = (OverlapPair) o;
            return Double.compare(percentage, that.percentage) == 0
                   && rawOverlap == that.rawOverlap
                   && Objects.equals(ac1,
                                     that.ac1)
                   && Objects.equals(ac2, that.ac2);
        }

        @Override
        public int hashCode() {
            return Objects.hash(ac1, ac2, percentage, rawOverlap);
        }
    }

 ...
 * the second step in the wizard dialog, helps in making a choice between the classical way of comparing and comparing by
 * calculating the value of kappa for the matrices generated from two annotated tiers. For additional comments, refer to the
 * first step.
 *
 * @author keeloo
 */
public class DocumenSelectionStep extends StepPane {
     ...
     *
     * @param wizard orchestrating the steps
     * @param transcription to select the tiers from
     */
    ...
    private void loadPreferences() {
        ...
        if (transcription == null) {
            ...
            if (prefTierSource != null) {
                if (CompareConstants.FILE_MATCHING.ACROSS_FILES.getValue().equals(prefTierSource)) {
                    multipleFileRB.setSelected(true);
                }// otherwise default, single file
            }
        } else {
            if (prefTierSource != null) {
                if (CompareConstants.FILE_MATCHING.CURRENT_DOC.getValue().equals(prefTierSource)) {
                    currentDocumentRB.setSelected(true);
                } else if (CompareConstants.FILE_MATCHING.IN_SAME_FILE.getValue().equals(prefTierSource)) {
                    singleFileRB.setSelected(true);
                } else if (CompareConstants.FILE_MATCHING.ACROSS_FILES.getValue().equals(prefTierSource)) {
                    multipleFileRB.setSelected(true);
                }
            }
        }
        ...
        if (prefTierMatch != null) {
            if (CompareConstants.MATCHING.MANUAL.getValue().equals(prefTierMatch)) {
                manualSelectRB.setSelected(true);
            } else if (CompareConstants.MATCHING.AFFIX.getValue().equals(prefTierMatch)) {
                affixBasedRB.setSelected(true);
            } else if (CompareConstants.MATCHING.SAME_NAME.getValue().equals(prefTierMatch) && sameNameRB.isEnabled()) {
    ...
    private void setStateOnRadioButtonEvent() {
        manualSelectRB.setEnabled(true);
        if (currentDocumentRB.isSelected()) {
            ...
            // prevent the combination of (Fleiss) group wise compare and different files and manual selection
            if (Boolean.TRUE.equals(multiPane.getStepProperty(CompareConstants.GROUP_COMPARE_KEY))) {
                manualSelectRB.setEnabled(false);
    ...
    public void updateLocale() {
        ...
        border.setTitle(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.Hint"));
        locationLabel.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.TierLocation"));
        // update the radio button's text
        currentDocumentRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.CurrentDocumentRB"));
        singleFileRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.SingleFileRB"));
        multipleFileRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.MultipleFileRB"));

        matchingLabel.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.TierMatching"));
        manualSelectRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.ManualMatching"));
        affixBasedRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.AffixBased"));
        sameNameRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.DocumentSelectionStep.SameName"));
    }

    /**
     * Create the panel representing the second step of the wizard dialog. The panel will be created by using a GridBagLayout
     * and a GridBagConstraints object.
     */
    ...
    /**
     * An object in this class that listens to changes in the state of the radio buttons. After a button is pushed, the other
     * button's state will set accordingly.
     */
    private class RadioButtonListener implements ActionListener {
        /**
         * Currently doesn't check which button has been clicked, the same checks are performed on any click.
         */
        @Override
    ...
    /**
     * Act on the message send when entering this step after choosing 'next' in the  step.
     *
    ...
    /**
     * Act on the message send when entering this step after choosing 'previous' in the succeeding step.
     *
     * @see mpi.eudico.client.annotator.gui.multistep.Step#enterStepBackward()
     */
    @Override
    ...
    public boolean leaveStepForward() {
        if (transcription != null) {
            ...
            if (currentDocumentRB.isSelected()) { // error condition, shouldn't happen
                LOG.warning(
                    "Cannot proceed to the next step when \"current\" is selected and there's no transcription loaded"
                    + ".");
                return false;
            }
        ...
        if (manualSelectRB.isSelected()) {
            ...
            multiPane.putStepProperty(CompareConstants.TIER_MATCH_KEY,
                                      CompareConstants.MATCHING.SAME_NAME); // only in combination with inMultipleFiles
        }

        // preferences
        CompareConstants.FILE_MATCHING curTierSource =
            (CompareConstants.FILE_MATCHING) multiPane.getStepProperty(CompareConstants.TIER_SOURCE_KEY);
        if (curTierSource != null) {
            Preferences.set(CompareConstants.TIER_SOURCE_KEY, curTierSource.getValue(), null);
        }
        CompareConstants.MATCHING curTierMatch =
            (CompareConstants.MATCHING) multiPane.getStepProperty(CompareConstants.TIER_MATCH_KEY);
        if (curTierMatch != null) {
            Preferences.set(CompareConstants.TIER_MATCH_KEY, curTierMatch.getValue(), null);
        }
    ...
    /**
     * Depending on the selected radio buttons either jump to the tier selection step or to the files selection step.
     */
    @Override

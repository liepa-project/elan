...
/**
 * Class for calculating several agreement values (loosely) based on Cohen's Kappa and variations thereof by Holle &amp;
 * Rein.
 *
 * <p>Holle, H., &amp; Rein, R. (2014). EasyDIAg: A tool for easy determination of interrater agreement.
 * Behavior Research Methods, published online August 2014. doi:10.3758/s13428-014-0506-7
 *
 * <p>The input is expected to be a matrix that includes a row and column for "unmatched" or "nil" values
 * (representing samples identified by only one of the raters). But kappa and max kappa are calculated for the matrix
 * excluding the unmatched values as well.
 *
 * <p>The private methods don't perform (m)any checks
 */
public class GlobalCKappa {
    ...
    public GlobalCKappa(int[][] matrix, boolean matrixIncludesUnmatched) {
        ...
        numDiagonalTotalMatched = getDiagonalSum(matrixIntMatched);
        ...
        if (chanceAgreementMatched == 1 || Double.isNaN(chanceAgreementMatched)) {
            ...
            //            }
            //kappaMatched =  Math.max( (rawAgreementMatched - chanceAgreementMatched) / (1 - chanceAgreementMatched)
            // , 0);
        ...
        numDiagonalTotalAll = getDiagonalSum(matrix);
        if (numTotalAll == 0) {
    ...
    /**
     * Returns an array of int, each element containing the sum of the corresponding row in the input table.
     *
    ...
    /**
     * Returns an array of int, each element containing the sum of the corresponding row in the input table, excluding the
     * last column, the unmatched or nil values.
     *
     * @param matrix a square matrix
     *
    ...
    /**
     * Returns an array of int, each element containing the sum of the corresponding column in the input table.
     *
     * @param matrix a square matrix
     *
    ...
    private int[][] getSubMatrix(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.length - 1; i++) {
            ...
            System.arraycopy(matrix[i], 0, subMatrix[i], 0, matrix.length - 1);
        }
    ...
    /**
     * Calculates the total sum of all "matched" values, i.e. it calculates the total of all cell excluding those in the last
     * column and last row.
     *
    ...
    /**
     * Returns the total of the values on the agreement diagonal, from top left corner to lower right corner.
     *
    ...
    /**
     * Returns the total of the values on the agreement diagonal, from top left corner to lower right corner, excluding the
     * very last cell.
     *
    ...
    /**
     * Calculates the overall chance agreement as the sum of the product of corresponding row- (R1) and column (R2) marginal
     * values-as-percentages-of-total. The length of the two arrays should be the same.
     *
    ...
    /**
     * Calculates the maximal marginal probability (poM or Pmax) value used for calculating the maximum kappa given the
     * marginal distribution. The values is based on the minimum values of corresponding row and column totals.
     *
     * @param rowMarginals the row marginal values (total per row)
     * @param colMarginals the column marginal values (total per column)
     * @param totalSum the total number of scores, not 0

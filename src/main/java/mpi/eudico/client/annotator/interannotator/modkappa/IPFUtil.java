...
/**
 * A utility class for performing Iterative Proportional Fitting on a two dimensional matrix given row and column marginals.
 * Loosely based on the code of the EasyDIAg application (implemented in Matlab) and on work of Eddie Hunsinger (and others).
 * IPF is applied to adjust data cells of a two dimensional table such that the rows and columns add up to the given marginal
 * values.
 */
public class IPFUtil {
    ...
    /**
     * Calculates adjusted cell values as doubles. For each iteration first the row values are adjusted, then the column
     * values. Algorithm: if CV is a cell's value, CT is the current total of the cells in the row or column and MT is the
     * (target) marginal total value, to each cell apply: <br> {@code (CV / CT) * MT}<br> In words: the cell value divided by
     * the actual total of the cells and then multiplied by the marginal total.
     *
     * @param rowMarginals the array of row totals
     * @param columnMarginals the array of column totals
     * @param matrix the array of arrays of cell values. The length of the outer array (the rows) should be equal to the
     *     length of the row marginals array, the length of the inner arrays should be equal to length of the column
     *     marginals
    ...
    public double[][] applyIPF(int[] rowMarginals, int[] columnMarginals, int[][] matrix, int numIterations) {
        ...
        if (matrix[0].length != columnMarginals.length) { // assume there is at least 1 row in the table
            throw new IllegalArgumentException(
                "The number of column marginals should be equal to the number of columns in the table."); // add the
            // numbers?
        }
    ...
    /**
     * Calculates the sum of all values in an array. No error checking.
     *
    ...
    /**
     * Calculates the sum of a column in a table, i.e. the sum of all n-th element of each array in the array. No error
     * checking.
     *

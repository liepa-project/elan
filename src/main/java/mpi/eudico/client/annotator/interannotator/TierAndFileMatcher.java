...
/**
 * Utility class for matching tiers (tier names) or files (file names) based on same name with different prefix or suffix.
 */
public class TierAndFileMatcher {
    ...
    /**
     * Creates a list of matching tier names (if any) for each tier name in the list of selected tier names.
     *
    ...
    /**
     * Combines files based on the file names (excluding/ignoring the file path). If the tiers to compare are in different
     * files, e.g. File1_A1.eaf and File1_A2.eaf, and the files can be matched based on some encoding in the file name.
     *
     ...
     * @param fileExtension the extension of the file which is to ignore while doing the matching. Can be null, in which
     *     case it is assumed that the bit after the last "." is the extension
     *
    ...
    public List<List<File>> getMatchingFiles(List<File> selFiles, CompareConstants.MATCHING fileMatching,
        ...
        if (fileExtension != null) {
            loExtension = fileExtension.toLowerCase(Locale.getDefault());
        }
    ...
    private String noExtFileName(String fileName, String fileExtension) {
        if (fileName != null) {
            if (fileExtension != null) {
                if (fileName.toLowerCase(Locale.getDefault()).endsWith(fileExtension)) {
    ...
    /**
     * Checks whether the input string matches the specified substring. The substring is the first source, without the prefix
     * or suffix but with the delimiter as the first or last character.
     *

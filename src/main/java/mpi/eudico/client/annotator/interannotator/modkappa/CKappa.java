...
/**
 * Class for calculations of kappa agreement values based on a 2x2 contingency table. Also calculates kappa max as in
 * Holle&amp;Rein's EasyDIAg application
 */
public class CKappa {
    ...
    /**
     * Calculates the raw agreement. Algorithm: sum of diagonal ([0,0],[1,1]) / total (of all positions)
     *
    ...
    /**
     * Calculates chance agreement. Algorithm: percentage positive scores of R1 x percentage positive scores of R2 plus
     * percentage negative scores of R1 x percentage negative scores of R2
     *
     * <p>N = total sum
     * (([0,0] + [0,1]) / N)  *  (([0,0] + [1,0]) / N) + (([1,0] + [1,1]) / N)  *  (([0,1] + [1,1]) / N)
     *
     * @param matrix 2x2 matrix, not null
     *
    ...
    /**
     * Calculates (roughly) Cohen's kappa, algorithm: (raw agreement - chance agreement) / (1 - chance agreement)
     *
     * @param matrix 2x2 matrix, not null
     *
     * @return kappa value
    ...
    /**
     * Calculates the maximum kappa value for the given matrix. Algorithm according to Holle &amp; Rein: k_max = (poM - pe) /
     * (1 - pe) pe = change agreement poM = sum of minimal values of corresponding row and column totals (marginals) divided
     * by the total sum
     *
    ...
    private double calcMaxKappaPr(int[][] matrix) {
        ...
        int minSum = 0;
        minSum += Math.min(matrix[0][0] + matrix[0][1],
                           matrix[0][0] + matrix[1][0]); // minimal value of sum of row(0) and sum of col(0)
        minSum += Math.min(matrix[0][1] + matrix[1][1],
                           matrix[1][0] + matrix[1][1]); // minimal value of sum of row(1) and sum of col(1)

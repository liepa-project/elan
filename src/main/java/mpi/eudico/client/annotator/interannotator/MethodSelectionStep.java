 ...
 * the first step in the wizard dialog, helps in making a choice between the classical way of comparing and comparing by
 * calculating the value of kappa for the matrices generated from two annotated tiers.
 *
 * <p>Because the class extends LocalizableStepPane, it needs to conform to the
 * limitations imposed by that class. This means that methods need to invoke corresponding methods from the superclass first.
 * Unless such a method is kind of abstract of course.
 *
 * @author keeloo
 */
public class MethodSelectionStep extends StepPane {
    ...
    /**
     * This method will be invoked on a change in language preference. The text associated with the components will retrieved
     * by means of an ElanLocale object.
     */
    public void updateLocale() {

        // update the border text
        border.setTitle(ElanLocale.getString("CompareAnnotatorsDialog.MethodSelectionStep.Hint"));

        // update the radio button's text
        classicCompareRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.MethodSelectionStep.ClassicRB"));
        kappaCompareRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.MethodSelectionStep.KappaRB"));
        staccatoCompareRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.MethodSelectionStep.StaccatoRB"));
        fleissKappaCompareRB.setText(ElanLocale.getString("CompareAnnotatorsDialog.MethodSelectionStep.FleissKappaRB"));
    }

    /**
     * Create the panel representing the first step of the wizard dialog. The panel will be created by using a GridBagLayout
    ...
    public void createPanel() {
        ...
        if (prefMethod != null) {
            if (CompareConstants.METHOD.CLASSIC.getValue().equals(prefMethod)) {
                classicCompareRB.setSelected(true);
            } else if (CompareConstants.METHOD.MOD_KAPPA.getValue().equals(prefMethod)) {
                kappaCompareRB.setEnabled(true);
            } else if (CompareConstants.METHOD.STACCATO.getValue().equals(prefMethod)) {
                staccatoCompareRB.setSelected(true);
            } else if (CompareConstants.METHOD.MOD_FLEISS.getValue().equals(prefMethod)) {
     ...
     *
     * @return the identifier of the preferred next step; when null the wizard will follow the steps in the order of
     *     declaration.
     */
    @Override
    ...
    /**
     * Act on message from the wizard when entering this step after choosing 'previous' in the succeeding step.
     *
     * @see mpi.eudico.client.annotator.gui.multistep.Step#enterStepBackward()
     */
    @Override
    ...
    public boolean leaveStepForward() {
        if (classicCompareRB.isSelected()) {
            ...
            Preferences.set(CompareConstants.METHOD_KEY, CompareConstants.METHOD.CLASSIC.getValue(), null);
        } else if (kappaCompareRB.isSelected()) {
            multiPane.putStepProperty(CompareConstants.METHOD_KEY, CompareConstants.METHOD.MOD_KAPPA);
            multiPane.putStepProperty(CompareConstants.GROUP_COMPARE_KEY, Boolean.FALSE);
            Preferences.set(CompareConstants.METHOD_KEY, CompareConstants.METHOD.MOD_KAPPA.getValue(), null);
            ...
            multiPane.putStepProperty(CompareConstants.GROUP_COMPARE_KEY, Boolean.FALSE);
            Preferences.set(CompareConstants.METHOD_KEY, CompareConstants.METHOD.STACCATO.getValue(), null);
            ...
            Preferences.set(CompareConstants.METHOD_KEY, CompareConstants.METHOD.MOD_FLEISS.getValue(), null);
        } else {
            return false;
        }

...
/**
 * This step provides ui elements for method-specific customization; depending on the selected method in the first step
...
public class CustomizationStep extends StepPane {
    ...
    private final int defPercentage = 60;
    ...
    /**
     * Updates the message to the selected locale
     */
    ...
    /**
     * Called when this panel becomes visible after leaving the first step (the method selection step) and when the new
     * method differs from a possible current or previous method. Removes the current gui elements, if any at all, and
     * populates the panel with appropriate elements.
     *
    ...
    private void updatePanel(CompareConstants.METHOD method) {
        ...
        if (method == CompareConstants.METHOD.MOD_KAPPA || method == CompareConstants.METHOD.MOD_FLEISS) {
            // add label + slider + basic or extensive output
            JLabel percentageLabel =
                new JLabel(ElanLocale.getString("CompareAnnotatorsDialog.CustomizeStep.PercentageLabel"));
            ...
            verboseOutCB = new JCheckBox(ElanLocale.getString("CompareAnnotatorsDialog.CustomizeStep.VerboseOutput"), false);
            ...
            if (method == CompareConstants.METHOD.MOD_KAPPA) {
                ...
                if (verbosePref != null) {
                    verboseOutCB.setSelected(verbosePref);
                }
            } else if (method == CompareConstants.METHOD.MOD_FLEISS) {
                percentageLabel.setText(ElanLocale.getString("CompareAnnotatorsDialog.CustomizeStep"
                                                             + ".PercentageLabelAverage"));
    ...
    public void enterStepForward() {
        CompareConstants.METHOD method = (CompareConstants.METHOD) multiPane.getStepProperty(CompareConstants.METHOD_KEY);

...
/**
 * An AnnotatorCompare object holds a wizard dialog for configuring and carrying out rater agreement analysis.
 *
 * <p>Review comments
 *
 * @author keeloo
...
public class AnnotatorCompare {
    ...
    /**
     * Create wizard dialog for comparing raters after receiving a message from a CompareAnnotatorsDlgCommand object.
     *
     * @param transcription transcription currently associated with a frame object
     * @param frame JFrame object that is parent to the dialog
     * @param modal if and only if true, the dialog will be modal
     */
    public AnnotatorCompare(TranscriptionImpl transcription, JFrame frame, Boolean modal) {
        ...
        dialog = wizard.createDialog(frame, ElanLocale.getString("CompareAnnotatorsDialog.Title"), modal);
    ...
    /**
     * Create wizard dialog for comparing raters after receiving a message from a CompareAnnotatorsMA object.
     *
     ...
     *
     * <p>Notes on the wizard. Forward steps are disabled by default, backward steps
     * are enabled by default. In the dialog belonging to a step, use putStepProperty to convey the information obtained in
     * the step between the steps. At any step, the choices can be inspected and the appropriate actions can be taken.
     *
     * <p>The path taken by the steps is controlled by the wizard buttons: previous,
     * next, cancel, finish. Next to this, the path can be altered by skipping to a certain step.
     *
     * @param wizard to add steps to
     *
     * @return wizard
     */

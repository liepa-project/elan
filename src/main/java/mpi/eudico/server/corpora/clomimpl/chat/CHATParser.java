...
/**
 * A parser for {@code CHAT (.cha)} transcription files 
 * (see the <a href="https://talkbank.org/">TalkBank</a> documentation).
 * 
...
public class CHATParser extends Parser {
	...
	public ArrayList<String> getTimeOrder(String fileName) {
		...
		long[] lastAddedSlot = null;
		if (timeOrder.size() > 0) {
			lastAddedSlot = (long[]) timeOrder.get(timeOrder.size() - 1);
		}
		if (lastAddedSlot != null && timeSlots.indexOf(lastAddedSlot) != timeSlots.size() - 1) {	 // not last
		...
		// Functionality to check if all top level tiers are unaligned was removed
		// from TranscriptionImpl.
		// Align them all here now, using a default annotation duration of 1 second.
		int[] nums = numUnalignedSlots();
		if (nums[0] > 0 && nums[0] == nums[1]) {
			alignAllSlots();
		} else if (nums[1] > 0) {
			// some slots unaligned but not all, wasn't handled before, could handle it here
		}
	...
	/*
	 * Returns an array of size 2; index 0 is the total number of
	 * slots (on top level tiers, could be added), index[1] is the
	 * number of unaligned slots.
	 */
	private int[] numUnalignedSlots() {
		// nums[0] total slots, nums[1] unaligned slots
		int[] nums = new int[2];
		/*
		List<String> topTiers = new ArrayList<String>();
		for (String name : tierNames) {
			if (!parentHash.containsKey(name)) {
				topTiers.add(name);
			}
		}
		List<AnnotationRecord> topRecords = new ArrayList<AnnotationRecord>();
		for (String name : topTiers) {
			topRecords.addAll(getAnnotationsOf(name, lastParsed));
		}
		*/
		for (long[] slot : timeOrder) {
			if (slot[1] == TimeSlot.TIME_UNALIGNED) {
				nums[1]++;
			}
			nums[0]++;
		}
	...
	/*
	 * If all slots are unaligned, they are given a calculated time based on
	 * a fixed annotation duration. Even slots are begin time slots, odd slots
	 * are end time slots.
	 */
	private void alignAllSlots() {
		final int dur = 1000;
		int count = 0;
		
		for (int i = 0; i < timeOrder.size(); i++) {
			long[] slot = timeOrder.get(i);
			if (i % 2 == 0) {
				slot[1] = dur * count++;
			} else {
				slot[1] = dur * count;
	...
	public ArrayList<AnnotationRecord> getAnnotationsOf(String tierName, String fileName) {
		...
		for (Map.Entry<AnnotationRecord, String> recordEntry : annotRecordToTierMap.entrySet()) {
			if (recordEntry.getValue().equals(tierName)) {
				resultAnnotRecords.add(recordEntry.getKey());
			}
		}
	...
	private void parse(String fileName) {			
		...
		// read to check the presence of an encoding header
		try (BufferedReader br = new BufferedReader(new FileReader(fileName))){
			// start with assuming the file is in default encoding
			String line = null;
			do {
				line = br.readLine();
		...
		if (charset == null) {
			...
			System.out.println("The CHAT file does not seem to have any contents: " + fileName);
			return;
		}
	...
	private void parseLines(BufferedReader br) throws IOException {
		String line = null;
		String outputLine = "";

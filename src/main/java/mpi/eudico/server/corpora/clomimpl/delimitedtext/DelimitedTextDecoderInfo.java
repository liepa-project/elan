...
public class DelimitedTextDecoderInfo implements DecoderInfo {
	...
	/**
	 * Returns the time format of the specified column.
	 * 
	 * @param column the index of the column to find the format for
	 * @return the detected time format for the column, or {@code null}
	 */
	...
	/**
	 * Returns the path of a template file ({@code .etf}) defining the tiers onto which
	 * annotations, imported from the text file, have to be added. This depends on tier
	 * name equivalence in the text and {@code .etf} file. The importer tries to enforce
	 * constraints defined by the tier relations in the template. 
	 * 
	 * @return the path to an {@code .etf} file or {@code null} in case of a plain import
	 */
	...
	/**
	 * Sets the path of a template file ({@code .etf}) if the contents of the text file
	 * needs to be fitted into the structure of a template.
	 * 
	 * @param templateFilePath the full path of a template ({@code .etf}) file
	 */

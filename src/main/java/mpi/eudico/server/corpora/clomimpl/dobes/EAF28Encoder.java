...
//##### per element methods that can be overridden in future versions of the encoder #####//
    ...
    protected void addMediaDescriptors(EAFBase eafFactory, Element headerElement, 
        ...
        while (mdIter.hasNext()) {
            ...
            if (md.extractedFrom != null && !md.extractedFrom.isEmpty()) {
            ...
            for (Annotation ann : t.getAnnotations()) {
                ...
                if (ann instanceof AlignableAnnotation) {
                   ...
                   annSubElement = eaf28Fact.newAlignableAnnotation(annId,
                    ...
                    // log a message if this reference is null. Further up in the call tree this should
                    // result in a runtime exception
                    if (refId == null) {
                    	if (LOG.isLoggable(Level.WARNING)) {
                    		LOG.warning(String.format("The reference annotation with id \"%s\" has no reference to a parent.", ann.getId()));
                    	}
                    }
...
//    				}
    ...
    protected void addLexiconRefs(EAFBase eafFactory, Element annotDocument, Transcription transcription, 
        ...
        for (Map.Entry<String, LexiconQueryBundle2> lexRefEntry : lexRefs.entrySet()) {
            String lexRef = lexRefEntry.getKey(); 
        	LexiconQueryBundle2 queryBundle = lexRefEntry.getValue();
        ...
        for (Map.Entry<String, LexiconLink> lexLinkEntry : lexiconLinks.entrySet()) {
        	String linkName = lexLinkEntry.getKey();
        	if(!savedLexiconLinks.containsKey(linkName)) {
        		Element lexSrvcElement = eaf28Fact.newLexiconLink("lr" + lexRefIndexMut.intValue++, 
        				lexLinkEntry.getValue());
        		annotDocument.appendChild(lexSrvcElement);
        	}
        }            	
    }
    ...
    /**
     * Adds elements for external resource references.
     * 
  ...
  //##### end per element methods that can be overridden in future versions of the encoder #####//
    ...
    /**
     * Adds a time zone string to date string.
     * 
     * @param calendar a calendar object providing the local time zone
     * @param dateString a formatted date string
     * @return the date string with time zone appended
     */
	...
	*/
     ...
     * Saves the document.
     * 
     * @param documentElement the document root element
     * @param path the output file location
     * @throws IOException any wrapped exception
     */

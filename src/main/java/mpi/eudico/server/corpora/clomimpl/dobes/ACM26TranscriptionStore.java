...
public class ACM26TranscriptionStore implements TranscriptionStore {
 ...
 //           typesCopy.add(i, linguisticTypes.get(i));
        ...
        for (Map.Entry<TierImpl, String> parentEntry : parentHash.entrySet()) {
        	TierImpl depTier = parentEntry.getKey();
        	depTier.setParentTier(attisTr.getTierWithId(parentEntry.getValue()));
        }
        ...
        HashMap<TierImpl, ArrayList<Annotation>> tempAnnotationsForTiers = new HashMap<TierImpl, ArrayList<Annotation>>();
        ...
        while (tierIter.hasNext()) {
            ...
            while (it1.hasNext()) {
                ...
                if (extReferences != null && annotationRecord.getExtRefId() != null) {
                	ExternalReferenceImpl eri = extReferences.get(annotationRecord.getExtRefId());
                	if (eri != null) {
        ...
        for (Map.Entry<String, String> referenceEntry : references.entrySet()) {
            String key = referenceEntry.getKey();
            Annotation referedAnnotation = idToAnnotation.get(referenceEntry.getValue());
        ...
        for (Map.Entry<String, String> chainEntry : referenceChains.entrySet()) {
            RefAnnotation previous = (RefAnnotation) idToAnnotation.get(chainEntry.getValue());
            RefAnnotation a = (RefAnnotation) idToAnnotation.get(chainEntry.getKey());
        ...
        for (Map.Entry<TierImpl, ArrayList<Annotation>> tierEntry : tempAnnotationsForTiers.entrySet()) {
            TierImpl t = tierEntry.getKey();
            ArrayList<Annotation> annots = tierEntry.getValue();
        ...
        Iterator<TierImpl> tierIter2 = tiers.iterator();

        while (tierIter2.hasNext()) {
            TierImpl t = tierIter2.next();

            if (t.isTimeAlignable() && t.hasParentTier()) {
                Iterator<AbstractAnnotation> alannIter = t.getAnnotations().iterator();
        ...
        if ((cvTable != null) && (cvTable.size() > 0)) {
        	...
        	for (Map.Entry<String, CVRecord> recordEntry : cvTable.entrySet()) {
        		String cvName = recordEntry.getKey();
        		if (cvName == null) {
        			continue;
        		}
        		cv = new ControlledVocabulary(cvName);
        		cv.setInitMode(true);
        		// the contents record can contain one description String
        		// and many CVEntryRecords
        		CVRecord contents = recordEntry.getValue();
        		...
        		if (contents.hasContents()) {
        			...
        			List<CVEntryRecord> entryRecords = contents.getEntries();
        			for (CVEntryRecord entryRec : entryRecords) {
        				CVEntry entry = new CVEntry(cv, entryRec.getValue(), entryRec.getDescription());
        				
						// check external reference
						if (extReferences != null && entryRec.getExtRefId() != null) {
							ExternalReferenceImpl eri = extReferences.get(
									entryRec.getExtRefId());
							if (eri != null) {
								try {
    ...
    /**
     * Concatenates the symbolic associations
    ...
    public void concatenateSymbolicAssociations(TranscriptionImpl transcription) {
		...
		while (tierIter.hasNext()) {
			...
			if (c != null && c.getStereoType() == Constraint.SYMBOLIC_ASSOCIATION) {
				...
				Iterator<AbstractAnnotation> annIter = t.getAnnotations().iterator();

...
public class ECV02Encoder {
	 ...
	 * @param urlString the source URL (the EXT_REF of the External CV)
	 * @throws IOException any IO exception
	 */
	...
	private Element createDOM(List<ExternalCV> cvs) {
        ...
        if (cvs != null) {
            ...
            for (LanguageRecord lr : langs.values()) {
        	...
        	for (ExternalCV cv : cvs) {
		        ...
		        for (CVEntry entry : cv) {
		        	String extRefId = getExtRefId(getExtRefIdParams, entry.getExternalRef());

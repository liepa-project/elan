...
public class Transcriber14Parser extends Parser {
	...
	public List<String> getTimeOrder(String fileName) {
		...
		for (Map.Entry<String, Long> slotEntry : timeSlots.entrySet()) {
			tempOrder.add(Pair.makePair(slotEntry.getKey(), slotEntry.getValue()));
		}
		
		Collections.sort(tempOrder, new PairComparator());
	
		for (Pair<String, Long> slotPair : tempOrder) {
			timeOrder.add(slotPair.getFirst());
		}
	...
	public List<AnnotationRecord> getAnnotationsOf(String tierName, String fileName) {
		...
		for (Map.Entry<AnnotationRecord, String> recordEntry : annotRecordToTierMap.entrySet()) {
			AnnotationRecord annRec = recordEntry.getKey();
    ...
    private void parse(String fileName) {
        ...
        try {
            ...
            saxParser.parse(is, transcriberHandler);
        } catch (SAXException e) {
            System.out.println("Parsing error: " + e.getMessage());
        } catch (IOException e) {
        	System.out.println("Parsing IO error: " + e.getMessage());
            //e.printStackTrace();
        } catch (Exception e) {
	...
	private void processSectionsAndTurns() {
		...
		for (int i = 0; i < sectionArrayList.size(); i++) {
			...
			long sectionBegin = (long) ((Double.parseDouble(sectionR.startTime)) * 1000);
			long sectionEnd = (long) ((Double.parseDouble(sectionR.endTime)) * 1000);
			...
			for (int j = 0; j < sectionR.turnRecords.size(); j++) {
				...
				long turnBegin = (long) ((Double.parseDouble(turnR.startTime)) * 1000);
				long turnEnd = (long) ((Double.parseDouble(turnR.endTime)) * 1000);
	...
	private void processUtterances() {
		...
		for (int i = 0; i < utteranceRecords.size(); i++) {
			...
			long uttBegin = (long) ((Double.parseDouble(utteranceR.startTime)) * 1000);
			long uttEnd = (long) ((Double.parseDouble(utteranceR.endTime)) * 1000);
	...
	private void processBackgrounds() {
		...
		for (int i = 0; i < backgroundArrayList.size(); i++) {
			...
			bgEndTime = (long) ((Double.parseDouble(backgroundR.time)) * 1000);

			if (lastBackgroundRecord != null && !lastBackgroundRecord.level.equals("off")) {	// not first background
				bgBeginTime = (long) ((Double.parseDouble(lastBackgroundRecord.time)) * 1000); 
		...
		if (lastBackgroundRecord != null && !lastBackgroundRecord.level.equals("off") && sectionArrayList.size() > 0) {		// handle last background record, end at last section end
			...
			if (sectionR != null) {
				bgBeginTime = (long) ((Double.parseDouble(lastBackgroundRecord.time)) * 1000); 
				bgEndTime = (long) ((Double.parseDouble(sectionR.endTime)) * 1000);
	...
	private void processComments() {
		...
		for (int i = 0; i < commentRecords.size(); i++) {
			...
			long commBegin = (long) ((Double.parseDouble(commentR.begin)) * 1000);
			long commEnd = (long) ((Double.parseDouble(commentR.end)) * 1000);
	...
	static class PairComparator implements Comparator<Pair<String, Long>>, Serializable {
		private static final long serialVersionUID = 4220005121637039231L;

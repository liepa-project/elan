...
/**
 * Class that implements a controller which loads video images from a video
 * source (image producer) and pushes an image to a video renderer with each
 * media time update.  
 * 
 * @author Han Sloetjes
 *
...
public class JAVFVideoLoadController {
	...
	/**
	 * Returns the image for a specified time, if it is already in one of the
	 * controller's buffers.
	 *  
	 * @param time the time in milliseconds 
	 * @return the corresponding image if it is in one of the buffers, 
	 * {@code null} otherwise
	 */
...
//						}
	...
	/**
	 * CLeans up the controller and its buffers.
	 */
	public void close() {
		if (sequenceLoader != null && sequenceLoader.isAlive()) {

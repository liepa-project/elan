...
//	private WritableRaster raster;
	...
	public synchronized BufferedImage getVideoFrameImage(long sampleTime) {
		...
		if (numBytes > 0) {
			...
			if (LOG.isLoggable(System.Logger.Level.WARNING)) {
				LOG.log(System.Logger.Level.WARNING, "Error retrieving frame bytes from the MMFoundation image generator ");
			}
		}
	...
	*/
	...
	/**
	 * Activates or deactivates native debug mode.
	 * 
	 * @param debugMode if {@code true} native debug mode is activated
	 */
	public static native void setDebugMode(boolean debugMode);
	/**
	 * Returns the current native debug state.
	 * 
	 * @return {@code true} if debug mode is activate, {@code false} otherwise
	 */
